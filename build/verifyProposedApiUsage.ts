...
function getModifiedPackageJson():
	| { enabledApiProposals: string[]; engines: { vscode: string } }
	| undefined {
	try {
...
async function verifyProposedApiUsage() {
	if (
		context.payload.pull_request?.body?.includes(
			commentThatWillIgnoreVerification,
		)
	) {
		console.info(
			`Proposed API verification is ignored due to override in PR body.`,
		);
		return;
	}
	...
	const currentApiProposals = new Set(
		currentPackageJson.enabledApiProposals.sort(),
	);
	...
	const newApiProposalsAdded = modifiedPackageJson.enabledApiProposals.filter(
		(api) => !currentApiProposals.has(api),
	);
