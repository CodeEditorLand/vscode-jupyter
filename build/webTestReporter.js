			...
			if (req.method === "OPTIONS") {
				res.writeHead(200, {
					...
					"Access-Control-Allow-Methods":
						"GET, POST, DELETE, PUT, PATCH", // REQUIRED CORS HEADER
				...
				req.on("end", () => {
					try {
						...
						if (
							"command" in jsonData &&
							jsonData.command === "captureScreenShot"
						) {
...
async function addCell(cells, output, failed, executionCount) {
	...
	const fullTestNameHash = (
		await computeHash(output.fullTitle() || "", "SHA-256")
	).substring(0, 10);
...
exports.dumpTestSummary = async () => {
	try {
		...
		for (let output of summary) {
			...
			output.parent = {
				fullTitle: () =>
					("fullTitle" in currentParent
						? currentParent.fullTitle
						: "") || "",
			};
			if ("stats" in output) {
