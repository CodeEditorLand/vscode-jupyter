...
suite("Jupyter Hub Password Connect", () => {
	...
	function createJupyterHubSetup() {
		...
		when(loginHeaders.raw()).thenReturn({
			"set-cookie": ["super-cookie-login=foobar"],
		});
		...
		return async (
			...
			if (urlString === "http://testname:8888/hub/api") {
				...
				urlString ===
					"http://testname:8888/hub/api/users/test/tokens" &&
				init &&
				init.method === "POST" &&
				(init.headers as any).Referer ===
					"http://testname:8888/hub/login" &&
