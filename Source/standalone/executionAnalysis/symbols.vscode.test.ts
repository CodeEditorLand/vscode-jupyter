...
suite("Analysis", () => {
	test("Basic type dependencies", () => {
		...
		for (let i = 0; i < refsArr.length; i++) {
			const refs: ILocationWithReferenceKind[] = (
				refsArr[i][1] as unknown[]
			).map((ref: any) => ({
				uri: vscode.Uri.parse("df.ipynb").with({
					fragment: ref.uri.fragment,
				}),
				range: {
					start: {
						line: ref.range[0].line,
						character: ref.range[0].character,
					},
					end: {
						line: ref.range[1].line,
						character: ref.range[1].character,
					},
				},
			}));
