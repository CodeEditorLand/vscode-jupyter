...
export interface ILocationWithReferenceKindAndSymbol
	extends ILocationWithReferenceKind {
...
export class CellAnalysis {
	...
	getPredecessorCells(
		cell: vscode.NotebookCell,
		forceReadNotebook: boolean = false,
	): vscode.NotebookCell[] {
		...
		const slicedCellExecution = virtualCellList.slice(
			0,
	...
	getSuccessorCells(cell: vscode.NotebookCell): vscode.NotebookCell[] {
		...
		for (let i = cellIndex; i < virtualCellList.length; i++) {
			if (cellBitmap[i]) {
				const deps =
					this._cellRefs.get(
						virtualCellList[i].document.uri.fragment,
					) || [];

				const modificationDeps =
					modificationCellRefs.get(
						virtualCellList[i].document.uri.fragment,
					) || [];
				...
				mergedDeps.forEach((dep) => {
					const index = virtualCellList.findIndex(
						(item) =>
							item.document.uri.fragment === dep.uri.fragment,
					);
	...
	private _resolveDependencies(
		...
		if (reversedCellRefs.has(cellFragment)) {
			for (const dependency of reversedCellRefs.get(cellFragment)!) {
				const index = cellExecution.findIndex(
					(cell) => cell.document.uri.fragment === dependency,
				);

				if (index === -1) {
					if (!ignoreUnboundDependencies) {
						throw new Error(
							`Dependency ${dependency} is not in the execution list.`,
						);
					} else {
						continue;
					}
...
export class NotebookDocumentSymbolTracker {
				...
				if (e.notebook === this._notebookEditor.notebook) {
					e.contentChanges.forEach((change) => {
						...
						change.addedCells.forEach((cell) => {
							this._requestCellSymbols(cell, false).then(
								noop,
								noop,
							);
						});
					...
					e.cellChanges.forEach((change) => {
						if (change.document) {
							this._requestCellSymbols(change.cell, false).then(
								noop,
								noop,
							);
				...
				(e) => {
					...
					if (
						...
						this._cellExecution.push({
							cell: e.cell,
							executionCount:
								e.cell.executionSummary?.executionOrder ?? 0,
						});

						this._updateCellStatus(
							e.cell,
							CellExecutionStatus.Fresh,
						);
					}
				},
			),
	...
	private async _requestCellSymbolsSync() {
		...
		for (const key of _pendingRequestsKeys) {
			const cell = this._notebookEditor.notebook
				.getCells()
				.find((cell) => cell.document.uri.fragment === key);
	...
	async getPrecedentCells(cell: vscode.NotebookCell) {
		...
		const analysis = new CellAnalysis(
			this._notebookEditor.notebook,
			this._cellExecution,
			this._cellRefs,
		);
		...
		const staleCellIndex = precedentCells.findIndex(
			(cell) =>
				(this._staleCellRefs.get(cell.document.uri.fragment) ??
					CellExecutionStatus.Stale) === CellExecutionStatus.Stale,
		);

		const cellRanges = cellIndexesToRanges(
			(staleCellIndex === -1
				? precedentCells
				: precedentCells.slice(staleCellIndex)
			).map((cell) => cell.index),
		);
	...
	async getSuccessorCells(cell: vscode.NotebookCell) {
		...
		const successorCells = analysis.getSuccessorCells(
			cell,
		) as vscode.NotebookCell[];

		const cellRanges = cellIndexesToRanges(
			successorCells.map((cell) => cell.index),
		);
	...
	async debugSymbols() {
		...
		const locationsByUriFragment = new Map<
			string,
		...
		locationsByUriFragment.forEach((locations, fragment) => {
			const matcheEditor = vscode.window.visibleTextEditors.find(
				(editor) =>
					editor.document.uri.path ===
						this._notebookEditor.notebook.uri.path &&
			...
			if (matcheEditor) {
				...
				const dedupedLocations = locations.reduce(
					(
						acc: ILocationWithReferenceKind[],
						current: ILocationWithReferenceKind,
					) => {
						const isDuplicate = acc.find((item) =>
							areRangesEqual(item.range, current.range),
						);
	...
	private async _requestCellSymbols(
		cell: vscode.NotebookCell,
		synchronous: boolean,
	) {
		...
		setTimeout(() => {
			if (
				this._pendingRequests.get(cell.document.uri.fragment) !==
				request
			) {
				return;
			}
	...
	private async _getDocumentSymbols(cell: vscode.NotebookCell) {
		if (this._client.getDocumentSymbols) {
			...
			const symbols = await this._client.getDocumentSymbols(
				cell.document,
				tokenSource.token,
			);
	...
	private async _doRequestCellSymbols(
		cell: vscode.NotebookCell,
		...
		const references: (LocationWithReferenceKind & {
			associatedSymbol: ISymbol;
		})[] = [];
		...
		for (const symbol of symbols) {
			...
			if (symbolReferences) {
				references.push(
					...symbolReferences
						.filter(
							(ref) => ref.uri.scheme === "vscode-notebook-cell",
						)
						.map((ref) => ({
							...ref,
...
export class SymbolsTracker {
	...
	constructor(private readonly _client: INotebookLanguageClient) {
		...
		this._disposables.push(
			vscode.window.onDidChangeVisibleNotebookEditors((e) => {
				e.forEach((editor) => {
					if (
						!this._notebookDocumentSymbolTrackers.has(
							editor.notebook.uri.toString(),
						)
					) {
						this._notebookDocumentSymbolTrackers.set(
							editor.notebook.uri.toString(),
							new NotebookDocumentSymbolTracker(
								editor,
								this._client,
							),
						);
	...
	async debugSymbols(notebookDocument: vscode.NotebookDocument) {
		const tracker = this._notebookDocumentSymbolTrackers.get(
...
export class ExecutionFixCodeActionsProvider
	implements vscode.CodeActionProvider
{
	...
	async provideCodeActions(
		...
		const notebookDocuments = vscode.workspace.notebookDocuments.filter(
			(notebookDocument) =>
				notebookDocument.uri.path === document.uri.path,
		);
		...
		const tracker = this.symbolsTracker.getNotebookDocumentSymbolTracker(
		...
		const precedentCellsRanges =
			await tracker.getPrecedentCells(targetCell);
			...
			cellRangesToIndexes(precedentCellsRanges).map(async (index) => {
				...
				const symbolRef = symbols
					...
					.find(
						(s) =>
							s.uri.toString() ===
							targetCell?.document.uri.toString(),
					);
