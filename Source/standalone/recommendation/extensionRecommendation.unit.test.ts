...
suite("Extension Recommendation", () => {
	["kernelspec", "language_info"].forEach((whereIsLanguageDefined) => {
		["csharp", "fsharp", "powershell"].forEach((languageToBeTested) => {
			suite(
				`Notebook language '${languageToBeTested}' defined in ${whereIsLanguageDefined}`,
				() => {
					...
					function startNewSession() {
						onDidOpenNotebookDocument =
							new EventEmitter<NotebookDocument>();
						onNotebookControllerSelected = new EventEmitter<{
							notebook: NotebookDocument;
							controller: VSCodeNotebookController;
						}>();
						vscNotebook = mock<IVSCodeNotebook>();
						when(vscNotebook.onDidOpenNotebookDocument).thenReturn(
					...
					function createNotebook(language: string) {
						...
						const kernelSpecLanguage =
							whereIsLanguageDefined === "kernelspec"
								? language
								: undefined;
						const languageInfoLanguage =
							whereIsLanguageDefined === "kernelspec"
								? ""
								: language;
					...
					test(`Got recommendations once per session when selecting a kernel`, async () => {
						...
						const controller2 =
							createController(languageToBeTested);
						onNotebookControllerSelected.fire({
							notebook,
							controller,
						});
