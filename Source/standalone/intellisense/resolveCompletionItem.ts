...
const kernelIdsThatToNotSupportCompletionResolveOrAreTooSlowToReply =
	new Set<string>();
const totalNumberOfTimeoutsWaitingForResolveCompletionPerKernel = new Map<
	string,
	number
>();
...
export async function resolveCompletionItem(
	...
	if (
		kernelIdsThatToNotSupportCompletionResolveOrAreTooSlowToReply.has(
			kernelId,
		)
	) {
		return item;
	...
	const properties: TelemetryProperties<Telemetry.KernelCodeCompletionResolve> =
		{
			...
			kernelLanguage: getKernelConnectionLanguage(
	...
	const request = sendInspectRequest(
		message,
...
function handleKernelRequestTimeout(kernel: IKernel, monacoLanguage: string) {
	...
	let numberOfFailedAttempts =
		totalNumberOfTimeoutsWaitingForResolveCompletionPerKernel.get(
			kernelId,
		) || 0;
	numberOfFailedAttempts += 1;
	totalNumberOfTimeoutsWaitingForResolveCompletionPerKernel.set(
		kernelId,
	...
	if (
		...
		kernelIdsThatToNotSupportCompletionResolveOrAreTooSlowToReply.add(
			kernelId,
		);
		return;
	}
...
async function sendInspectRequest(
	...
	if (doesKernelHaveTooManyPendingRequests(kernel)) {
		traceInfo(
			`Too many pending requests ${getPendingRequestCount(
				kernel,
			)} for kernel ${kernel.id}, waiting for it to be ready.`,
		);
		await raceCancellation(
			token,
			waitForKernelToBeReadyToHandleRequest(kernel, token),
		);
	}
...
function checkHowLongKernelTakesToReplyEvenAfterTimeoutOrCancellation(
	...
	const timeout = setTimeout(() => {
		...
		traceWarning(
			`Timeout (after ${maxTime}ms) waiting to inspect code '${codeForLogging}'`,
		);
	}, maxTime);
...
export const pendingInspectRequests = new WeakMap<
	Kernel.IKernelConnection,
	{ count: number }
>();

function doesKernelHaveTooManyPendingRequests(
	kernel: Kernel.IKernelConnection,
) {
	...
	return (
		(pendingInspectRequests.get(kernel)?.count || 0) >= MAX_PENDING_REQUESTS
	);
}
