...
function handleKernelRequestTimeout(
	kernel: IKernel,
	monacoLanguage: string,
	kernelId: string,
) {
...
const emptyResult: KernelMessage.IInspectReplyMsg["content"] = {
	data: {},
	found: false,
	status: "ok",
	metadata: {},
...
function shoudlSendInspectRequest(kernel: IKernel) {
	...
	if (
		times.numberOfTimesMaxedOut >
		maxNumberOfTimesAllowedToExceedTimeoutBeforeIgnoringAllRequests
	) {
...
async function waitForKernelToBeReady(
	kernel: IKernel,
	token: CancellationToken,
) {
	if (!kernel.session?.kernel) {
...
const pendingInspectRequests = new WeakMap<
	Kernel.IKernelConnection,
	{ count: number }
>();
