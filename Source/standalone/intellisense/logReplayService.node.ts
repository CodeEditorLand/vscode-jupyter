...
export class LogReplayService implements IExtensionSyncActivationService {
	...
	constructor(
		...
		private readonly commandService: ICommandManager,
	...
	public activate() {
		...
		this.isLogActive = new ContextKey(
			EditorContexts.ReplayLogLoaded,
			this.commandService
		);
	...
	private async replayPylanceLog() {
		if (vscode.window.activeNotebookEditor) {
			const file = await this.appShell.showOpenDialog({
				title: "Open Pylance Output Log",
			});
	...
	private async step() {
		if (
			...
			if (
				step.textDocument.uri.includes("_NotebookConcat_") &&
				this.activeNotebook
			) {
			...
			if (!cell) {
				...
				cellData.outputs = [];
				cellData.metadata = {};
				const nbEdit = vscode.NotebookEdit.insertCells(index, [
					cellData,
				]);
	...
	private async parsePylanceLogSteps(fileName: string) {
		...
		const regex =
			/textDocument\/didChange'[\s\S]*?Params:\s(?<json_event>[\s\S]*?\n\})/g;
	...
	private async getConverter() {
		if (!this.converter && this.activeNotebook) {
			const { createConverter } = await import(
