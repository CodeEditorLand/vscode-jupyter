...
export class IntellisenseProvider
	implements INotebookCompletionProvider, IExtensionSyncActivationService
{
	private servers = new Map<string, Promise<LanguageServer | undefined>>();
	private activeInterpreterCache = new Map<
		string,
		PythonEnvironment | undefined
	>();
	private knownControllers: WeakMap<
		NotebookDocument,
		IVSCodeNotebookController
	...
	public activate() {
		...
		this.notebooks.onDidOpenNotebookDocument(
			this.openedNotebook,
			this,
			this.disposables
		);
		...
		this.notebooks.notebookDocuments.forEach((n) =>
	...
	public async getLanguageClient(notebook: NotebookDocument) {
		...
		const interpreterId = interpreter
			? getComparisonKey(interpreter.uri)
			: undefined;
		const server = interpreterId
			? await this.servers.get(interpreterId)
			: undefined;
	...
	private getActiveInterpreterSync(
		...
		const folder =
			...
			(this.workspaceService.rootFolder
				? this.workspaceService.rootFolder
				: undefined);
	...
	private async controllerChanged(e: {
		notebook: NotebookDocument;
		...
		if (!this.notebookPythonPathService.isUsingPylance()) {
			...
			const oldInterpreterId = oldInterpreter
				? getComparisonKey(oldInterpreter.uri)
				: undefined;
			const oldLanguageServer = oldInterpreterId
				? await this.servers.get(oldInterpreterId)
				: undefined;
	...
	private async openedNotebook(n: NotebookDocument) {
		if (
			...
			const interpreter =
				controller?.connection.interpreter ||
				(await this.interpreterService.getActiveInterpreter(n.uri));
	...
	private shouldAllowIntellisense(
		...
		const notebookInterpreter = controller
			? controller.connection.interpreter
			: this.getActiveInterpreterSync(uri);
		let notebookId = notebookInterpreter
			? getComparisonKey(notebookInterpreter.uri)
			: undefined;
		...
		if (
			...
			notebookId = activeInterpreter
				? getComparisonKey(activeInterpreter.uri)
				: undefined;
		}
