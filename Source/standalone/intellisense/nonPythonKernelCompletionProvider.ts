...
export class NotebookCellSpecificKernelCompletionProvider
	implements CompletionItemProvider
{
	...
	async provideCompletionItems(
		...
		if (
			...
			this.pendingCompletionRequest.get(document)?.version ===
				document.version
		) {
			return [];
		}
		this.pendingCompletionRequest.set(document, {
			position,
 ...
 */
@injectable()
export class NonPythonKernelCompletionProvider
	extends DisposableBase
	implements IExtensionSyncActivationService
{
	private readonly kernelCompletionProviders = new WeakMap<
		IKernel,
