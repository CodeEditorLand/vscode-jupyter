...
{
	private _id = uuid();
	private _commands: Map<string, Disposable[]> = new Map<
		string,
		Disposable[]
	>();
	...
	public static async createLanguageServer(
		...
		if (serverOptions) {
			...
			const {
				...
				ExecuteCommandRequest,
				LanguageClient,
			} = await import("vscode-languageclient/node");
			...
			const features: (StaticFeature | DynamicFeature<any>)[] = (
			...
			return new LanguageServer(client, interpreter, middleware, [
				outputChannel,
			]);
		}
	}
		...
		if (python) {
			...
			if (await fs.pathExists(runJediPath)) {
				...
				const serverOptions: ServerOptions = {
					command:
						interpreterPath.length > 0 ? interpreterPath : "python",
					args: [runJediPath],
				};
