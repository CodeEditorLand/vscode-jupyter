...
class NotebookCellSpecificKernelCompletionProvider
	implements CompletionItemProvider
{
	...
	async provideCompletionItems(
		...
		try {
			...
			const completionsFromOtherSourcesPromise =
				createDeferredFromPromise(
					raceCancellation(
						token,
						(
			...
			let otherCompletions = await raceTimeout(
				0,
			...
			return completions.filter((item) => {
				const label =
					typeof item.label === "string"
						...
						: item.label.label;
				return (
					!existingCompletionItems.has(label) &&
					!existingCompletionItems.has(`* ${label}`)
				);
			});
		} catch (ex) {
		...
		if (
			...
			items = kernelCompletions.content.matches.map((label, index) => {
				...
				this.previousCompletionItems.set(completionItem, {
					...dataToStore,
					originalCompletionItem: JSON.parse(
						JSON.stringify(completionItem),
					), // Used to resolve completion items.
				});
	...
	private async getKernelCompletion(
		code: string,
		cursor_pos: number,
		token: CancellationToken,
	) {
	...
	private async getPythonKernelCompletion(
		code: string,
		cursor_pos: number,
		token: CancellationToken,
	) {
		...
		const content = await execCodeInBackgroundThread<
			ICompleteReplyMsg["content"]
		>(this.kernel, [codeToExecute], token);
...
function sendCompletionTelemetry(
	...
	let timesExceededTimeout =
		lastTelemetryExceedingMaxTimeout.get(kernel)?.count || 0;
	if (measures.duration >= Settings.IntellisenseTimeout) {
		lastTelemetryExceedingMaxTimeout.set(kernel, {
			count: timesExceededTimeout + 1,
			measures,
			properties,
		});
	}
...
class KernelSpecificCompletionProvider
	extends DisposableBase
	implements CompletionItemProvider
{
	private cellCompletionProviders = new WeakMap<
		TextDocument,
 ...
 */
@injectable()
export class KernelCompletionProvider
	extends DisposableBase
	implements IExtensionSyncActivationService
{
	public readonly kernelCompletionProviders = new WeakMap<
		IKernel,
		...
		this._register(
			kernelProvider.onDidStartKernel(async (e) => {
				if (
					 ...
					 */
					void e.session.kernel.requestComplete({
						code: "__file__.",
						cursor_pos: 9,
					});
				}
...
function isKernelCompletionEnabled(resource: Resource) {
	return workspace
		.getConfiguration("jupyter", resource)
		.get<boolean>("enableKernelCompletions", false);
}
...
function getCompletionTriggerCharacter(kernel: IKernel) {
	...
	if (kernelLanguage in triggerCharacters) {
		...
		return Array.isArray(triggerCharacters[kernelLanguage])
...
function positionInsideString(line: string, position: Position) {
	...
	const lastIndex =
		lastIndexDoubleQuote >= 0 ? lastIndexDoubleQuote : lastIndexSingleQuote;
	return (
		index >= 0 &&
		position.character > index &&
		position.character <= lastIndex
	);
}
...
export function generatePythonCompletions(
	...
	const wordRangeWithTriggerCharacter =
		wordRange && charBeforeCursorPosition
			? wordRange.union(charBeforeCursorPosition)
			: undefined;
	...
	const result = completions
		.map((r, i) => {
			let itemText =
				typeof r.label === "string" ? r.label : r.label.label;
			...
			let newRange:
				| Range
				| { inserting: Range; replacing: Range }
				| undefined = undefined;
