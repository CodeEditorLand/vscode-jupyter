...
const formatters = new Map<
	string,
	(documentation: string) => MarkdownString | string
>([
	...
	lines.map((line, index) => {
		...
		if (
			...
			(typeof possibleSection.nextLine === "string"
				...
				: lines[index + 1].trim().replace(possibleSection.nextLine, "")
						.length === 0)
		) {
...
function convertRDocumentationToMarkdown(
	...
	lines.map((line) => {
		...
		const isSection =
			...
			// Same number of `_` & ``
			line.split("_").length ===
				line.split(String.fromCharCode(8)).length;
