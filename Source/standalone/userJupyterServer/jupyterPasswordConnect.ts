...
export class JupyterPasswordConnect {
	private savedConnectInfo = new Map<
		string,
	...
	public getPasswordConnectionInfo(options: {
		...
		if (!result) {
			result = this.getJupyterConnectionInfo({
				...
				if (
					!value ||
					(value.requiresPassword && Object.keys(value).length === 1)
				) {
	...
	private async getJupyterConnectionInfo(options: {
		...
		if (
			xsrfCookie &&
			sessionCookieName &&
			(sessionCookieValue || options.isTokenEmpty)
		) {
			sendTelemetryEvent(Telemetry.GetPasswordSuccess);
			const cookieString = `_xsrf=${xsrfCookie}; ${sessionCookieName}=${
				sessionCookieValue || ""
			}`;
	...
	private async needPassword(url: string): Promise<boolean> {
		...
		const response = await this.makeRequest(
			new URL("tree?", addTrailingSlash(url)).toString(),
			{
				method: "get",
				redirect: "manual",
	...
	private async getSessionCookie(
		...
		const response = await this.makeRequest(
			new URL("login?", addTrailingSlash(url)).toString(),
			{
				method: "post",
				headers: {
