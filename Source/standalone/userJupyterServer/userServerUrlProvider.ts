		...
		JupyterServerCommandProvider
{
	...
	async provideCommands(
		...
		return [
			{
				label: DataScience.jupyterSelectUriCommandLabel,
			...
			this.migratedOldServers = this.oldStorage
				...
				.then(async (servers) => {
					...
					const split = blob.split(
						Settings.JupyterServerRemoteLaunchUriSeparator
					);
					split
						.slice(0, Math.min(split.length, indexes.length))
						.forEach((item, index) => {
							try {
								const uriAndDisplayName = item.split(
								...
								const serverInfo = parseUri(
									uri,
									uriAndDisplayName[1] || uri
								);
								if (
									serverInfo &&
									servers.every((s) => s.uri !== uri)
								) {
									// We have a saved Url.
									const handle = uuid();
	...
	private initializeServers(): Promise<void> {
		...
		(async () => {
			const NEW_STORAGE_MIGRATION_DONE_KEY =
				"NewUserUriMigrationCompleted";
			if (
				this.globalMemento.get<string>(
					NEW_STORAGE_MIGRATION_DONE_KEY
				) !== env.machineId
			) {
				await Promise.all([
					this.migrateOldServers().catch(noop),
					this.newStorage.migrationDone,
				]);
	...
	private recommendInstallingJupyterHubExtension() {
		if (
			workspace
				...
				.get<boolean>("bypassJupyterHubExtensionCheck")
		) {
			return;
		}
					...
					if (nextStep === "Check Passwords") {
						...
						try {
							...
							if (
								...
								JupyterSelfCertsError.isSelfCertsError(err)
							) {
	...
	private async addNewServer(server: {
		handle: string;
		uri: string;
		serverInfo: IJupyterServerUri;
	}) {
	...
	async getServerUri(id: string): Promise<IJupyterServerUri> {
		...
		try {
			...
			serverUriToReturn = Object.assign({}, serverInfo, {
				authorizationHeader:
					passwordResult.requestHeaders ||
					serverInfo.authorizationHeader,
			});
		...
		if (!initialValue && !isWeb) {
			try {
				...
				initialValue =
					text &&
					parsedUri &&
					parsedUri.scheme.toLowerCase().startsWith("http")
						? text
						: "";
			} catch {
...
function sendRemoteTelemetryForAdditionOfNewRemoteServer(
	...
	Promise.all([
		...
		getTelemetrySafeHashedString(id.toLowerCase()),
	])
...
export class SecureConnectionValidator {
	...
	public async promptToUseInsecureConnections(): Promise<boolean> {
		...
		try {
			...
			input.buttons = [QuickInputButtons.Back];
			input.items = [
				{ label: Common.bannerLabelYes },
				{ label: Common.bannerLabelNo },
			];
			input.show();
			disposables.push(
				input.onDidHide(() => deferred.reject(InputFlowAction.cancel))
...
export function parseUri(
	...
	try {
		...
		const baseUrl = `${url.protocol}//${url.host}${
			url.pathname === "/lab" || url.pathname === "/tree"
				? ""
				: url.pathname
		}`;
...
export class OldStorage {
	...
	public async getServers(): Promise<
		{ handle: string; uri: string; serverInfo: IJupyterServerUri }[]
	> {
		const serverList = this.globalMemento.get<
		...
		const encryptedList = cache.split(
			Settings.JupyterServerRemoteLaunchUriSeparator
		);
		if (
			encryptedList.length === 0 ||
			encryptedList.length !== serverList.length
		) {
	...
	public async clear() {
		await this.encryptedStorage
			...
			.catch(noop);
		await this.globalMemento
			.update(UserJupyterServerUriListMementoKey, [])
			.then(noop, noop);
	}
}
...
export class NewStorage {
	...
	public async migrateDisplayNames(
		...
		userServers.forEach((server) => {
			server.serverInfo.displayName =
				userServersFromUriStorage.get(server.handle) || server.uri;
		});
		await this.encryptedStorage.store(
			Settings.JupyterServerRemoteLaunchService,
			UserJupyterServerUriListKeyV2,
	...
	public async add(server: {
		...
		if (this.servers) {
			this.servers = this.servers
				.filter((s) => s.handle !== server.handle)
				.concat(server);
		}
