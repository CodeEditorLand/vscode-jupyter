...
suite("User Uri Provider", () => {
	...
	setup(() => {
		...
		sinon
			.stub(JupyterHubPasswordConnect.prototype, "isJupyterHub")
			.resolves(false);
		...
		const jupyterServerProviderRegistry =
			mock<IJupyterServerProviderRegistry>();
		const collection = mock<JupyterServerCollection>();
		when(collection.dispose()).thenReturn();
		when(
			jupyterServerProviderRegistry.createJupyterServerCollection(
				anything(),
				anything(),
				anything(),
				anything()
	...
	async function testMigration() {
		...
		when(globalMemento.get(Settings.JupyterServerUriList)).thenReturn(
			oldIndexes
		);
		...
		when(
			...
			oldUrls.join(Settings.JupyterServerRemoteLaunchUriSeparator)
		);
		...
		const serverUris = await Promise.all(
			servers.map((s) =>
				provider
					.resolveJupyterServer(s, token)
					.then((j) => j.connectionInformation)
			)
		);
		serverUris.sort((a, b) =>
			a.baseUrl.toString().localeCompare(b.baseUrl.toString())
		);
		assert.deepEqual(
	...
	test("Migrate display names from Uri Storage", async () => {
		...
		assert.deepEqual(servers.map((s) => s.label).sort(), [
			"Azure ML",
	...
	test("Prompt user for a Url and use what is in clipboard, then verify it is in the storage", async () => {
		...
		displayNameStub.resolves("Foo Bar");
		when(clipboard.readText()).thenResolve(
			"https://localhost:3333?token=ABCD"
		);
