...
suite("JupyterServer Password Connect", () => {
	...
	setup(() => {
		...
		when(appShell.showInputBox(anything())).thenReturn(
	...
	function createMockSetup(
		...
		const fetchMock: typemoq.IMock<typeof nodeFetch.default> =
			typemoq.Mock.ofInstance(nodeFetch.default);
		const rootUrl = secure
			? "https://testname:8888/"
			: "http://testname:8888/";
	...
	test("Empty Password and empty token", async () => {
		when(appShell.showInputBox(anything())).thenReject(
	...
	test("Bad password followed by good password.", async () => {
		...
		mockSessionResponseBad
			.setup((mr) => mr.headers)
			.returns(() => mockSessionHeadersBad.object);
