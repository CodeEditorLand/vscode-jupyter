	...
	implements IExtensionSyncActivationService, IDisposable
{
	...
	constructor(
		...
		this.nativeContext = new ContextKey(
			EditorContexts.IsNativeActive,
			this.commandManager
		);
		...
		this.isPythonNotebook = new ContextKey(
			EditorContexts.IsPythonNotebook,
			this.commandManager
		);
		...
		this.kernelSourceContext = new ContextKey(
			EditorContexts.KernelSource,
			this.commandManager
		);
	}
	public dispose() {
	...
	public activate() {
		this.docManager.onDidChangeActiveTextEditor(
		...
		if (
			this.docManager.activeTextEditor?.document.languageId ===
			PYTHON_LANGUAGE
		) {
	...
	private updateNativeNotebookCellContext() {
		...
		const hasNativeCells =
			(this.vscNotebook.activeNotebookEditor?.notebook.cellCount || 0) >
			0;
