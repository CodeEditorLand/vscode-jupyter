	...
	implements IExportedKernelServiceFactory
{
	...
	constructor(
		...
		@inject(IThirdPartyKernelProvider)
		private readonly thirdPartyKernelProvider: IThirdPartyKernelProvider,
...
class JupyterKernelService implements IExportedKernelService {
	...
	private static readonly wrappedKernelConnections = new WeakMap<
		IBaseKernel,
		IKernelConnectionInfo
	>();
	constructor(
	...
	private async startOrConnect(
		...
		let wrappedConnection =
			JupyterKernelService.wrappedKernelConnections.get(kernel);
	...
	private wrapKernelConnection(kernel: IBaseKernel): IKernelConnectionInfo {
		...
		const connection = new KernelConnectionWrapper(
			kernel,
			this.disposables
		);
		// eslint-disable-next-line @typescript-eslint/no-use-before-define
		const info = {
			connection,
			kernelSocket: new KernelSocketWrapper(kernel),
		};
...
class KernelSocketWrapper implements IKernelSocket {
	...
	private sendHooks = new Set<
		(data: unknown, cb?: (err?: Error) => void) => Promise<void>
	>();
	...
	private removeHooks() {
		...
		this.receiveHooks.forEach(
			(hook) => this.socket?.removeReceiveHook(hook)
		);
