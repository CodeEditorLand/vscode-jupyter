		...
		controllerRegistration.onDidChange((e) => {
			for (let controller of e.added) {
				if (
					...
					controller.connection.serverProviderHandle.id ===
						serverId.id &&
					controller.connection.serverProviderHandle.handle ===
						serverId.handle
				) {
					resolve();
				}
...
export function buildApi(
	...
	const api: IExtensionApi = {
		...
		async showDataViewer(
			dataProvider: IDataViewerDataProvider,
			title: string
		): Promise<void> {
			sendApiUsageTelemetry(extensions, "showDataViewer");
			const dataViewerProviderService =
				serviceContainer.get<IDataViewerFactory>(IDataViewerFactory);
		...
		registerRemoteServerProvider(
			...
			const container =
				serviceContainer.get<IJupyterUriProviderRegistration>(
					IJupyterUriProviderRegistration
				);
			let disposeHook = noop;
			const register = async () => {
				const extensions =
					serviceContainer.get<IExtensions>(IExtensions);
				const extensionId = provider.id.startsWith("_builtin")
					? JVSC_EXTENSION_ID
					: (await extensions.determineExtensionFromCallStack())
							.extensionId;
				const disposable = container.registerProvider(
					provider,
					extensionId
				);
		...
		getSuggestedController: async (
			...
			if (
				...
				window
					.showErrorMessage(
						"The Jupyter API getSuggestedController is being deprecated."
					)
					.then(noop, noop);
				return;
			}
		...
		addRemoteJupyterServer: async (providerId: string, handle: string) => {
			...
			await new Promise<void>(async (resolve) => {
				...
				await selector.addJupyterServer({
					id: providerId,
					handle,
					extensionId,
				});
		...
		openNotebook: async (uri: Uri, kernelId: string) => {
			...
			const id = controllers.all.find(
				(controller) => controller.id === kernelId
			)?.id;
			if (!id) {
		...
		createJupyterServerCollection: (id, label, serverProvider) => {
			...
			const collection: Omit<
				JupyterServerCollection,
				"onDidChangeProvider" | "serverProvider" | "extensionId"
			> = {
				dispose: () => {
					isDisposed = true;
					proxy?.dispose();
				...
				set commandProvider(
					value: JupyterServerCommandProvider | undefined
				) {
			...
			(async () => {
				...
				const registration =
					serviceContainer.get<IJupyterServerProviderRegistry>(
						IJupyterServerProviderRegistry
					);
