...
export class ExportCommands implements IDisposable {
	...
	private async nativeNotebookExport(
		...
		const document = notebookUri
			? this.notebooks.notebookDocuments.find((item) =>
	...
	private async export(
		...
		if (exportMethod) {
			await this.fileConverter.export(
				exportMethod,
				sourceDocument,
				defaultFileName,
				interpreter
	...
	private getExportQuickPickItems(
		...
		if (
			...
			(sourceDocument.metadata &&
				isPythonNotebook(getNotebookMetadata(sourceDocument)))
		) {
			items.push({
