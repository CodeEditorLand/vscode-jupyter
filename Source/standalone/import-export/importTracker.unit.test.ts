...
suite("Import Tracker", async () => {
		...
		public static async expectHashes(
			...
			if (hashes.length > 0) {
				...
				await waitForCondition(
					...
					1_000,
					() =>
						`Incorrect number of hashed package name events sent. Expected ${
							hashes.length
						}, got ${
							Reporter.properties.filter(
								(item) => Object.keys(item).length
	...
	setup(() => {
		...
		sinon
			.stub(reporter, "sendTelemetryEvent")
			.callsFake((eventName: string, properties?: {}, measures?: {}) => {
				Reporter.eventNames.push(eventName);
				Reporter.properties.push(properties!);
				Reporter.measures.push(measures!);
			});
		...
		when(vscNb.onDidOpenNotebookDocument).thenReturn(
			onDidOpenNbEvent.event
		);
		when(vscNb.onDidCloseNotebookDocument).thenReturn(
			onDidCloseNbEvent.event
		);
		when(vscNb.onDidSaveNotebookDocument).thenReturn(
			onDidSaveNbEvent.event
		);
		when(vscNb.onDidChangeNotebookCellExecutionState).thenReturn(
	...
	test("Track packages when a cell is executed", async () => {
		...
		onDidChangeNotebookCellExecutionState.fire({
			cell: nb.cellAt(0),
