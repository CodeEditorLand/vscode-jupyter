...
export type MultiStepInputQuickPicResponseType<T, P> =
	| T
...
export class MultiStepInput<S> implements IMultiStepInput<S> {
	...
	public async showQuickPick<
		T extends QuickPickItem,
		P extends IQuickPickParameters<T>,
	>(options: P): Promise<MultiStepInputQuickPicResponseType<T, P>> {
		return this.showLazyLoadQuickPick<T, P>(options).selection;
	}

	public showLazyLoadQuickPick<
		T extends QuickPickItem,
		P extends IQuickPickParameters<T>,
	>({
		title,
			...
			input.onDidChangeSelection(async (selectedItems) => {
				...
				if (itemLabel && validate && selectedItems.length) {
					...
					if (message) {
						...
						await this.shell.showErrorMessage(message, {
							modal: true,
						});
					}
					input.enabled = true;
				...
				input.onDidAccept(async () => {
					if (!input.busy) {
						...
						if (!validationMessage) {
							...
							input.busy = true;
							// No validation allowed on a quick pick. Have to put up a dialog instead
							await this.shell.showErrorMessage(
								validationMessage
							);
							input.enabled = true;
							input.busy = false;
