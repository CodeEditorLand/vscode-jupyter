...
export class JupyterSettings implements IWatchableJupyterSettings {
	...
	public static getInstance(
		...
		const workspaceFolderUri = JupyterSettings.getSettingsUriAndTarget(
			resource,
			workspace
		).uri;
	...
	public createSystemVariables(resource: Resource): ISystemVariables {
		return new this._systemVariablesCtor(
			resource,
			this._workspaceRoot,
			this._workspace
		);
	...
	private onWorkspaceFoldersChanged() {
		...
		const workspaceKeys = this._workspace.workspaceFolders!.map(
	...
	private initialize(): void {
		const onDidChange = () => {
			const currentConfig = this._workspace.getConfiguration(
				"jupyter",
				this._workspaceRoot
			);
			const pythonConfig = this._workspace.getConfiguration(
				"python",
				this._workspaceRoot
			);
		...
		this._disposables.push(
			this._workspace.onDidChangeWorkspaceFolders(
				this.onWorkspaceFoldersChanged,
				this
			)
		);
		this._disposables.push(
			this._workspace.onDidChangeConfiguration(
		...
		const initialConfig = this._workspace.getConfiguration(
			"jupyter",
			this._workspaceRoot
		);
		const pythonConfig = this._workspace.getConfiguration(
