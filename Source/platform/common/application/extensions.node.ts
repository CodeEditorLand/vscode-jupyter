...
export class Extensions implements IExtensions {
	...
	public async determineExtensionFromCallStack(): Promise<{
		...
		if (stack) {
			const jupyterExtRoot = path.join(
			...
			for (const frame of frames) {
				...
				while (dirName && dirName.length < last.length) {
					const possiblePackageJson = Uri.file(
						...
						try {
							...
							if (
								...
								if (
									...
									(frame.startsWith(
										matchingExt.extensionUri.toString()
									) ||
										frame.startsWith(
											matchingExt.extensionUri.fsPath.toString()
										))
								) {
									if (
										syncId.extensionId ===
									...
									return {
										extensionId: matchingExt.id,
										displayName:
											matchingExt.packageJSON.displayName,
									};
								}
							}
						} catch {
	...
	private determineExtensionFromCallStackSync(stack: string): {
		extensionId: string;
		displayName: string;
	} {
		try {
			if (stack) {
				const jupyterExtRoot = this.getExtension(JVSC_EXTENSION_ID)!
