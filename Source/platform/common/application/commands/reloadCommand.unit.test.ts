...
suite("Common Commands ReloadCommand", () => {
	...
	setup(async () => {
		...
		when(
			mockedVSCodeNamespaces.commands.executeCommand(anything()),
		).thenResolve();
		when(
			mockedVSCodeNamespaces.window.showInformationMessage(anything()),
		).thenResolve();
	...
	test("Display prompt to reload VS Code with message passed into command", async () => {
		...
		const commandHandler = capture(
			mockedVSCodeNamespaces.commands.registerCommand as any,
		).first()[1] as Function;
