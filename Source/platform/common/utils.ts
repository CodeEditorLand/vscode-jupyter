...
export function getResourceType(uri?: Uri): "notebook" | "interactive" {
	...
	return uriPath.extname(uri).toLowerCase().endsWith("ipynb")
		? "notebook"
		: "interactive";
}
...
export function isJupyterNotebook(option: NotebookDocument | string) {
	if (typeof option === "string") {
		return (
			option === JupyterNotebookView || option === InteractiveWindowView
		);
	} else {
		return (
			option.notebookType === JupyterNotebookView ||
			option.notebookType === InteractiveWindowView
		);
	}
}
...
export function getCellMetadata(cell: NotebookCell): JupyterCellMetadata {
	const metadata: JupyterCellMetadata = JSON.parse(
