...
export async function getExtensionTempDir(
	...
	if (isWeb()) {
		return Uri.joinPath(
			...
			getCurrentTempDirName(context),
		);
	} else {
		// Ensure we use the file scheme when dealing with the desktop version of VS Code.
		// eslint-disable-next-line local-rules/dont-use-fspath
		let dir = Uri.joinPath(
			Uri.file(context.globalStorageUri.fsPath),
			getCurrentTempDirName(context),
		);
	...
	try {
		...
		await Promise.all(
			dirs
				...
				.map(([dir]) =>
					workspace.fs.delete(
						Uri.joinPath(context.globalStorageUri, dir),
						{ recursive: true },
					),
				),
		).catch(noop);
