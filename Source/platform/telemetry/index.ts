...
import { IEventNamePropertyMapping, TelemetryEventInfo } from "../../telemetry";
import {
	AppinsightsKey,
	isTestExecution,
	isUnitTestExecution,
...
export function onDidChangeTelemetryEnablement(
...
export function setSharedProperty<
	P extends SharedPropertyMapping,
	E extends keyof P,
...
>(
	...
	if (
		!isPerfMeasurementOnCI(eventName.toString()) &&
		(isTestExecution() || !isTelemetrySupported())
	) {
		return;
	}
 ...
 */
export function capturePerfTelemetry<
	This,
	P extends IEventNamePropertyMapping,
	E extends keyof PickPropertiesOnly<P>,
>(
	eventName: E,
	properties?: P[E] extends TelemetryEventInfo<infer R>
		? ExcludeType<R, number> extends never | undefined
 ...
 * @param properties Properties to send with the event; must be valid for the event.
 */
export function captureUsageTelemetry<
	This,
	P extends IEventNamePropertyMapping,
	E extends keyof P,
>(
