...
import { Disposable, LogLevel, Uri, window, workspace } from "vscode";
...
export function initializeLoggers(options: {
	...
	if (options.addConsoleLogger) {
		...
		registerLogger(
			new ConsoleLogger(isCI ? undefined : "Jupyter Extension:"),
		);
	}
	...
	return args.map((arg) => {
		...
		if (
			...
			if (
				...
				info.push(
					`Interpreter Id = ${getDisplayPath(arg.kernelConnectionMetadata.interpreter.id)}`,
				);
			}
		}
		if (arg.stack) {
...
export function debugDecorator(
	message: string,
