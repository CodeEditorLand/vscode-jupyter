...
} from "./reservedNamedProvider.node";
...
suite("Reserved Names Provider", () => {
	...
	setup(() => {
		...
		when(workspace.getConfiguration("jupyter")).thenReturn(
	...
	test("Returns valid Uris of files and folders that can override builtins", async () => {
		...
		when(fs.searchLocal("*.py", cwd.fsPath, true)).thenResolve([
			"xml.py",
			"urllib.py",
			"random.py",
		]);
	...
	test("Test ignoring and changing settings (linux)", async () => {
		...
		settingsChanged.fire({
			affectsConfiguration: (section) =>
				section === `jupyter.${ignoreListSettingName}`,
		});
