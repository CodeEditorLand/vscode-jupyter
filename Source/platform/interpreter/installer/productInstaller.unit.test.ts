...
import { IOutputChannel, InterpreterUri } from "../../../platform/common/types";
...
class AlwaysInstalledDataScienceInstaller extends DataScienceInstaller {
	...
	public override async isInstalled(
		_product: Product,
		_resource?: InterpreterUri | Environment,
	): Promise<boolean> {
		return true;
	}
}
...
suite("DataScienceInstaller install", async () => {
	...
	test("Will ignore with no installer modules", async () => {
		...
		expect(result).to.equal(
			...
			"Should be InstallerResponse.Ignore",
		);
	});
	...
	test("Will cancel when signaled", async () => {
		...
		testInstaller
			.setup((c) => c.type)
			.returns(() => ModuleInstallerType.Conda);
		testInstaller
			.setup((c) =>
				c.installModule(
		...
		const resultPromise = dataScienceInstaller.install(
			Product.ipykernel,
			testEnvironment,
			tokenSource,
		);
	...
	test("Will invoke poetry", async () => {
		...
		testInstaller
			.setup((c) => c.type)
			.returns(() => ModuleInstallerType.Poetry);
		testInstaller
			.setup((c) =>
				c.installModule(
					TypeMoq.It.isValue(Product.ipykernel),
	...
	test("Will invoke pipenv", async () => {
		...
		testInstaller
			.setup((c) => c.type)
			.returns(() => ModuleInstallerType.Pipenv);
		testInstaller
			.setup((c) =>
				c.installModule(
					TypeMoq.It.isValue(Product.ipykernel),
