...
export function getPythonEnvDisplayName(
	...
	if ("executable" in interpreter) {
		...
		const envName = interpreter.environment
			? basename(interpreter.environment?.folderUri)
			: "";
	...
	const pythonVersion = (
		getTelemetrySafeVersion(interpreter.version?.raw || "") || ""
	).trim();
	// If this is a conda environment without Python, then don't display `Python` in it.
	const isCondaEnvWithoutPython =
		interpreter.envType === EnvironmentType.Conda &&
		interpreter.isCondaEnvWithoutPython === true;
	const nameWithVersion = pythonVersion.trim()
...
export function getPythonEnvironmentName(pythonEnv: PythonEnvironment) {
	...
	if (
		pythonEnv.envPath &&
		pythonEnv.envType === EnvironmentType.Conda &&
		!pythonEnv.envName
	) {
...
export function isCondaEnvironmentWithoutPython(env: Environment) {
	return (
		getEnvironmentType(env) === EnvironmentType.Conda && !env.executable.uri
	);
}
