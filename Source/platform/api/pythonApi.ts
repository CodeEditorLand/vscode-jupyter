...
import { PythonExtension, Telemetry, isCI } from "../common/constants";
...
export function deserializePythonEnvironment(
	...
	if (pythonVersion) {
		const result = {
			...
			envPath: pythonVersion.envPath
				? Uri.file(pythonVersion.envPath)
				: undefined,
			displayPath:
				"displayPath" in pythonVersion &&
				typeof pythonVersion.displayPath === "string"
...
export function resolvedPythonEnvToJupyterEnv(
	...
	let envType = envTools.length
		? (envTools[0] as EnvironmentType)
	...
	if (env.executable.uri) {
	...
	} else if (envType === EnvironmentType.Conda && supportsEmptyCondaEnv) {
		...
		traceWarning(
			`Python environment ${getDisplayPath(
				env.id,
			)} excluded as Uri is undefined`,
		);
		return;
	}
...
export function serializePythonEnvironment(
	...
	if (jupyterVersion) {
		const result = Object.assign({}, jupyterVersion, {
			...
			envPath: jupyterVersion.envPath
				? getFilePath(jupyterVersion.envPath)
				: undefined,
			displayPath: jupyterVersion.displayPath
				? getFilePath(jupyterVersion.displayPath)
				: undefined,
		});
		// Cleanup stuff that shouldn't be there.
...
export class InterpreterService implements IInterpreterService {
	...
	private interpreterListCachePromise:
	...
	private readonly _interpreters = new Map<
		string,
		{ resolved: PythonEnvironment }
	>();
	public get resolvedEnvironments(): PythonEnvironment[] {
		this.hookupOnDidChangeInterpreterEvent();
		return Array.from(this._interpreters.values()).map(
			(item) => item.resolved,
		);
	}
	...
	private getInterpreters(): Promise<PythonEnvironment[]> {
		...
		if (!this.interpreterListCachePromise) {
			...
			const cancellation = (this.getInterpretersCancellation =
	...
	public async refreshInterpreters(forceRefresh = false) {
		const promise = (async () => {
		...
		if (
			...
			promise
				.then((item) => {
					...
					const key = `${getDisplayPath(resource)}'-${getDisplayPath(
						item?.id,
					)}`;
					...
					traceInfo(
						`Active Interpreter ${
							resource ? `for '${getDisplayPath(resource)}' ` : ""
						}is ${getDisplayPath(item?.id)} (${item?.envType}, '${
							item?.envName
	...
	private trackResolvedEnvironment(env: ResolvedEnvironment | undefined) {
		if (env) {
			const displayEmptyCondaEnv =
				...
				this.apiProvider.pythonExtensionVersion.compare(
					"2023.3.10341119",
				) >= 0;
			const resolved = resolvedPythonEnvToJupyterEnv(
				env,
				displayEmptyCondaEnv ? true : false,
			);
			if (!resolved) {
				return;
			}
	...
	private async getApi(): Promise<PythonExtensionApi | undefined> {
		...
		if (!this.apiPromise) {
			...
			this.apiPromise
				.then((api) => (api ? (this.api = api) : undefined))
				.catch(noop);
		}
	...
	private async getInterpretersImpl(
		...
		await this.getApi().then(async (api) => {
			...
			const previousListOfInterpreters = api.environments.known.length;
			try {
				await Promise.all(
					api.environments.known.map(async (item) => {
						try {
							const env =
								await api.environments.resolveEnvironment(item);
							const resolved = this.trackResolvedEnvironment(env);
							if (resolved) {
								allInterpreters.push(resolved);
							} else if (
								item.executable.uri &&
								item.environment?.type !== EnvironmentType.Conda
							) {
		...
		traceVerbose(
			`Full interpreter list is length: ${
					...
					api.environments.onDidChangeEnvironments(
						async (e) => {
							...
							const pythonInstalledIntoConda =
								e.type === "update" &&
								this._interpreters.get(e.env.id)?.resolved
									.isCondaEnvWithoutPython &&
							...
							this.populateCachedListOfInterpreters(true)
								.finally(() => {
									const info = this._interpreters.get(
										e.env.id,
									);
