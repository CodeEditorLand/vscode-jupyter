...
export class OldPythonApiProvider implements IPythonApiProvider {
	...
	public async getNewApi(): Promise<PythonExtensionApi | undefined> {
		await this.init();
		const extension =
			this.extensions.getExtension<PythonExtensionApi>(PythonExtension);
	...
	private async init() {
		...
		const pythonExtension = this.extensions.getExtension<{
...
export class InterpreterService implements IInterpreterService {
	...
	public async getActiveInterpreter(
		...
		if (this.workspace.workspaceFolders?.length) {
			resource =
				this.workspace.getWorkspaceFolder(resource)?.uri ||
				...
				.then((item) => {
					...
					traceInfo(
						`Active Interpreter ${
							...
							item?.id
						)} (${item?.envType}, '${item?.envName}', ${item
							?.version?.major}.${item?.version?.minor}.${item
							?.version?.patch})`
					);
				})
				.catch(noop);
