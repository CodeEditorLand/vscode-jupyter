 ...
 */
export function getErrorMessageFromPythonTraceback(traceback = "") {
...
export function getLastTwoLinesFromPythonTracebackWithErrorMessage(
	...
	if (reversedLines[0].includes("Error")) {
		return [
			reversedLines.length > 1 ? reversedLines[1] : "",
			reversedLines[0],
		];
	} else {
		return;
...
export function analyzeKernelErrors(
	...
	if (stdErr.includes(noModule) && !isNotAPackage) {
		...
		return {
			...
			message:
				DataScience.fileSeemsToBeInterferingWithKernelStartup(fileName),
			moreInfoLink:
				"https://aka.ms/kernelFailuresOverridingBuiltInModules",
			telemetrySafeTags: ["import.error", "override.modules"],
		};
...
export function createOutputWithErrorMessageForDisplay(errorMessage: string) {
	...
	const stack = splitLines(errorMessage, {
