...
suite("Progress Reporter", () => {
	...
	let vscodeProgressReporter: VSCProgress<{
		message?: string | undefined;
		increment?: number | undefined;
	}>;
	class VSCodeReporter {
		public report(_value: {
			message?: string | undefined;
			increment?: number | undefined;
		}) {
			noop();
	...
	test("Report progress until disposed", async () => {
		...
		reporter.report({
			action: ReportableAction.KernelsGetKernelSpecs,
			phase: "started",
		});
		expectedProgressMessages.push(
		...
		reporter.report({
			action: ReportableAction.KernelsRegisterKernel,
			phase: "started",
		});
		expectedProgressMessages.push(
