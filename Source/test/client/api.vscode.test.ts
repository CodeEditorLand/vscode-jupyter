...
} from "../datascience/notebook/executionHelper";
...
suite("3rd Party Kernel Service API @kernelCore", function () {
	...
	setup(async function () {
		...
		const kernelFiner =
			api.serviceContainer.get<IKernelFinder>(IKernelFinder);
	...
	test("Start Kernel", async function () {
		const kernelService = await api.getKernelService();
		const interpreterService =
			await api.serviceContainer.get<IInterpreterService>(
				IInterpreterService
			);
		...
		const pythonKernel = await waitForCondition(
				...
				return IS_REMOTE_NATIVE_TEST()
					? kernelSpecs.find(
							(item) =>
								...
								item.kernelSpec.language === "python"
					  )
					: kernelSpecs.find(
							(item) =>
								item.kind === "startUsingPythonInterpreter" &&
			...
			() =>
				`Python Kernel not found, active interpreter is ${activeInterpreter.uri.toString()}, found kernel specs ${
					...
					.map(
						(i) => `${i.id}, ${i.kind}, ${i.interpreter?.uri?.path}`
					)
					.join("\n")}, \n ${JSON.stringify(
					kernelSpecs,
					undefined,
		...
		const kernelInfo = await kernelService?.startKernel(
			pythonKernel!,
			notebook.uri!
		);
		...
		assert.strictEqual(
			kernels![0].metadata.id,
			kernel!.metadata.id,
			"Kernel Connection not same for the document"
		);
		...
		assert.strictEqual(
			kernelInfo!.connection.connectionStatus,
			"disconnected"
		);
