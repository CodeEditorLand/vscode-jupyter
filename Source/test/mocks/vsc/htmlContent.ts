...
export namespace vscMockHtmlContent {
			...
			return (
				...
				(typeof (<IMarkdownString>thing).isTrusted === "boolean" ||
					...
					(typeof (<IMarkdownString>thing).isTrusted === "object" &&
						isMarkdownTrustOptions(
							(<IMarkdownString>thing).isTrusted
						)))
			);
		}
		return false;
	...
	export function isMarkdownTrustOptions(
		thing: any
	): thing is ITrustedMarkdownOptions {
		if (thing && typeof thing === "object") {
			if (
				Array.isArray((<ITrustedMarkdownOptions>thing).enabledCommands)
			) {
				return true;
			}
		}
