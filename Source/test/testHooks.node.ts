...
export const rootHooks: Mocha.RootHookObject = {
	beforeAll() {
		traceInfoIfCI(
			`Environment Variable dump: ${JSON.stringify(process.env)}`
		);
		if (!IS_CI_SERVER) {
			return;
		}
	...
	afterEach(this: Context) {
		...
		if (
			...
			(process.env.VSC_JUPYTER_WARMUP &&
				process.env.VSC_JUPYTER_WARMUP == "true")
		) {
			return;
		}

		let result = this.currentTest?.isFailed()
			? "failed"
			: this.currentTest?.isPassed()
			? "passed"
			: "skipped";

		const measures = this.currentTest?.duration
		...
		traceInfoIfCI(
			`Sending telemetry event ${
				Telemetry.RunTest
			} with dimensions ${JSON.stringify(dimensions)}`
		);
