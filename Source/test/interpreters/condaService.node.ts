...
async function getCondaFileFromKnownLocations(): Promise<string> {
	const globPattern =
		getOSType() === OSType.Windows
			...
			: CondaLocationsGlob;
	const condaFiles = await promisify(glob)(globPattern).catch<string[]>(
		(failReason) => {
			traceWarning(
...
async function getCondaEnvironments(): Promise<
	CondaEnvironmentInfo[] | undefined
> {
	...
	try {
		...
		let envInfo = await processService
		...
		if (!envInfo) {
			...
			traceVerbose(
				`Attempting new path for conda env list: ${newEnv.PATH}`
			);
...
export async function getCondaEnvironment(
	...
	const matchingEnvs = Array.isArray(environments)
		? environments.filter((item) =>
				arePathsSame(item.path, interpreterPathToMatch)
		  )
		: [];
	...
	if (matchingEnvs.length > 0) {
		return {
			...
			path: Uri.file(interpreterPathToMatch),
		};
	}
