...
const interpreterInfoCache = new Map<
	string,
		...
		try {
			...
			const argv = [
				...
				fileToCommandArgument(
					path.join(SCRIPTS_DIR, "interpreterInfo.py")
				),
			];
			const cmd = argv.reduce(
				(p, c) => (p ? `${p} "${c}"` : `"${c.replace("\\", "/")}"`),
				""
			); // CodeQL [SM02383] Replace just the first occurrence of \\ as this could be a UNC path.
			...
			const rawVersion = `${json.versionInfo.slice(0, 3).join(".")}-${
				json.versionInfo[3]
			}`;
			return {
				id: json.exe,
...
export async function getActivatedEnvVariables(
	...
	const promise = (async () => {
		...
		if (result.stderr && result.stderr.length) {
			traceError(
				`Failed to get env vars for shell ${defaultShell} with ${argv} stderr: ${result.stderr}`
			);
			return;
		}
		try {
			...
			return JSON.parse(
				output
					.substring(output.indexOf(separator) + separator.length)
					.trim()
			);
		} catch (ex) {
			traceError(
				`Failed to get env vars for shell ${defaultShell} with ${argv}`,
				ex
			);
		}
	})();
