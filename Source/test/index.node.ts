...
process.on("unhandledRejection", (ex: Error, _a) => {
	...
	if (
		(msg.includes("Canceled future for") &&
			msg.includes("message before replies were done")) ||
	...
	if (isCI) {
		fs.appendFileSync(
			path.join(EXTENSION_ROOT_DIR_FOR_TESTS, "unhandledErrors.txt"),
			`${msg}\n`
		);
	}
});
...
function configure(): SetupOptions {
	...
	const invert =
		(process.env.VSC_JUPYTER_CI_TEST_INVERT_GREP || "").length > 0;
...
export async function run(): Promise<void> {
	...
	if (!IS_SMOKE_TEST()) {
		const reactHelpers =
			require("./datascience/reactHelpers") as typeof import("./datascience/reactHelpers");
	...
	console.log(
		`Running tests with options ${JSON.stringify(options, undefined, 4)}`
	);
