...
const extensionDevelopmentPath = process.env.CODE_EXTENSIONS_PATH
	...
	: EXTENSION_ROOT_DIR_FOR_TESTS;
const isRunningVSCodeTests =
	process.env.TEST_FILES_SUFFIX?.includes("vscode.test");
 ...
 */
async function installPythonExtension(
...
async function createSettings(): Promise<string> {
	...
	const userDataDirectory =
		process.env.VSC_JUPYTER_USER_DATA_DIR || (await createTempDir());
...
async function start() {
	...
	const baseLaunchArgs = requiresPythonExtensionToBeInstalled()
		? []
		: ["--disable-extensions"];
		...
		launchArgs: baseLaunchArgs
			...
			.concat(
				IS_SMOKE_TEST()
					...
					: []
			) // For some reason pylance crashes and takes down the entire test run. See https://github.com/microsoft/vscode-jupyter/issues/13200
