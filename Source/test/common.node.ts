...
export async function getExtensionSettings(
	...
	const systemVariables = await import(
		"../platform/common/variables/systemVariables.node"
	);
...
async function setPythonPathInWorkspace(
	...
	if (!value || value[prop] !== pythonPath) {
		...
		await settings
			.update("pythonPath", pythonPath, config)
			.then(noop, noop);
		await settings
			.update("defaultInterpreterPath", pythonPath, config)
			.then(noop, noop);
		...
		async startJupyterServer(
			...
			if (IS_REMOTE_NATIVE_TEST()) {
				...
				try {
					...
					console.error(
						"Failed to select jupyter server, retry in 1s",
						ex
					);
				}
