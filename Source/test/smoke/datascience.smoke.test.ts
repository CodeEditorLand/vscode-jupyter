...
suite("Smoke Tests", function () {
	...
	test("Run Cell in Notebook", async function () {
		...
		if (os.platform() === "darwin" || os.platform() === "linux") {
			...
			pythonPath = `${PYTHON_PATH.substring(
				0,
				1
			).toLowerCase()}${PYTHON_PATH.substring(1)}`;
		}
	...
	test("Interactive window should always pick up current active interpreter", async function () {
		...
		assert.ok(
			vscode.workspace.notebookDocuments.length === 1,
			"Unexpected number of notebook documents created"
		);
		...
		const interpreterService =
			api.serviceManager.get<IInterpreterService>(IInterpreterService);
		await waitForCondition(
	...
	async function computeHash(
		...
		const hashBuffer = await require("node:crypto").webcrypto.subtle.digest(
			{ name: algorithm },
			inputBuffer
		);
	 ...
	 */
	function getNormalizedInterpreterPath(
		path: vscode.Uri,
		...
		if (!isWindows) {
			...
			return fsPath.endsWith("/bin/python") &&
				fsPath.split("/").length > 4
