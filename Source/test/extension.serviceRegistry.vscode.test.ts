...
async function getInjectableClasses(
	fileNames: string[],
	...
	function visit(sourceFile: ts.SourceFile, node: ts.Node) {
		...
		if (ts.isClassDeclaration(node) && node.modifiers) {
			...
			if (
				node.modifiers.find((d) =>
					d.getText(sourceFile).includes("injectable")
				)
			) {
...
suite("Verify serviceRegistry is correct", function () {
	...
	testMandatory(
		"Verify all classes with inject on them are in the container",
		async () => {
			assert.ok(
			...
			const map = (api.serviceManager.getContainer() as any)
				._bindingDictionary._map as Map<
				number,
			...
			assert.equal(
				notFound.size,
				0,
				`List of classes not in registry that are marked as injectable: ${[
					...notFound,
				].join("\n")}`
			);
		}
