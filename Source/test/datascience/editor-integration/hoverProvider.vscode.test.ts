...
suite("Hover provider @lsp", async () => {
	...
	suiteSetup(async function () {
		...
		const configService = api.serviceManager.get<IConfigurationService>(
			IConfigurationService
		);
		...
		const experimentService =
			api.serviceManager.get<IExperimentService>(IExperimentService);
	...
	test("Tensor tooltips", async () => {
		...
		const hoverProvider = new HoverProvider(
			api.serviceContainer.get<IJupyterVariables>(
				IJupyterVariables,
				Identifiers.KERNEL_VARIABLES
			),
			api.serviceContainer.get<IInteractiveWindowProvider>(
		...
		assert.ok(
			...
			"No results returned from hover provider request"
		);
