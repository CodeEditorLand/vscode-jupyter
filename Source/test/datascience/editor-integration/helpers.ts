			...
			.returns((r: Range) => {
				...
				if (r) {
					for (
						...
						line <= r.end.line && line < inputLines.length;
						line += 1
					) {
						const startIndex =
...
export function createMockedDocument(
	...
	when(document.lineAt(anything())).thenCall((index: number) =>
		instance(textLines[index])
	);
...
export function createMockedNotebookDocument(
	...
	when(notebook.getCells(anything())).thenCall((range: NotebookRange) =>
		nbCells.slice(range.start, range.end)
	);
