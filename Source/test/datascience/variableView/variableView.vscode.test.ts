...
suite("VariableView @variableViewer", function () {
	...
	test("VariableView document switching (webview-test)", async function () {
		...
		await insertCodeCell('test = "MYTESTVALUE"', { index: 0 });
		const cell =
			vscodeNotebook.activeNotebookEditor?.notebook.getCells()![0]!;
		await Promise.all([
			runCell(cell),
	...
	test("VariableView basic types A (webview-test)", async function () {
		if (
			activeInterpreter.version?.major === 3 &&
			activeInterpreter.version.minor >= 10
		) {
...
`;
	...
	test("Open dataviewer", async function () {
		...
		assert.equal(
			dataViewer!.title,
			"Data Viewer - test",
			"Title for data viewer is wrong"
		);
		...
		assert.equal(
			dataViewer!.title,
			"Data Viewer - test2",
			"Title for data viewer2 is wrong"
		);
	});
});
