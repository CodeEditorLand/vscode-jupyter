...
export async function waitForVariablesToMatch(
	expected: IVariableInfo[],
	variableView: ITestWebviewHost
) {
	let htmlVariables: IVariableInfo[] | undefined;
	return waitForCondition(
		async () => {
			const htmlResult = await variableView?.getHTMLById(
		...
		() =>
			`Variables do not match. Expected ${JSON.stringify(
				expected
			)} but got ${JSON.stringify(htmlVariables)}`
	);
}
