...
export class TestPersistentStateFactory implements IPersistentStateFactory {
	...
	constructor(globalState: Memento, localState: Memento) {
		this.realStateFactory = new PersistentStateFactory(
			globalState,
			localState
		);
	}
	...
	public createGlobalPersistentState<T>(
		...
		return this.realStateFactory.createGlobalPersistentState(
			key,
			defaultValue,
			expiryDurationMs
		);
	...
	public createWorkspacePersistentState<T>(
		...
		return this.realStateFactory.createWorkspacePersistentState(
			key,
			defaultValue,
			expiryDurationMs
		);
