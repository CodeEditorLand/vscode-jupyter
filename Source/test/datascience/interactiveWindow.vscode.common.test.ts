...
suite(`Interactive window execution @iw`, async function () {
	...
	test.skip("__file__ exists even after restarting a kernel", async function () {
		...
		const controllerRegistration =
			api.serviceManager.get<IControllerRegistration>(
				IControllerRegistration
			);
		// Ensure we picked up the active interpreter for use as the kernel
		const interpreterService =
			await api.serviceManager.get<IInterpreterService>(
				IInterpreterService
			);
		...
		const controller = notebookDocument
			? controllerRegistration.getSelected(notebookDocument)
			: undefined;
		if (!IS_REMOTE_NATIVE_TEST()) {
		...
		async function verifyCells() {
			...
			assert.ok(
				isSysInfoCell(firstCell),
				"First cell should be sys info cell"
			);
			assert.equal(
				firstCell?.kind,
				vscode.NotebookCellKind.Markup,
				"First cell should be markdown cell"
			);
			...
			assert.equal(
				actualSource,
				source,
				`Executed cell has unexpected source code`
			);
		...
		await activeInteractiveWindow.addCode(
			source,
	...
	test("Execute cell from input box", async () => {
		...
		const notebook = await waitForInteractiveWindow(
			activeInteractiveWindow
		);

		await runInteractiveWindowInput(
			'print("foo")',
			activeInteractiveWindow,
			1
		);
	...
	test("Clear input box", async () => {
		...
		let interactiveWindow = await createStandaloneInteractiveWindow(
			interactiveWindowProvider
		);
		...
		assert.ok(
			...
			"Text not cleared from input editor"
		);
		// Undo
	...
	test("Cells with errors cancel execution for others", async () => {
		...
		const { activeInteractiveWindow } =
			await submitFromPythonFileUsingCodeWatcher(source, disposables);
		const notebookDocument = vscode.workspace.notebookDocuments.find(
			(doc) =>
				doc.uri.toString() ===
				activeInteractiveWindow?.notebookUri?.toString()
		...
		const [, , secondCell, thirdCell, fourthCell] =
	...
	test("Dispose test", async () => {
		const interactiveWindow =
			await interactiveWindowProvider.getOrCreate(undefined);
		await interactiveWindow.dispose();
		const interactiveWindow2 =
			await interactiveWindowProvider.getOrCreate(undefined);
		assert.ok(
			interactiveWindow.notebookUri?.toString() !==
				interactiveWindow2.notebookUri?.toString(),
...
`;
		...
		const { activeInteractiveWindow: interactiveWindow } =
			await submitFromPythonFile(
				interactiveWindowProvider,
				codeWithWhitespace,
				disposables
	...
	test("Run current file in interactive window (with cells)", async () => {
		...
		assert.equal(
			...
			3,
			`Running a whole file did not split cells`
		);
	...
	test("Run current file in interactive window (without cells)", async () => {
		...
		if (!IS_REMOTE_NATIVE_TEST()) {
			const interpreterService =
				api.serviceManager.get<IInterpreterService>(
					IInterpreterService
				);
			...
			const controller = notebookDocument
				? controllerSelection.getSelected(notebookDocument)
				: undefined;
			const activeInterpreter =
				await interpreterService.getActiveInterpreter();
			assert.ok(
	...
	test("Error stack traces have correct line hrefs with mix of cell sources", async function () {
		...
		const interactiveWindow = await createStandaloneInteractiveWindow(
			interactiveWindowProvider
		);
		...
		const lastCell = await waitForLastCellToComplete(
			interactiveWindow,
			2,
			true
		);
		...
		await waitForCondition(
			async () =>
				...
				lastCell.outputs[0].items.length > 0,
			defaultNotebookTestTimeout,
	...
	test("Raising an exception from within a function has a stack trace", async function () {
		...
		assert.equal(hrefs.length, 4, "4 hrefs not found in traceback");
		assert.ok(
			hrefs[0].endsWith("line=3'"),
			`Wrong first ref line : ${hrefs[0]}`
		);
		assert.ok(
			hrefs[1].endsWith("line=4'"),
			`Wrong second ref line : ${hrefs[1]}`
		);
		assert.ok(
			hrefs[2].endsWith("line=1'"),
			`Wrong last ref line : ${hrefs[2]}`
		);
		assert.ok(
			hrefs[3].endsWith("line=2'"),
			`Wrong last ref line : ${hrefs[2]}`
		);
	});
	...
	test("Raising an exception from system code has a stack trace", async function () {
		...
		const ipythonVersionCell =
			activeInteractiveWindow.notebookDocument?.cellAt(
				lastCell.index - 1
			);
		...
		if (ipythonVersion >= 8) {
			assert.isAtLeast(
				hrefs.length,
				4,
				"Wrong number of hrefs found in traceback for IPython 8"
			);
		} else {
			assert.isAtLeast(
				hrefs.length,
				1,
				"Wrong number of hrefs found in traceback for IPython 7 or earlier"
			);
		}
	});
	...
	test("Export Interactive window to Python file", async () => {
		...
		await vscode.commands.executeCommand(
			...
			activeInteractiveWindow.notebookDocument
		);

		await waitForCondition(
			() => {
				// open document is python file with 3 "cells"
				let exportedDocument = vscode.window.visibleTextEditors.find(
					(editor) => {
	...
	test("Cells from python files and the input box are executed in correct order", async () => {
		...
		let runFilePromise = vscode.commands.executeCommand(
			Commands.RunFileInInteractiveWindows
		);
		...
		)) as InteractiveWindowMode;
		const interactiveWindow = interactiveWindowProvider.getExisting(
			tempFile.file,
			mode
		...
		assert.equal(
			output.trim(),
			"1",
			"original value should have been printed"
		);
	});
	...
	test("Get the notebook resource for the IW input box", async () => {
		...
		const notebookUri = getNotebookUriFromInputBoxUri(
			activeInteractiveWindow.inputUri
		);
