	...
	require.extensions[".js"] = function (mod: any, filename) {
		if (configurationRegex.test(filename)) {
			...
			try {
				...
				if (
					e.stack.includes("protocolDiagnostic.js") ||
					e.stack.includes("protocolCallHierarchyItem.js")
				) {
					console.error(
						`Failed to load module for tests ${mod} in ${filename}`,
						e
					);
				} else {
					throw e;
