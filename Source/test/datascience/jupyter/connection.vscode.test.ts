...
suite("Connect to Remote Jupyter Servers @mandatory", function () {
	...
	setup(async function () {
		...
		when(commands.registerCommand(anything(), anything())).thenReturn(
			new Disposable(noop)
		);
		...
		const onDidRemoveUriStorage = new EventEmitter<
			JupyterServerProviderHandle[]
		>();
		disposables.push(onDidRemoveUriStorage);
		when(serverUriStorage.onDidRemove).thenReturn(
			onDidRemoveUriStorage.event
		);
		requestCreator = api.serviceContainer.get<IJupyterRequestCreator>(
			IJupyterRequestCreator
		);
		...
		userUriProvider = new UserJupyterServerUrlProvider(
			...
			instance(memento),
			disposables,
			api.serviceContainer.get<IMultiStepInputFactory>(
				IMultiStepInputFactory
			),
			api.serviceContainer.get<IAsyncDisposableRegistry>(
				IAsyncDisposableRegistry
			),
			instance(commands),
			api.serviceContainer.get<IJupyterRequestAgentCreator>(
				IJupyterRequestAgentCreator
			),
			api.serviceContainer.get<IJupyterRequestCreator>(
				IJupyterRequestCreator
			),
			api.serviceContainer.get<IExtensionContext>(IExtensionContext),
			api.serviceContainer.get<IFileSystem>(IFileSystem),
			api.serviceContainer.get<IJupyterServerProviderRegistry>(
				IJupyterServerProviderRegistry
			)
		);
	...
	async function testConnectionAndVerifyBaseUrl({
		...
		const prompt = await hijackPrompt(
			...
			{
				result: DataScience.jupyterSelfCertEnable,
		...
		sinon
			.stub(UserJupyterServerUriInput.prototype, "getUrlFromUser")
			.resolves({
				url: userUri,
				jupyterServerUri: parseUri(userUri, "")!,
			});
		const baseUrl = `${new URL(userUri).protocol}//localhost:${
			new URL(userUri).port
		}/`;
		const computedBaseUrl = await getBaseJupyterUrl(
			userUri,
			requestCreator
		);
		assert.strictEqual(
			computedBaseUrl?.endsWith("/")
				? computedBaseUrl
				: `${computedBaseUrl}/`,
			baseUrl
		);
		sinon
			...
			.resolves(true);
		sinon
			.stub(UserJupyterServerDisplayName.prototype, "getDisplayName")
			.resolves(displayName);
		const errorMessageDisplayed = createDeferred<string>();
		inputBox.value = password || "";
		sinon
			.stub(inputBox, "validationMessage")
			.set((msg) =>
				msg ? errorMessageDisplayed.resolve(msg) : undefined
			);
		const [cmd] = await userUriProvider.provideCommands(
			userUri,
			token.token
		);
		const handlePromise = createDeferredFromPromise(
			userUriProvider.handleCommand(cmd, token.token)
		);
		await Promise.race([
			handlePromise.promise,
			errorMessageDisplayed.promise,
		]);

		if (failWithInvalidPassword) {
			assert.strictEqual(
				errorMessageDisplayed.value,
			...
			if (new URL(userUri).protocol.includes("https")) {
				assert.ok(
					...
					"Prompt for trusting certs not displayed"
				);
			}
			assert.equal(
				errorMessageDisplayed.value || "",
				"",
				`Password should be valid, ${errorMessageDisplayed}`
			);
	...
	test("Connect to server with auto generated Token in URL and custom path", async () => {
		const token = new URL(
			jupyterLabWithAutoGeneratedToken.url
		).searchParams.get("token")!;
	...
	test("Connect to server with Token in URL", () =>
		testConnectionAndVerifyBaseUrl({
			userUri: jupyterNotebookWithHelloToken.url,
			password: undefined,
		}));
	test("Connect to server with Password and Token in URL", () =>
		testConnectionAndVerifyBaseUrl({
			userUri: jupyterNotebookWithHelloPassword.url,
			password: "Hello",
		}));
	test("Connect to Notebook server with Password and no Token in URL", () =>
		testConnectionAndVerifyBaseUrl({
			userUri: `http://localhost:${
				new URL(jupyterNotebookWithHelloPassword.url).port
			}/`,
			password: "Hello",
		}));
	test("Connect to Lab server with Password and no Token in URL", () =>
		testConnectionAndVerifyBaseUrl({
			userUri: `http://localhost:${
				new URL(jupyterLabWithHelloPasswordAndWorldToken.url).port
			}/`,
			password: "Hello",
		}));
	...
	test("Connect to Lab server with Password & Token in URL", async () =>
		testConnectionAndVerifyBaseUrl({
			userUri: jupyterLabWithHelloPasswordAndWorldToken.url,
			password: "Hello",
		}));
	test("Connect to server with empty Password & empty Token in URL", () =>
		testConnectionAndVerifyBaseUrl({
			userUri: jupyterNotebookWithEmptyPasswordToken.url,
			password: "",
		}));
	test("Connect to server with empty Password & empty Token (nothing in URL)", () =>
		testConnectionAndVerifyBaseUrl({
			userUri: `http://localhost:${
				new URL(jupyterNotebookWithEmptyPasswordToken.url).port
			}/`,
			password: "",
		}));
	test("Connect to Lab server with Hello Password & empty Token (not even in URL)", () =>
		testConnectionAndVerifyBaseUrl({
			userUri: `http://localhost:${
				new URL(jupyterLabWithHelloPasswordAndEmptyToken.url).port
			}/`,
			password: "Hello",
		}));
