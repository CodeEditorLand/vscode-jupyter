...
suite("Install IPyKernel (install) @kernelInstall", function () {
	...
	const executable =
		getOSType() === OSType.Windows ? "Scripts/python.exe" : "bin/python"; // If running locally on Windows box.
	...
	let venvNoRegPath = Uri.file(
		path.join(
			EXTENSION_ROOT_DIR_FOR_TESTS,
			"src/test/datascience/.venvnoreg",
			executable
		)
	);
	let venvKernelPath = Uri.file(
		path.join(
			EXTENSION_ROOT_DIR_FOR_TESTS,
			"src/test/datascience/.venvkernel",
			executable
		)
	);
	const expectedPromptMessageSuffix = `requires the ${ProductNames.get(
		Product.ipykernel
	)!} package`;
    ...
    */
	suiteSetup(async function () {
		...
		if (
			!fs.pathExistsSync(venvNoKernelPath.fsPath) ||
		...
		interactiveWindowProvider = api.serviceManager.get(
			IInteractiveWindowProvider
		);
		commandManager =
			api.serviceContainer.get<ICommandManager>(ICommandManager);
		...
		previousDisableJupyterAutoStartValue =
			configSettings.disableJupyterAutoStart;
		configSettings.disableJupyterAutoStart = true;
		const pythonApi = await api.serviceManager
			.get<IPythonApiProvider>(IPythonApiProvider)
			.getNewApi();
		...
		const [interpreter1, interpreter2, interpreter3] =
	...
	setup(async function () {
		...
		nbFile = await createTemporaryNotebookFromFile(
			templateIPynbFile,
			disposables
		);
		...
		fs.writeFileSync(
			nbFile.fsPath,
			fs.readFileSync(nbFile.fsPath).toString("utf8").replace("<id>", id)
		);
		await Promise.all([
	...
	test("Ensure ipykernel install prompt is displayed every time you try to run a cell in a Notebook", async function () {
		...
		await waitForKernelToChange(
			{ interpreterPath: venvNoKernelPath },
			editor
		);
		...
		await waitForCondition(
			async () =>
				hasErrorOutput(cell.outputs) && assertVSCCellIsNotRunning(cell),
			defaultNotebookTestTimeout,
	...
	test.skip("Ensure ipykernel install prompt is displayed every time you try to run a cell in an Interactive Window and you can switch kernels", async function () {
		...
		const pythonApiProvider =
			api.serviceManager.get<IPythonApiProvider>(IPythonApiProvider);
		const source = "print(__file__)";
		const { activeInteractiveWindow, untitledPythonFile } =
			await submitFromPythonFile(
		...
		const notebookDocument = workspace.notebookDocuments.find(
			(doc) =>
				doc.uri.toString() ===
				activeInteractiveWindow?.notebookUri?.toString()
		)!;
		...
		await activeInteractiveWindow
			.addCode(source, untitledPythonFile.uri, 0)
			.catch(noop);
		await verifyIPyKernelPromptDisplayed(prompt, venvNoKernelPath.fsPath);
		await verifyErrorInCellOutput(
			notebookDocument,
		...
		assert.strictEqual(
			prompt.getDisplayCount(),
			1,
			"Should not display additional prompts"
		);
		...
		const stub = sinon
			.stub(kernelSelector, "selectKernel")
			.callsFake(async function () {
				...
				let editor =
					await activeInteractiveWindow.showInteractiveEditor();
				await waitForKernelToChange(
					{
						interpreterPath: venvNoRegPath,
						isInteractiveController: true,
					},
					editor
				);
				return true;
		...
		await Promise.all([
			...
			waitForCondition(
				() => prompt.getDisplayCount() === 2,
				delayForUITest,
				"Prompt not displayed twice"
			),
				...
				() =>
					prompt.messages.some((message) =>
						message.includes(
							path.basename(
								path.dirname(
									path.dirname(venvNoKernelPath.fsPath)
								)
							)
						)
					),
		...
		await activeInteractiveWindow
			.addCode(
				`import sys${EOL}print(sys.executable)`,
		...
		await waitForCondition(
			() => {
				...
				const sysExecutable = Uri.file(
					getCellOutputs(lastCodeCell).trim()
				);
	...
	test("Get a single prompt when running all cells in a .py file without ipykernel and will run all cells upon installation", async () => {
		...
		const { activeInteractiveWindow } =
			await submitFromPythonFileUsingCodeWatcher(
				source,
				disposables,
				venvNoKernelPath
			);
	...
	test("Ensure ipykernel install prompt is displayed even selecting another kernel which too does not have IPyKernel installed (VSCode Notebook)", async function () {
		...
		await openNotebookAndInstallIpyKernelWhenRunningCell(
			venvNoKernelPath,
			venvNoRegPath
		);
	});
	...
	test("Ensure ipykernel install prompt is displayed and we can select another kernel after uninstalling IPyKernel from a live notebook and then restarting the kernel (VSCode Notebook)", async function () {
		...
		// Now that IPyKernel is missing, if we attempt to restart a kernel, we should get a prompt.
		// Previously things just hang at weird spots, its not a likely scenario, but this test ensures the code works as expected.
		const startController = controllerRegistration.getSelected(
		...
		const { kernelSelected } = hookupKernelSelected(
			promptToInstall,
			venvNoRegPath
		);
		...
		await Promise.all([
			...
			waitForCondition(
				async () => kernelSelected,
				delayForUITest,
				"New kernel not selected"
			),
	...
	test("Ensure ipykernel install prompt will switch", async function () {
		...
		const stub = sinon
			...
			.callsFake(async function () {
				await waitForKernelToChange(
					{ interpreterPath: venvKernelPath },
					editor
				);
				return true;
			} as any);
	...
	test("Ensure ipykernel install prompt is NOT displayed when auto start is enabled & ipykernel is missing (VSCode Notebook)", async function () {
		...
		assert.strictEqual(
			promptToInstall.getDisplayCount(),
			0,
			"Prompt should not have been displayed"
		);
	...
	async function verifyErrorInCellOutput(
	...
	async function verifyIPyKernelPromptDisplayed(
		prompt: WindowPromptStub,
		venvPath: string
	) {
		// The prompt should be displayed when we run a cell.
		await waitForCondition(
		...
		assert.equal(
			prompt.getDisplayCount(),
			1,
			"Display prompt shown more than once"
		);
	...
	async function openNotebookAndInstallIpyKernelWhenRunningCell(
		...
		ipykernelInstallRequirement:
			| "DoNotInstallIPyKernel"
			| "ShouldInstallIPYKernel" = "ShouldInstallIPYKernel"
	) {
		...
		try {
			if (
				!workspace.notebookDocuments.some(
					(item) =>
						item.uri.fsPath.toLowerCase() ===
						nbFile.fsPath.toLowerCase()
				)
			) {
				const { editor } = await openNotebook(nbFile);
			...
			const cell =
				vscodeNotebook.activeNotebookEditor?.notebook.cellAt(0)!;
			...
			const expectedInterpreterPath =
				interpreterOfNewKernelToSelect || interpreterPath;
			assert.isTrue(
				areInterpreterPathsSame(
					expectedInterpreterPath,
					Uri.file(output)
				),
				`Kernel points to ${getDisplayPathFromLocalFile(
					output
				)} but expected ${getDisplayPath(expectedInterpreterPath)}`
			);
				...
				if (installerSpy.callCount > 0) {
					...
					assert.fail(
						`IPyKernel was installed when it should not have been, here are the calls: ${installerSpy
							.getCalls()
							.map((call) => {
								const args: Parameters<IInstaller["install"]> =
									call.args as any;
	...
	function waitForIPyKernelToGetInstalled() {
		return waitForCondition(
			...
			delayForUITest,
			() =>
				`Prompt not displayed or not installed successfully, call count = ${
					installerSpy.callCount
				}, arg0 ${
					installerSpy.callCount
						? installerSpy.getCall(0).args[0]
						: undefined
				}, result ${
					installerSpy.callCount
						? installerSpy.getCall(0).returnValue
						: undefined
				}`
		);
	}
	async function verifyIPyKernelWasInstalled() {
		...
		assert.strictEqual(
			await installerSpy.getCall(0).returnValue,
	...
	function verifyInstallIPyKernelInstructionsInOutput(cell: NotebookCell) {
		const textToLookFor = `Run the following command to install '${ProductNames.get(
			Product.ipykernel
		)!}'`;
	...
	function hookupKernelSelected(
		promptToInstall: Awaited<
			ReturnType<typeof selectKernelFromIPyKernelPrompt>
		>,
		...
		const controller = controllerManager.registered.find(
			(item) =>
				...
				areInterpreterPathsSame(
					item.connection.interpreter.uri,
					pythonPathToNewKernel
				)
		);
		if (!controller) {
			const registeredControllers = controllerManager.registered
				.map((item) => item.id)
				.join(",");
			throw new Error(
		...
		const selectADifferentKernelStub = sinon
			...
			.callsFake(async function (cmd: string) {
				if (cmd === "notebook.selectKernel") {
					...
					return commands.executeCommand.apply(commands, [
						cmd,
		...
		return {
			kernelSelected: kernelSelected.promise,
			selectADifferentKernelStub,
		};
	}
});
