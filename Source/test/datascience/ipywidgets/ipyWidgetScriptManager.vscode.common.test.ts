...
suite("IPyWidget Script Manager @widgets", function () {
	...
	suiteSetup(async function () {
		...
		widgetScriptManagerFactory =
			api.serviceContainer.get<IIPyWidgetScriptManagerFactory>(
				IIPyWidgetScriptManagerFactory
			);
	...
	test("Upon restarting the kernel copy the nbextensions folder again", async function () {
		...
		const files = (await fs.getFiles(nbExtensionsFolder)).map((item) =>
			item.toString()
		);
		...
		assert.isTrue(
			await fs.exists(nbExtensionsFolder),
			`Directory '${nbExtensionsFolder}'does not exist`
		);

		// await del([nbExtensionsFolder]);
		const fsNode =
			api.serviceContainer.get<IFileSystemNode>(IFileSystemNode);
		...
		const newFiles = (await fs.getFiles(nbExtensionsFolder)).map((item) =>
			item.toString()
		);
	...
	test.skip("Get a list of Widgets and script paths", async () => {
		...
		await Promise.all(
			Object.keys(moduleMappings!).map(async (moduleName) => {
				...
				assert.isOk(
					uri,
					`Script Uri not defined for widget ${moduleName}`
				);
				if (!uri) {
					return;
				}
				...
				if (isLocalConnection(kernel.kernelConnectionMetadata)) {
					...
					assert.isTrue(
						fileExists,
						`File '${file}' does not exist on disc, for module ${moduleName}`
					);
				} else {
					...
					assert.isTrue(
						result.ok,
						`Uri '${file}' does not seem to be valid, for module ${moduleName}`
					);
				}
			})
		);
	...
	test("Should not contain any modules that we already bundle with our ipywidgets bundle", async () => {
		...
		[
			...
			assert.isFalse(
				moduleName in moduleMappings!,
				`Module ${moduleName} should not exist in the mapping`
			);
		});
	});
});
