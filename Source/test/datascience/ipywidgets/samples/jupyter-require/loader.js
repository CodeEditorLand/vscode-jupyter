...
define(["./core", "./display", "./logger"], function (core, display, Logger) {
	...
	function get_display_cells() {
		...
		return cells.filter((c) => {
			return (
				...
				c.output_area.outputs.some(
					(d) => d.output_type === "display_data"
				)
			);
		});
	}
	...
	function freeze_cells() {
		...
		return Promise.all(
			cells.map((cell) => display.freeze_cell_outputs(cell))
		)
	...
	function finalize_cells() {
		...
		return Promise.all(
			cells.map((cell) => display.finalize_cell_outputs(cell))
		)
			.then(() => {
	...
	function register_events() {
		events.on("config.JupyterRequire", (e, d) =>
			core.set_notebook_config(d.config)
		);
		events.on("require.JupyterRequire", (e, d) =>
			core.set_cell_requirements(d.cell, d.require)
		);
		...
		events.on("output_added.OutputArea", (e, d) => {
			...
			if (
				display_data instanceof display.DisplayData ||
				display_data.metadata.frozen === false
			) {
	...
	function init_existing_cells() {
		...
		cells.forEach(async (cell) => {
			...
			outputs.forEach((output) => {
				if (output.metadata === undefined) return;
	...
	return function load_extension({ reload = false } = {}) {
		return new Promise((resolve) => {
			...
			core.register_targets().then(log.debug);
