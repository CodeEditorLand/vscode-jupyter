...
suite("File Converter @export", () => {
	...
	setup(async () => {
		...
		when(
			filePicker.showDialog(anything(), anything(), anything())
		).thenReturn(Promise.resolve(Uri.file("test.pdf")));
		// eslint-disable-next-line no-empty,@typescript-eslint/no-empty-function
		when(appShell.showErrorMessage(anything())).thenResolve();
		// eslint-disable-next-line no-empty,@typescript-eslint/no-empty-function
		when(exportUtil.generateTempDir()).thenResolve({
			path: "test",
			dispose: () => {},
		});
		when(
			exportUtil.makeFileInDirectory(anything(), anything(), anything())
		).thenResolve("foo");
		when(
			exportUtil.getTargetFile(anything(), anything(), anything())
		).thenResolve(Uri.file("bar"));
		// eslint-disable-next-line no-empty,@typescript-eslint/no-empty-function
		when(fileSystem.createTemporaryLocalFile(anything())).thenResolve({
			filePath: "test",
			dispose: () => {},
		});
		when(
			exportPdf.export(anything(), anything(), anything(), anything())
		).thenResolve();
		when(
			exportPythonPlain.export(anything(), anything(), anything())
		).thenResolve();
		when(filePicker.showDialog(anything(), anything())).thenResolve(
			Uri.file("foo")
		);
		when(
			exportInterpreterFinder.getExportInterpreter(anything())
		).thenResolve();
		when(exportFileOpener.openFile(anything(), anything())).thenResolve();
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		when(
			reporter.createProgressIndicator(anything(), anything())
		).thenReturn(instance(mock<IDisposable>()) as any);
	...
	test("Erorr message is shown if export fails", async () => {
		when(
			exportHtml.export(anything(), anything(), anything(), anything())
	...
	test("Export to Python is called when export method is Python", async () => {
		...
		verify(
			exportFileOpener.openFile(ExportFormat.python, anything())
		).once();
	});
});
