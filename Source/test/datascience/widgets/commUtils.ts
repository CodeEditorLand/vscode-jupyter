...
export function initializeWidgetComms(disposables: IDisposable[]): Utils {
	...
	const disposable = messageChannel.onDidReceiveMessage(
		async ({ editor, message }) => {
			...
			if (message && message.command === "INIT") {
				...
				messageChannel
					.postMessage({ command: "hijackLogging" }, editor)
					.then(noop, noop);
			}
		}
	);
...
export class Utils {
	...
	public async queryHtml(cell: NotebookCell, selector?: string) {
		...
		traceInfo(
			`Sending message to Widget renderer ${JSON.stringify(request)}`
		);
		this.messageChannel.postMessage!(request, editor).then(noop, noop);
		return new Promise<string>((resolve, reject) => {
			const disposable = this.messageChannel.onDidReceiveMessage(
				({ message }) => {
	...
	public async click(cell: NotebookCell, selector: string) {
		...
		return new Promise<void>((resolve, reject) => {
			const disposable = this.messageChannel.onDidReceiveMessage(
				({ message }) => {
					traceInfo(
						`Received message (click) from Widget renderer ${JSON.stringify(
							message
						)}`
					);
					if (message && message.requestId === request.requestId) {
	...
	public async setValue(cell: NotebookCell, selector: string, value: string) {
		...
		return new Promise<void>((resolve, reject) => {
			const disposable = this.messageChannel.onDidReceiveMessage(
				({ message }) => {
					traceInfo(
						`Received message (setValue) from Widget renderer ${JSON.stringify(
							message
						)}`
					);
					if (message && message.requestId === request.requestId) {
