...
[true, false].forEach((useCDN) => {
	...
	suite(
		`Third party IPyWidget Tests ${
			useCDN ? "with CDN" : "without CDN"
		} @widgets`,
		function () {
			...
			const widgetScriptSourcesValue: WidgetCDNs[] = useCDN
			...
			test("Slider Widget", async function () {
				...
				const version = getTextOutputValue(
					cellVersion.outputs[0]
				).trim();
			...
			test("Render IPySheets & search", async function () {
				...
				await assertOutputContainsHtml(cell2, comms, [
					">train<",
					">foo<",
				]);
