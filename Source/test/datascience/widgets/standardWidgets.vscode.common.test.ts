...
export async function initializeNotebookForWidgetTest(
	...
	const nbEdit = NotebookEdit.replaceCells(
		new NotebookRange(0, editor.notebook.cellCount),
		newCells
	);
...
export async function assertOutputContainsHtml(
	...
	await waitForCondition(
		async () => {
			...
			htmlFragmentsToLookFor.forEach((fragment) =>
				assert.include(html, fragment)
			);
			return true;
		},
		WidgetRenderingTimeoutForTests,
		() =>
			`Widget did not render or ${htmlFragmentsToLookFor.join(
				", "
			)} not in html = ${html}`,
...
suite("Standard IPyWidget Tests @widgets", function () {
	...
	suiteSetup(async function () {
		...
		sinon.restore();
		editor = (await createEmptyPythonNotebook(disposables, undefined, true))
			.editor;
	...
	test("Slider Widget", async function () {
		await initializeNotebookForWidgetTest(
			disposables,
			{ templateFile: "slider_widgets.ipynb" },
			editor
		);
		...
		const cellVersion =
			vscodeNotebook.activeNotebookEditor?.notebook.cellAt(4)!;
		await Promise.all([
			runCell(cellVersion),
			waitForCondition(
	...
	suite("All other Widget tests", () => {
		...
		test("Textbox Widget", async () => {
			...
			const cell =
				vscodeNotebook.activeNotebookEditor?.notebook.cellAt(1)!;
			await executeCellAndWaitForOutput(cell, comms);
			await assertOutputContainsHtml(
				cell,
				comms,
		...
		test("Linking Widgets slider to textbox widget", async function () {
			...
			const [, cell1, cell2, cell3] =
				vscodeNotebook.activeNotebookEditor!.notebook.getCells()!;
			...
			await assertOutputContainsHtml(cell3, comms, [
				'<input type="number',
			]);
		...
		test("Checkbox Widget", async () => {
			...
			const cell =
				vscodeNotebook.activeNotebookEditor?.notebook.cellAt(2)!;
			await executeCellAndWaitForOutput(cell, comms);
			await assertOutputContainsHtml(
				cell,
				comms,
		...
		test("Button Widget (click button)", async () => {
			await initializeNotebookForWidgetTest(
				disposables,
				{ templateFile: "button_widgets.ipynb" },
				editor
			);
			const [cell0, cell1, cell2] =
				vscodeNotebook.activeNotebookEditor!.notebook.getCells();

			await executeCellAndWaitForOutput(cell0, comms);
			await executeCellAndWaitForOutput(cell1, comms);
			await executeCellAndWaitForOutput(cell2, comms);
			await assertOutputContainsHtml(cell0, comms, [
				"Click Me!",
				"<button",
			]);
			await assertOutputContainsHtml(cell1, comms, [
				"Click Me!",
				"<button",
			]);
		...
		test("Nested Output Widgets", async () => {
			...
			await assertOutputContainsHtml(
				cell1,
				comms,
				['<input type="text', "Label Widget"],
				".widget-output"
			);
			assert.strictEqual(
				cell3.outputs.length,
				0,
				"Cell 3 should not have any output"
			);

			// Run the 4th cell & verify we have output in the first nested output & second output.
			await Promise.all([
				runCell(cell4),
				waitForCellExecutionToComplete(cell2),
			]);
			await assertOutputContainsHtml(
				cell1,
				comms,
			...
			await comms.setValue(
				cell1,
				".widget-text input",
				"Widgets are linked an get updated"
			);
			await assertOutputContainsHtml(
				cell1,
				comms,
		...
		test("More Nested Output Widgets", async () => {
			...
			const [cell1, cell2, cell3, cell4, cell5, cell6] =
				vscodeNotebook.activeNotebookEditor!.notebook.getCells();
			let html = "";
			...
			const runCellAndTestOutput = async (cell: NotebookCell) => {
				...
				const firstRandomNumber = matches[0]
					.replace("Outside", "")
					.trim();
				...
				assert.strictEqual(matches?.length, 2, "Expected two matches");
				assert.strictEqual(
					matches[0].replace("Outside", "").trim(),
					matches[1].replace("Inside", "").trim()
				);
				const secondRandomNumber = matches[0]
					.replace("Outside", "")
					.trim();
		...
		test("Interactive Button", async () => {
			...
			await waitForCondition(
				() => {
					assert.strictEqual(
						getTextOutputValue(cell.outputs[1]).trim(),
						"Button clicked"
					);
					return true;
				},
		...
		test("Interactive Function", async () => {
			...
			assert.strictEqual(
				getTextOutputValue(cell.outputs[2]).trim(),
				`'Hello World'`
			);
		...
		test("Interactive Plot", async function () {
			...
			assert.strictEqual(
				cell.outputs.length,
				4,
				"Cell should have 4 outputs"
			);

			// This cannot be displayed by output widget, hence we need to handle this.
			assert.strictEqual(
				cell.outputs[1].items[0].mime,
				"application/vnd.custom"
			);
			assert.strictEqual(
				Buffer.from(cell.outputs[1].items[0].data).toString(),
				"Text Value is Foo"
			);
			...
			const outputUpdated = new Promise<boolean>((resolve) => {
				workspace.onDidChangeNotebookDocument(
					(e) => {
						...
						if (
							!currentCellChange ||
							!currentCellChange.outputs ||
							currentCellChange.outputs.length < 4
						) {
							return;
						}
						...
						if (
							Buffer.from(
								secondOutput.items[0].data
							).toString() === "Text Value is Bar"
						) {
							resolve(true);
						}
					},
			...
			assert.strictEqual(
				Buffer.from(cell.outputs[1].items[0].data).toString().trim(),
				"Text Value is Bar"
			);
