	...
	renderOutputItem: (outputItem: OutputItem, element: HTMLElement) => {
		...
		if (rendererContext && rendererContext.postMessage) {
			rendererContext.postMessage({
				command: "log",
				message: `Rendering (2) ${outputItem.id}`,
			});
			const message = {
				command: "TEST_RENDER_OUTPUT",
				data: outputItem.id,
			};
...
function initializeComms() {
	...
	rendererContext.onDidReceiveMessage((message) => {
		...
		if (handlers.has(message.command)) {
			...
			rendererContext.postMessage!({
				command: "log",
				message: `Error: Message not handled in Widget renderer ${JSON.stringify(
					message
				)}`,
			});
...
function queryInnerHTMLHandler({
	...
	try {
		const nodes = document.querySelectorAll(
			`.vsc-test-cell-index-${cellIndex} ${selector || ""}`.trim()
		);
		if (!nodes.length) {
...
function clickHandler({
	...
	try {
		const nodes = document.querySelectorAll(
			`.vsc-test-cell-index-${cellIndex} ${selector}`
		);
		if (!nodes.length) {
			return rendererContext.postMessage!({
				requestId,
...
function convertVSCodeOutputToExecuteResultOrDisplayData(
	...
	return {
		data: {
			[outputItem.mime]: outputItem.mime.toLowerCase().includes("json")
				? outputItem.json()
				: outputItem.text(),
		},
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		output_type:
			(outputItem.metadata as any)?.outputType || "execute_result",
	};
}
...
function hijackLogging() {
	...
	(["log", "error", "warn", "debug", "trace"] as ConsoleChannel[]).forEach(
		(channel) => {
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
