...
suite("Tests", () => {
	...
	setup(() => {
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		when(docManager.onDidChangeActiveTextEditor).thenReturn(
	...
	suite("Cell pruning", () => {
		test("Remove output and execution count from non code", () => {
			...
			assert.equal(
				Object.keys(result).indexOf("outputs"),
				-1,
				"Outputs inside markdown"
			);
			assert.equal(
				Object.keys(result).indexOf("execution_count"),
				-1,
				"Execution count inside markdown"
			);
		});
		test("Outputs dont contain extra data", () => {
			...
			assert.equal(
				...
				"23",
				"Output execution count removed"
			);
			const output = (result.outputs as nbformat.IOutput[])[0];
			assert.equal(
				Object.keys(output).indexOf("extra"),
				-1,
				"Output still has extra data"
			);
			assert.notEqual(
				Object.keys(output).indexOf("output_type"),
				-1,
				"Output is missing output_type"
			);
		});
				...
				outputs: [
					{
						...
						data: {
							"text/plain":
								"Box(children=(Label(value='My label'),))",
