...
suite("Non-Python Kernel @nonPython ", async function () {
	...
	const testJavaKernels =
		(process.env.VSC_JUPYTER_CI_RUN_JAVA_NB_TEST || "").toLowerCase() ===
		"true";
	...
	suiteSetup(async function () {
		...
		pythonChecker = api.serviceContainer.get<IPythonExtensionChecker>(
	...
	setup(async function () {
		...
		testJuliaNb = await createTemporaryNotebookFromFile(
			juliaNb,
			disposables
		);
		testJavaNb = await createTemporaryNotebookFromFile(javaNb, disposables);
		testCSharpNb = await createTemporaryNotebookFromFile(
			csharpNb,
			disposables
		);
		...
		await waitForCondition(
			async () => {
				const preferredController =
					await controllerPreferred.computePreferred(notebook);
				if (
					preferredController.preferredConnection?.kind ===
						"startUsingLocalKernelSpec" &&
					preferredController.preferredConnection.kernelSpec
						.language === "java"
				) {
			...
			`Preferred controller not found for Notebook, currently preferred ${controllerPreferred.getPreferred(
				notebook
			)?.connection.kind}:${controllerPreferred.getPreferred(notebook)
				?.connection.id}`,
			500
		);
	});
