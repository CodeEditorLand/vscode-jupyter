...
export async function findKernelSpecMatchingInterpreter(
	...
	await Promise.all(
		kernels.map(async (kernel) => {
			...
			if (
				...
				getKernelRegistrationInfo(kernel.kernelSpec) !==
					"registeredByNewVersionOfExtForCustomKernelSpec" &&
				(await getInterpreterHash(kernel.interpreter)) ===
					(await getInterpreterHash(interpreter)) &&
				kernel.interpreter.envName === interpreter.envName
			) {
				result.push(kernel);
			}
	...
	if (result.length > 1) {
		traceError(
			`More than one kernel spec matches the interpreter ${interpreter.uri}.`,
			result
		);
		if (isCI) {
			throw new Error(
				"More than one kernelspec matches the intererpreter"
			);
		}
	}
...
export async function rankKernels(
	...
	traceInfo(
		`Find preferred kernel for ${getDisplayPath(
			resource
		)} with metadata ${JSON.stringify(
			notebookMetadata || {}
		)} & preferred interpreter ${
			preferredInterpreter?.uri
				? getDisplayPath(preferredInterpreter?.uri)
				: "<undefined>"
		}`
	);
	...
	let preferredInterpreterKernelSpec =
		...
		(await findKernelSpecMatchingInterpreter(
			preferredInterpreter,
			kernels
		));
	...
	traceInfoIfCI(
		`preferredInterpreterKernelSpecIndex = ${preferredInterpreterKernelSpec?.id}`
	);
	...
	const actualNbMetadataLanguage: string | undefined =
		...
		(
			notebookMetadata?.kernelspec as undefined | IJupyterKernelSpec
	...
	kernels = kernels.filter((kernel) => {
		...
		if (
			...
			kernel.kernelSpec.language.toLowerCase() !==
				possibleNbMetadataLanguage
		) {
			return false;
		}
	...
	if (getResourceType(resource) === "interactive") {
		...
		possibleNbMetadataLanguage =
			...
			!possibleNbMetadataLanguage
				? PYTHON_LANGUAGE
				: (
						(notebookMetadata?.kernelspec?.language as string) ||
	...
	const interpreterHashes = new Map<
		KernelConnectionMetadata,
		string | undefined
	>();
	await Promise.all(
		kernels.map(async (a) => {
			const interpreterHashForKernel = a.interpreter
				? await getInterpreterHash(a.interpreter)
				: undefined;
...
export function compareKernels(
	...
	if (!notebookMetadata?.kernelspec) {
		...
		if (a === activeInterpreterConnection) {
			...
			const aVersion =
				...
				(aSysVersion.length &&
				!isNaN(parseInt(aSysVersion.substring(0)))
			...
			const bVersion =
				a.interpreter?.version?.major ||
				(bSysVersion.length &&
				!isNaN(parseInt(bSysVersion.substring(0)))
	...
	const originalSpecFileA =
		a.kernelSpec.metadata?.vscode?.originalSpecFile ||
		a.kernelSpec.metadata?.originalSpecFile;
	const originalSpecFileB =
		b.kernelSpec.metadata?.vscode?.originalSpecFile ||
		b.kernelSpec.metadata?.originalSpecFile;
	...
	if (
		...
		if (
			...
			bLang &&
			aLang !== possibleNbMetadataLanguage &&
			bLang !== possibleNbMetadataLanguage
		) {
			return 0;
		} else if (
			aLang === possibleNbMetadataLanguage &&
	...
	const comparisonOfDisplayNames =
		compareAgainstKernelDisplayNameInNotebookMetadata(
			a,
			b,
			notebookMetadata
		);
	const comparisonOfInterpreter = compareAgainstInterpreterInNotebookMetadata(
		aInfo,
		bInfo,
		notebookMetadata
	);
...
function givePreferenceToStartingWithoutCustomKernelSpec(
...
function compareKernelSpecOrEnvNames(
	...
	if (!notebookMetadata?.kernelspec?.name) {
		...
		notebookMetadata.kernelspec.name
			.toLowerCase()
			.match(isDefaultPythonKernelSpecName)
	) {
		const kernelRegA =
			a.kind !== "connectToLiveRemoteKernel"
				? getKernelRegistrationInfo(a.kernelSpec)
				: "";
		const kernelRegB =
			b.kind !== "connectToLiveRemoteKernel"
				? getKernelRegistrationInfo(b.kernelSpec)
				: "";
...
function compareAgainstInterpreterInNotebookMetadata(
	...
	if (
		aMetadata.kind === "connectToLiveRemoteKernel" &&
		bMetadata.kind === "connectToLiveRemoteKernel"
	) {
		return 0;
	...
	const interpreterMatchesThatInNotebookMetadataA =
		!!interpreterMatchesThatInNotebookMetadata(
			aMetadata,
			notebookMetadata,
			aInterpreterHash
		);
	const interpreterMatchesThatInNotebookMetadataB =
		!!interpreterMatchesThatInNotebookMetadata(
			bMetadata,
			notebookMetadata,
			bInterpreterHash
	...
	if (
		...
		aMetadata.kind === "startUsingLocalKernelSpec" &&
		aMetadata.kind !== bMetadata.kind
	) {
		// Give preference to kernel a that starts using a custom kernelspec python.
		return 1;
	} else if (
		bMetadata.kind === "startUsingLocalKernelSpec" &&
		aMetadata.kind !== bMetadata.kind
	) {
		// Give preference to kernel a that starts using a custom kernelspec python.
		return 1;
		...
		try {
			const preferredRemoteKernelId =
				resource && this.preferredRemoteFinder
					? await this.preferredRemoteFinder.getPreferredRemoteKernelId(
							resource
					  )
					: undefined;
			if (cancelToken?.isCancellationRequested) {
