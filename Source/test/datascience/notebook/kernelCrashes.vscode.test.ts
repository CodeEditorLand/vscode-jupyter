...
suite("VSCode Notebook Kernel Error Handling - @kernelCore", function () {
	...
	suiteSetup(async function () {
		...
		try {
			...
			const languageService =
				api.serviceContainer.get<NotebookCellLanguageService>(
					NotebookCellLanguageService
				);
			const commandManager =
				api.serviceContainer.get<ICommandManager>(ICommandManager);
			const documentManager =
				api.serviceContainer.get<IDocumentManager>(IDocumentManager);
			const workspaceService =
				api.serviceContainer.get<IWorkspaceService>(IWorkspaceService);
			const configuration =
				api.serviceContainer.get<IConfigurationService>(
					IConfigurationService
				);
			const appShell =
				api.serviceContainer.get<IApplicationShell>(IApplicationShell);
			const extensionChecker =
				api.serviceContainer.get<IPythonExtensionChecker>(
					IPythonExtensionChecker
				);
			const providerRegitry =
				api.serviceContainer.get<IJupyterUriProviderRegistration>(
					IJupyterUriProviderRegistration
				);
			const browser =
				api.serviceContainer.get<IBrowserService>(IBrowserService);
			const platform =
				api.serviceContainer.get<IPlatformService>(IPlatformService);
			const interpreters =
			...
			const createNbController = sinon.stub(
				vscodeNotebook,
				"createNotebookController"
			);
			disposables.push(
				new Disposable(() => createNbController.restore())
			);
			createNbController.callsFake((id, _view, _label, handler) => {
				...
				when(nbController.onDidChangeSelectedNotebooks).thenReturn(
					onDidChangeSelectedNotebooks.event
				);
				when(nbController.onDidReceiveMessage).thenReturn(
	...
	suite("Raw Kernels", () => {
		...
		async function runAndFailWithKernelCrash() {
			...
			await Promise.race([
				Promise.all([
					terminatingEventFired,
					deadEventFired,
					prompt.displayed,
				]),
				sleep(10_000).then(() =>
					Promise.reject(new Error("Did not fail"))
				),
			]);
			prompt.dispose();
			...
			await waitForCondition(
				async () => {
					...
					return (
						...
						output.includes(
							"https://aka.ms/vscodeJupyterKernelCrash"
						)
					);
				},
				defaultNotebookTestTimeout,
				() =>
					`Cell did not have kernel crash output, the output is = ${getCellOutputs(
						cell2
					)}`
			);
		}
		...
		test("Ensure cell output does not have errors when execution fails due to dead kernel", async function () {
			...
			assert.isUndefined(
				...
				"Should not have an execution order"
			);
		});
		test("Ensure we get only one prompt to restart kernel when running all cells against a dead kernel", async function () {
			...
			await Promise.all([
				restartPrompt.displayed,
				cellExecutionHandler(notebook.cells, notebook, controller),
			]);
			// Wait a while, it shouldn't take 1s, but things could be slow on CI, hence wait a bit longer.
			await sleep(1_000);

			assert.strictEqual(
				restartPrompt.getDisplayCount(),
				1,
				"Should only have one restart prompt"
			);
		});
		async function createAndOpenTemporaryNotebookForKernelCrash(
			nbFileName: string
		) {
		...
		async function displayErrorAboutOverriddenBuiltInModules() {
			...
			await Promise.race([
				prompt.displayed,
				sleep(10_000).then(() =>
					Promise.reject("Prompt not displayed")
				),
			]);
			prompt.dispose();
			...
			assert.include(
				err.traceback.join(""),
				"Consider renaming the file and starting the kernel again"
			);
		}
