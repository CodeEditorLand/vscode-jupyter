...
suite("Kernel Selection @kernelPicker", function () {
    ...
    */
	suiteSetup(async function () {
		...
		const pythonChecker = api.serviceContainer.get<IPythonExtensionChecker>(
			IPythonExtensionChecker
		);
		vscodeNotebook =
		...
		const [activeInterpreter, interpreter1, interpreter2, interpreter3] =
			await waitForCondition(
				async () => {
					try {
		...
		await Promise.all([
			proc.exec(venvNoKernelPython.fsPath, [
				"-m",
				"pip",
				"install",
				"ipykernel",
			]),
			proc.exec(venvKernelPython.fsPath, [
				"-m",
				"pip",
				"install",
				"ipykernel",
			]),
			proc.exec(venvNoRegPythonPath.fsPath, [
				"-m",
				"pip",
				"install",
				"ipykernel",
	...
	setup(async function () {
		...
		const env = await pythonApi?.environments.resolveEnvironment(
			venvNoKernelPythonPath.fsPath
		);
		// Don't use same file (due to dirty handling, we might save in dirty.)
		// Coz we won't save to file, hence extension will backup in dirty file and when u re-open it will open from dirty.
		nbFile1 = await createTemporaryNotebookFromFile(
			templateIPynbFile,
			disposables,
		...
		await waitForCondition(
			async () => {
				...
				if (
					...
					!output.includes(
						getNormalizedInterpreterPath(activeInterpreterPath)
							.fsPath
					) &&
					...
					assert.fail(
						output,
						`Expected ${getNormalizedInterpreterPath(
			...
			`Interpreter does not match for ${activeInterpreterSearchString}: expected ${getNormalizedInterpreterPath(
				activeInterpreterPath
			)} or ${activeInterpreterPath}, but go ${getTextOutputValue(
				cell.outputs[0]
			)}`
		);
	});
	...
	test("Ensure we select a Python kernel for a nb with python language information", async function () {
		...
		const cell2 =
			vscodeNotebook.activeNotebookEditor?.notebook.getCells()![1]!;
	...
	test("Switch kernel to an interpreter that is registered as a kernel", async function () {
		...
		if (activeInterpreterSearchString !== venvKernelSearchString) {
			assert.equal(
				outputText.toLowerCase().indexOf(venvKernelSearchString),
				-1
			);
		}
		...
		assert.isFalse(
			kernel ===
				kernelProvider.get(window.activeNotebookEditor!.notebook),
	...
	test("Switch kernel to an interpreter that is not registered as a kernel", async function () {
		...
		const outputText = getTextOutputValue(cell.outputs[0]).trim();
		assert.equal(
			outputText.toLowerCase().indexOf(venvNoRegSearchString),
			-1
		);
		...
		// Clear the cells & execute again
		commands
			.executeCommand("notebook.clearAllCellsOutputs")
			.then(noop, noop);
		await waitForCondition(
			async () => cell.outputs.length === 0,
			5_000,
