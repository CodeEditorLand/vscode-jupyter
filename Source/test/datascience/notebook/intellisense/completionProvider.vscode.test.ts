...
[true, false].forEach((useJedi) => {
	suite(
		...
		function () {
			...
			let previousPythonCompletionTriggerCharactersValue:
				| string
				| undefined;
			...
			suiteSetup(async function () {
				...
				this.timeout(120_000);
				jupyterConfig = workspace.getConfiguration(
					"jupyter",
					undefined
				);
				previousPythonCompletionTriggerCharactersValue =
					jupyterConfig.get<string>(
						"pythonCompletionTriggerCharacters"
					);
				previousJediSetting = jupyterConfig.get<boolean>(
				...
				completionProvider =
					api.serviceContainer.get<PythonKernelCompletionProvider>(
						PythonKernelCompletionProvider
					);
				traceInfo(
					`Start Suite (Completed) Code Completion via Jupyter`
				);
			});
			...
			setup(async function () {
				...
				sinon.restore();
				await startJupyterServer();
				await createEmptyPythonNotebook(
					disposables,
					Uri.file(path.join(EXTENSION_ROOT_DIR_FOR_TESTS, "temp"))
				); // TODO, can't do this on web tests
			...
			async function testCompletions(
				...
				const namesCsvPath = path
					.join(__dirname, "names.csv")
					.replace(/\\/g, "/")
					.replace("out", "src");
				...
				const cell4 =
					vscodeNotebook.activeNotebookEditor!.notebook.cellAt(3);
				// If we're testing string completions, ensure the cursor position is inside the string quotes.
				let position = new Position(
					0,
					cellCode.includes('"') || cellCode.includes("'")
						? cellCode.length - 1
						: cellCode.length
				);
				traceInfo("Get completions in test");
				let completions = await waitForCompletions(
					completionProvider,
					cell4,
					position,
					triggerCharacter
				...
				if (
					!textToFilterCompletions ||
					!itemToExistInCompletionAfterFilter
				) {
					return;
				}
				...
				edit.insert(
					cell4.document.uri,
					new Position(cellCode.length, 0),
					textToFilterCompletions
				);
				await workspace.applyEdit(edit);
				position = new Position(
					0,
					cellCode.length + textToFilterCompletions.length
				);
				completions = await waitForCompletions(
					completionProvider,
					cell4,
					position,
					triggerCharacter
			...
			test("Dataframe completions", async () => {
				const fileName = path.basename(
					vscodeNotebook.activeNotebookEditor!.notebook.uri.fsPath
				);
