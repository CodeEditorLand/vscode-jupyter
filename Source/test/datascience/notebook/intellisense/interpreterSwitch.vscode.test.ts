...
suite("Intellisense Switch interpreters in a notebook @lsp", function () {
	...
	suiteSetup(async function () {
		...
		const [activeInterpreter, interpreter1, interpreter2] =
			await waitForCondition(
				async () => {
					try {
		...
		const factory = api.serviceContainer.get<IPythonExecutionFactory>(
			IPythonExecutionFactory
		);
		const process = await factory.create({
			interpreter: { uri: venvNoKernelPythonPath } as PythonEnvironment,
		});
		await process.execModule("pip", ["uninstall", "pandas"], {
			throwOnStdErr: false,
		});
		...
		sinon.restore();
		traceInfo(
			`Start Suite (Completed) Intellisense Switch interpreters in a notebook`
		);
	});
	...
	test.skip("Check diagnostics with and without an import", async () => {
		...
		await waitForKernelToChange({
			interpreterPath: venvNoKernelPythonPath,
		});
