...
suite("Export @export", function () {
	...
	suiteSetup(async function () {
		...
		try {
			...
			importer =
				api.serviceContainer.get<JupyterImporter>(INotebookImporter);
			nbConvertDependencyChecker =
				api.serviceContainer.get<INbConvertInterpreterDependencyChecker>(
					INbConvertInterpreterDependencyChecker
				);
			interpreterService =
		...
		try {
			...
			defaultCellMarker =
				api.serviceContainer
					...
					.getSettings().defaultCellMarker ||
				Identifiers.DefaultCodeCellMarker;
			const version =
				await nbConvertDependencyChecker.getNbConvertVersion(
					activeInterpreter
				);
			if (!template) {
				template = (await importer.createTemplateFile(
					version!.major >= 6
				))!;
			}
		} catch (e) {
	...
	test("Export a basic notebook document", async () => {
		...
		assert(
			...
			"Document opened by export was not python"
		);
	...
	test("Import a notebook file from disk", async () => {
		...
		expect(text).to.equal(
			`# %%${os.EOL}a=1${os.EOL}a${os.EOL}${os.EOL}`,
