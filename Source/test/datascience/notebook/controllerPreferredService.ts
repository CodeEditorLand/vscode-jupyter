...
export class ControllerPreferredService {
	private preferredControllers = new WeakMap<
		NotebookDocument,
		IVSCodeNotebookController
	>();
	private preferredCancelTokens = new WeakMap<
		NotebookDocument,
		CancellationTokenSource
	>();
	...
	public static create(serviceContainer: IServiceContainer) {
		if (!ControllerPreferredService.instance) {
			ControllerPreferredService.instance =
				new ControllerPreferredService(
					serviceContainer.get<IControllerRegistration>(
						IControllerRegistration
					),
			...
			serviceContainer
				...
				.push(ControllerPreferredService.instance);
	...
	public activate() {
		...
		this.notebook.notebookDocuments.forEach((notebook) =>
			this.onDidOpenNotebookDocument(notebook)
		);
		this.disposables.add(
		...
		this.disposables.add(
			this.registration.onDidChange(
				({ added }) =>
					added.length
						? this.notebook.notebookDocuments.map((nb) =>
								this.onDidOpenNotebookDocument(nb)
						  )
						: undefined,
				this
			)
	...
	public async computePreferred(
		...
		traceInfoIfCI(
			`Clear controller mapping for ${getDisplayPath(document.uri)}`
		);
		...
		const changeHandler = cancelToken?.onCancellationRequested(() =>
			preferredSearchToken.cancel()
		);
		...
		try {
			...
			if (
				...
				const defaultPythonController =
					await this.defaultService.computeDefaultController(
						document.uri,
				...
				if (preferredConnection) {
					traceInfoIfCI(
						`Found target controller with default controller ${getDisplayPath(
							document.uri
			...
			if (
				...
				if (preferredConnection) {
					traceInfoIfCI(
						`Found target controller with an exact match (1) ${getDisplayPath(
							document.uri
						)} ${preferredConnection.kind}:${
							preferredConnection.id
						}.`
				...
				if (!preferredConnection) {
					...
					preferredConnection =
						await this.findPreferredKernelExactMatch(
							document,
							notebookMetadata,
							preferredSearchToken.token,
							undefined
					...
					if (preferredConnection) {
						traceInfoIfCI(
							`Found target controller with an exact match (2) ${getDisplayPath(
								document.uri
							)} ${preferredConnection.kind}:${
								preferredConnection.id
							}.`
				...
				if (
					preferredSearchToken.token.isCancellationRequested &&
					!preferredConnection
				) {
				...
				if (!preferredConnection) {
					traceInfoIfCI(
						`PreferredConnection not found for NotebookDocument: ${getDisplayPath(
							document.uri
						)}`
					);
					if (
						!preferredConnection &&
						this.preferredControllers.get(document)
					) {
						...
						traceVerbose(
							`Resetting the previous preferred controller ${this.preferredControllers.get(
								document
							)
								?.id} to default affinity for document ${getDisplayPath(
								document.uri
							)}`
						);
				...
				traceInfo(
					`PreferredConnection: ${
						preferredConnection.id
					} found for NotebookDocument: ${getDisplayPath(
						document.uri
					)}`
				);
				...
				if (!targetController) {
					traceVerbose(
						`Early registration of controller for Kernel connection ${preferredConnection.id}`
					);
					// eslint-disable-next-line @typescript-eslint/no-explicit-any
					this.registration.addOrUpdate(preferredConnection, [
						JupyterNotebookView,
					]);
				}
				...
				const defaultInteractiveController =
					await this.defaultService.computeDefaultController(
						document.uri,
						"interactive"
					);
			...
			if (preferredConnection) {
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
				targetController = this.registration.get(
					preferredConnection,
					document.notebookType as any
				);
			}

			if (targetController) {
				traceVerbose(
					`TargetController found ID: ${
						targetController.connection.kind
					}:${targetController.id} for document ${getDisplayPath(
						document.uri
					)}`
				);
				await this.preferredControllers
			...
			if (
				preferredConnection &&
				!targetController &&
				isLocalConnection(preferredConnection)
			) {
				...
				const controller = this.registration.addOrUpdate(
					preferredConnection,
					[
						document.notebookType as
							| typeof JupyterNotebookView
							| typeof InteractiveWindowView,
					]
				);
				traceInfoIfCI(
					`Controller for preferred connection ${
						preferredConnection.kind
					}${preferredConnection.id} for notebook ${getDisplayPath(
						document.uri
					)} does not yet exist, creating this now`
				);
				if (controller.length === 1) {
					targetController = controller[0];
				} else {
					traceError(
						`Failed to create the controller for preferred connection ${
							preferredConnection.kind
						}${
							preferredConnection.id
			...
			traceInfoIfCI(
				`TargetController found ID: ${preferredConnection?.id} type ${preferredConnection?.kind} for document ${getDisplayPath(
					document.uri
				)} & associated controller id ${targetController?.connection
					?.kind}:${targetController?.id}`
			);
	...
	private readonly debouncedPreferredCompute = new WeakMap<
		NotebookDocument,
		IDisposable
	>();
	/**
	...
	private onDidOpenNotebookDocument(document: NotebookDocument) {
		...
		if (
			(document.notebookType !== JupyterNotebookView &&
				document.notebookType !== InteractiveWindowView) ||
			!workspace.isTrusted
		) {
			return;
		}
	...
	private async findPreferredKernelExactMatch(
		...
		if (rankedConnections && rankedConnections.length) {
			const potentialMatch =
				rankedConnections[rankedConnections.length - 1];
			...
			const topMatchIsPreferredInterpreter =
				await findKernelSpecMatchingInterpreter(preferredInterpreter, [
					potentialMatch,
				]);
			if (cancelToken.isCancellationRequested) {
				return;
			...
			const isExactMatch = await this.kernelRankHelper.isExactMatch(
				uri,
				potentialMatch,
				notebookMetadata
			);
			...
			const languageInNotebookMetadata =
				getLanguageInNotebookMetadata(notebookMetadata);
			const isNonPythonLanguageMatch =
				languageInNotebookMetadata &&
				languageInNotebookMetadata !== PYTHON_LANGUAGE &&
				getKernelConnectionLanguage(potentialMatch) ===
					languageInNotebookMetadata;
			...
			let matchReason: PreferredKernelExactMatchReason =
				PreferredKernelExactMatchReason.NoMatch;
			onlyConnection &&
				(matchReason |= PreferredKernelExactMatchReason.OnlyKernel);
			topMatchIsPreferredInterpreter &&
				(matchReason |=
					PreferredKernelExactMatchReason.WasPreferredInterpreter);
			isExactMatch &&
				(matchReason |= PreferredKernelExactMatchReason.IsExactMatch);
			isNonPythonLanguageMatch &&
				(matchReason |=
					PreferredKernelExactMatchReason.IsNonPythonKernelLanguageMatch);
