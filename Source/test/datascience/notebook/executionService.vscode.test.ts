...
const expectedPromptMessageSuffix = `requires ${ProductNames.get(
	Product.ipykernel
)!} to be installed.`;
...
suite("Kernel Execution @kernelCore", function () {
	...
	testMandatory("Execute cell using VSCode Kernel", async () => {
		...
		assert.strictEqual(
			Buffer.from(cell.outputs[0].items[0].data).toString().trim(),
			"123412341234"
		);
	...
	test("Test __vsc_ipynb_file__ defined in cell using VSCode Kernel", async () => {
		...
		await Promise.all([
			...
			waitForTextOutput(cell, `${uri.fsPath}`),
		]);
	});
	test.skip("Test exceptions have hrefs", async () => {
		const ipythonVersionCell =
			await notebook.appendCodeCell(IPYTHON_VERSION_CODE);
		assert.strictEqual(
			await kernelExecution.executeCell(ipythonVersionCell),
			NotebookCellRunState.Success
		);
		const ipythonVersion = parseInt(
			getTextOutputValue(ipythonVersionCell!.outputs[0])
		);

		const codeCell = await notebook.appendCodeCell(
			'raise Exception("FOO")'
		);
		assert.strictEqual(
			await kernelExecution.executeCell(codeCell),
		...
		const ansiToHtml =
			require("ansi-to-html") as typeof import("ansi-to-html");
	...
	test("Clear output in empty cells", async function () {
		...
		cell.outputs.push(
			new NotebookCellOutput([NotebookCellOutputItem.text("Hello World")])
		);
	...
	test("Verify multiple cells get executed", async () => {
		...
		assert.equal(
			cell2.executionSummary?.executionOrder! - 1,
	...
	test("Verify output & metadata for executed cell with errors", async () => {
		...
		assert.equal(
			...
			"Incorrect evalue"
		); // As status contains ename, we don't want this displayed again.
	...
	test("Updating display data", async function () {
		...
		const displayCell = await notebook.appendCodeCell(
	...
	test("Shell commands should give preference to executables in Python Environment", async function () {
		...
		await Promise.all([
			Promise.all(
				notebook.cells.map((cell) => kernelExecution.executeCell(cell))
			),
		...
		const normalizedInterpreterPath = getNormalizedInterpreterPath(
			Uri.file(sysExecutable)
		).fsPath;
		const normalizedBinPath = path.dirname(normalizedInterpreterPath);
		const pathEquals =
			(getOSType() === OSType.Windows
				? normalizedBinPath.toLowerCase()
				: normalizedBinPath) ===
			(getOSType() === OSType.Windows
	...
	test("!python should point to the Environment", async function () {
		...
		const cell1 = await notebook.appendCodeCell(
			getOSType() === OSType.Windows ? "!where python" : "!which python"
		);
		await notebook.appendCodeCell("import sys");
	...
	test("Verify display updates", async () => {
		...
		const cell2 = await notebook.appendCodeCell(
			'dh = display(Markdown("foo"), display_id=True)'
		);
	...
	test("More messages from background threads", async function () {
		...
		await Promise.all([
			...
			waitForCondition(
				...
				defaultNotebookTestTimeout,
				() =>
					`'iteration 9' not in output => '${getTextOutputValues(
						cell
					)}'`
			),
			waitForCondition(
				...
				defaultNotebookTestTimeout,
				() =>
					`Main thread output not before background output, '${getTextOutputValues(
						cell
					)}'`
			),
			waitForCondition(
				...
				defaultNotebookTestTimeout,
				() =>
					`Main thread not after first background output, '${getTextOutputValues(
						cell
					)}'`
			),
		]);
	...
	test("Messages from background threads can come in other cell output", async function () {
		...
		await Promise.all([
			...
			waitForCondition(
				...
				defaultNotebookTestTimeout,
				() =>
					`'main thread started' not in output => '${getTextOutputValues(
						cell1
					)}'`
			),
			waitForCondition(
	...
	test("Stderr & stdout outputs should go into separate outputs", async function () {
		...
		assert.deepEqual(
			...
			expectedMetadata,
			`Metadata is incorrect for cell 0`
		);
		assert.deepEqual(
			cell.outputs[1].metadata,
			expectedMetadata,
			`Metadata is incorrect for cell 1`
		);
		assert.deepEqual(
			cell.outputs[2].metadata,
			expectedMetadata,
			`Metadata is incorrect for cell 2`
		);
		assert.deepEqual(
			cell.outputs[3].metadata,
			expectedMetadata,
			`Metadata is incorrect for cell 3`
		);

		assert.include(
			getTextOutputValue(cell.outputs[0]),
			"12",
			`Text is incorrect for cell 0`
		);
		assert.include(
			getTextOutputValue(cell.outputs[1]),
			"ab",
			`Text is incorrect for cell 1`
		);
		assert.include(
			getTextOutputValue(cell.outputs[2]),
			"3",
			`Text is incorrect for cell 2`
		);
		assert.include(
			getTextOutputValue(cell.outputs[3]),
			"c",
			`Text is incorrect for cell 3`
		);
	});
	...
	test("Execute all cells and run after error", async () => {
		...
		assert.equal(
			cell2.outputs.length,
			0,
			"Second cell is not empty on run all"
		);
	...
	test("Raw cells should not get executed", async () => {
		...
		assert.isUndefined(
			...
			"Second cell should not have an execution order"
		);
		assert.isUndefined(
			cell2.executionSummary?.timing,
			"Second cell should not have execution times"
		);
		assert.isUndefined(
			cell2.executionSummary?.success,
			"Second cell should not have execution result"
		);
	});
	test("Run whole document and test status of cells", async () => {
		const cells = await insertRandomCells(notebook, {
			count: 4,
			addMarkdownCells: false,
		});
	...
	test("Run cells randomly & validate the order of execution", async () => {
		const cells = await insertRandomCells(notebook, {
			count: 15,
			addMarkdownCells: true,
		});
		const codeCells = cells.filter(
			(cell) => cell.cell.kind === NotebookCellKind.Code
		);
		...
		await Promise.all(
			queuedCells
				.map((item) => item.cell)
				.map((cell) => waitForQueuedForExecutionOrExecuting(cell))
		);
		...
		await Promise.all(
			queuedCells
				.map((item) => item.cell)
				.map((cell) => waitForExecutionCompletedSuccessfully(cell))
		);
	...
	test("Run first 10 cells, fail 5th cell & validate first 4 ran, 5th failed & rest did not run", async () => {
		...
		cells.push(
			...(await insertRandomCells(notebook, {
				count: 5,
				addMarkdownCells: false,
			}))
		);
	...
	test("Run entire notebook then add a new cell, ensure new cell is not executed", async () => {
		...
		await Promise.all(
			queuedCells.map((cell) =>
				waitForQueuedForExecutionOrExecuting(cell)
			)
		);

		// Add a new cell to the document, this should not get executed.
		const [newCell] = await insertRandomCells(notebook, {
			count: 1,
			addMarkdownCells: false,
		});
	...
	test("Cell failures should not get cached", async () => {
		...
		await Promise.all([
			...
			waitForCondition(
				async () =>
					cell2.executionSummary?.executionOrder ===
					cell1.executionSummary!.executionOrder! + 2,
				5_000,
		...
		const lastExecutionOrderOfCell3 =
			cell3.executionSummary?.executionOrder!;
		assert.equal(
			lastExecutionOrderOfCell3,
			cell1.executionSummary!.executionOrder! + 3
		);
	...
	test("Test set_next_input message payload", async () => {
		...
		nbEditStub.callsFake((index, newCells) => {
			newCells.forEach((cell, i) => {
				if (cell.kind === NotebookCellKind.Code) {
					notebook
						.insertCodeCell(index + i, cell.value, cell.languageId)
						.catch(noop);
				} else {
					notebook
						.insertMarkdownCell(index + i, cell.value)
						.catch(noop);
				}
			});
		...
		await waitForCondition(
			async () => {
				expect(cellsPostExecute[1].document.getText()).to.equal(
					"print('INSERT')"
				);
				return true;
			},
		...
		await waitForCondition(
			async () => {
				expect(cellsPostExecute[2].document.getText()).to.equal(
					"print('REPLACE')"
				);
				return true;
			},
	...
	async function insertRandomCells(
		...
		for (let index = startIndex; index < endIndex; index++) {
			...
			if (
				...
				cell = await notebook.appendCodeCell(
					dedent`
                        ...
                        import os.path
                        from os import path
                        while os.path.exists('${tmpFile.replace(
							/\\/g,
							"\\\\"
						)}'):
                            time.sleep(0.1)
				...
				cell = await notebook.appendMarkdownCell(
					`Markdown Cell ${index}`
				);
			}
			...
			cellInfo.push({
				runToCompletion: () =>
					swallowExceptions(() => fs.unlinkSync(tmpFile)),
				cell,
			});
		}
