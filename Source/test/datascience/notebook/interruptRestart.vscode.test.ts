...
suite("Restart/Interrupt/Cancel/Errors @kernelCore", function () {
	...
	async function initSuite() {
		try {
			traceInfo(
				`Start Suite Test Restart/Interrupt/Cancel/Errors @kernelCore`
			);
			...
			traceInfo(
				`Start Suite Test Restart/Interrupt/Cancel/Errors @kernelCore metadata ${metadata.id}`
			);
			...
			traceInfo(
				`Start Suite Test Complete Restart/Interrupt/Cancel/Errors @kernelCore`
			);
		} catch (ex) {
			traceError(
				"Suite Setup (failed) - Test Restart/Interrupt/Cancel/Errors @kernelCore",
				ex
			);
	...
	setup(async function () {
		...
		if (previousTestFailed) {
			traceInfo(
				`Start Running Test Suite again for ${this.currentTest?.title}`
			);
			await closeNotebooksAndCleanUpAfterTests(
				disposables.concat(suiteDisposables)
			);
			await initSuite();
		}
		sinon.restore();
	...
	test("Interrupting kernel with Cancelling token will cancel cell execution", async () => {
		...
		await Promise.all([
			...
			interruptPromise,
			waitForCondition(
				async () => hasErrorOutput(cell.outputs),
				30_000,
				"No errors"
			),
		]);
	...
	test("Restarting kernel will cancel cell execution & we can re-run a cell", async function () {
		...
		const waitForKernelToRestart = createEventHandler(
			kernel,
			"onRestarted",
			disposables
		);
	...
	test("Restarting kernel during run all will skip the rest of the cells", async function () {
		...
		assert.equal(
			lastCell.outputs.length,
			0,
			"Last cell should not have run"
		);
	});
	test("Interrupt and running cells again should only run the necessary cells", async function () {
        ...
        */
		...
		await Promise.all([
			...
			waitForCondition(
				async () => hasErrorOutput(cell2.outputs),
				30_000,
				"Cell 2 does not have any errors"
			),
			waitForCondition(
				async () => assertVSCCellIsNotRunning(cell3),
				15_000,
				"Cell 3 did not get dequeued"
			),
		]);
		console.log("Step4");
		assert.equal(
			...
			1,
			"Execution order of cell 1 is incorrect"
		);
		assert.equal(
			cell2.executionSummary?.executionOrder,
			2,
			"Execution order of cell 2 is incorrect"
		);
		...
		await Promise.all([
			...
			waitForCondition(
				async () =>
					getTextOutputValue(cell2.outputs[0]).trim() != cell2Output,
				30_000,
		...
		assert.equal(
			cell2.executionSummary?.executionOrder,
			3,
			"Execution order of cell 2 should be 3"
		);
		...
		await Promise.all([
			...
			waitForCondition(
				async () =>
					NotebookCellStateTracker.getCellState(cell3) ===
					NotebookCellExecutionState.Idle,
				30_000,
				"Cell 3 is not idle"
			),
	...
	test("Can restart a kernel after it dies", async function () {
        ...
        */
		...
		assert.isAtLeast(
			cell1.executionSummary!.executionOrder!,
			1,
			"Cell 1 should have an execution order of 1"
		);
		assert.strictEqual(
			cell2.executionSummary?.executionOrder,
