...
suite("Remote Kernel Execution", function () {
	...
	test("Local Kernel state is not lost when connecting to remote @kernelPicker", async function () {
		...
		const controllerManager = svcContainer.get<IControllerRegistration>(
			IControllerRegistration
		);
		const preferredController = controllerManager.getSelected(notebook);
		traceInfoIfCI(
			`preferred controller ${preferredController?.connection.id}`
		);
		...
		await waitForCondition(
			async () => {
				...
				traceInfoIfCI(
					`Check ${controllers.length} registered controllers`
				);
				traceInfoIfCI(
		...
		traceInfoIfCI(
			`new preferred controller ${newPreferredController?.connection.id}`
		);

		// Run the second cell and verify we still have the same kernel state.
		await Promise.all([
			runCell(cell2),
			waitForTextOutput(cell2, "123412341234"),
		]);
	});
