...
export async function getServices() {
	...
	return {
		...
		controllerPreferred: ControllerPreferredService.create(
...
async function createTemporaryNotebookFromNotebook(
	...
	const uri = await generateTemporaryFilePath(
		".ipynb",
		disposables,
		rootFolder,
		prefix
...
export async function generateTemporaryFilePath(
	...
	const platformService =
		services.serviceContainer.get<IPlatformService>(IPlatformService);
	const workspaceService =
		services.serviceContainer.get<IWorkspaceService>(IWorkspaceService);
	const rootUrl =
		rootFolder ||
...
export async function createTemporaryNotebookFromFile(
	...
	return createTemporaryNotebookFromNotebook(
		notebook,
		disposables,
		undefined,
		urlPath.basename(file)
	);
}
...
async function shutdownAllNotebooks() {
	...
	const api = await initialize();
	const kernelProvider = api.serviceContainer.get<IKernelProvider>(
		IKernelProvider
	) as IKernelProvider;
	await Promise.all(
		kernelProvider.kernels.map((k) => k.dispose().catch(noop))
	);
}

export async function ensureNewNotebooksHavePythonCells() {
	const api = await initialize();
	const globalMemento = api.serviceContainer.get<Memento>(
		IMemento,
		GLOBAL_MEMENTO
	);
	const lastLanguage = (
		globalMemento.get<string | undefined>(LastSavedNotebookCellLanguage) ||
		PYTHON_LANGUAGE
	).toLowerCase();
	if (lastLanguage !== PYTHON_LANGUAGE.toLowerCase()) {
		await globalMemento
			.update(LastSavedNotebookCellLanguage, PYTHON_LANGUAGE)
			.then(noop, noop);
	}
}
async function shutdownRemoteKernels() {
	const api = await initialize();
	const serverUriStorage = api.serviceContainer.get<IJupyterServerUriStorage>(
		IJupyterServerUriStorage
	);
	const jupyterConnection =
	...
	try {
		const connection = await jupyterConnection.createConnectionInfo(
			(await serverUriStorage.getAll())[0].provider
		);
		...
		await Promise.all(
			liveKernels
				.filter((item) => item.id)
				.map((item) => KernelAPI.shutdownKernel(item.id!).catch(noop))
		);
	} catch {
		// ignore
	} finally {
...
async function shutdownKernels() {
	...
	const thirdPartyKernelProvider =
		api.serviceContainer.get<IThirdPartyKernelProvider>(
			IThirdPartyKernelProvider
		);
	await Promise.all(
		thirdPartyKernelProvider.kernels.map((k) => k.dispose().catch(noop))
	);
	await Promise.all(
		MockNotebookDocuments.map(
			(nb) => kernelProvider.get(nb)?.dispose().catch(noop)
		)
	);
...
export async function closeNotebooksAndCleanUpAfterTests(
	...
	if (!IS_SMOKE_TEST()) {
		...
		const configSettings = await import(
			"../../../platform/common/configSettings"
		);
...
export async function closeNotebooks(disposables: IDisposable[] = []) {
	...
	const api = await initialize();
	VSCodeNotebookController.kernelAssociatedWithDocument = undefined;
	const notebooks = api.serviceManager.get<IVSCodeNotebook>(
		IVSCodeNotebook
	) as VSCodeNotebook;
...
export async function waitForKernelToChange(
	searchCriteria:
		| (() => Promise<{
				labelOrId: string;
				isInteractiveController?: boolean;
		  }>)
	...
	waitForKernelPendingPromise = waitForKernelToChangeImpl(
		searchCriteria,
		notebookEditor,
		timeout,
		skipAutoSelection
...
async function waitForKernelToChangeImpl(
	...
	const isRightKernel = async () => {
		const criteria =
			typeof searchCriteria === "function"
				? await searchCriteria()
				: searchCriteria;
		let labelOrId =
			"labelOrId" in criteria ? criteria.labelOrId : undefined;
		if (labelOrId) {
			controller = controllerRegistration.registered
				.filter((k) =>
					criteria.isInteractiveController
						? k.id.includes(InteractiveControllerIdSuffix)
						: true
				)
				.find(
					(k) =>
						(labelOrId && k.label === labelOrId) ||
						(k.id && k.id == labelOrId)
				);
			if (!controller) {
				...
				controller = controllerRegistration.registered.find(
					(k) =>
						(labelOrId && k.label.includes(labelOrId)) ||
						(k.id && k.id == labelOrId)
				);
			}
		}
		const interpreterPath =
			"interpreterPath" in criteria
				? criteria.interpreterPath
				: undefined;
		if (interpreterPath && !controller) {
			controller = controllerRegistration.registered
				...
				.find((k) =>
					...
					k.connection
						.interpreter!.uri.fsPath.toLowerCase()
						.includes(interpreterPath.fsPath.toLowerCase())
				);
			if (controller) {
				// eslint-disable-next-line local-rules/dont-use-fspath
				traceVerbose(
					`Did match a controller that matches the interpreter ${interpreterPath.fsPath}`
				);
			} else {
				// eslint-disable-next-line local-rules/dont-use-fspath
				traceWarning(
					`Did not find a controller that matches the interpreter ${interpreterPath.fsPath}`
				);
			}
		}
		traceInfo(
		...
		const selectedController = controllerRegistration.getSelected(
			editor.notebook
		);
		...
		if (selectedController.id === controller?.id) {
			traceInfo(
				`Found selected kernel id:label ${selectedController.id}:${selectedController.label}`
			);
			return true;
		}
		traceInfo(
			`Active kernel is id:label = ${selectedController.id}:${selectedController.label}`
		);
		return false;
	};
		...
		await waitForCondition(
			async () => {
				...
				if (!(await isRightKernel()) && !skipAutoSelection) {
					...
					traceInfoIfCI(
						`Notebook select.kernel command switching to kernel id ${controller
							?.connection
							.kind}${controller?.id}: Try ${tryCount} for ${JSON.stringify(
							criteria
						)}`
					);
...
async function waitForActiveNotebookEditor(
	notebookEditor?: NotebookEditor
...
async function getActiveInterpreterKernelConnection() {
	...
	return waitForCondition(
        ...
        ${interpreter?.id} (${getDisplayPath(interpreter?.uri)}) for kernels (${
			...
			.map(
				(item) =>
					`${item.id}=> ${item.kind} (${getDisplayPath(
						item.interpreter?.uri
					)})`
			)
			.join(", ")}`,
...
export function selectDefaultController(
	...
	return IS_REMOTE_NATIVE_TEST() || isWeb()
		? selectPythonRemoteKernelConnectionForActiveInterpreter(
				notebookEditor,
				timeout
		  )
		: selectActiveInterpreterController(notebookEditor, timeout);
}
async function selectActiveInterpreterController(
	notebookEditor: NotebookEditor,
	timeout = defaultNotebookTestTimeout
) {
		...
		() =>
			controllerRegistration.registered.find(
				(k) =>
					...
					(
						k.connection.kernelSpec.language || PYTHON_LANGUAGE
					).toLowerCase() === PYTHON_LANGUAGE.toLowerCase() &&
					areInterpreterPathsSame(
						k.connection.interpreter.uri,
						interpreter?.uri
					)
			),
		timeout,
	...
	await waitForCondition(
		() =>
			controllerRegistration.getSelected(notebookEditor.notebook)?.id ===
				controller.id &&
			controllerRegistration.getSelected(notebookEditor.notebook)
				?.viewType === notebookEditor.notebook.notebookType,
		timeout,
		`Controller ${
			controller.id
...
export async function waitForKernelToGetAutoSelected(
	...
	await waitForCondition(
		async () => {
			...
			await waitForCondition(
				async () =>
					controllerRegistration.all.length >
					lastLoadedControllerCount,
				1000,
				() =>
...
export async function waitForKernelToGetAutoSelectedImpl(
	...
	const searchCriteria = async () => {
		...
		const preferredKind = useRemoteKernelSpec
			? "startUsingRemoteKernelSpec"
			: preferred?.connection.kind;
		let match: IVSCodeNotebookController | undefined;
		if (preferred) {
			if (
				preferred.connection.kind !== "connectToLiveRemoteKernel" &&
				(!expectedLanguage ||
					preferred.connection.kernelSpec?.language?.toLowerCase() ===
						expectedLower) &&
		...
		if (!match) {
			traceInfoIfCI(
				`Manually pick a preferred kernel from all kernel specs`
			);
			const matches = notebookControllers.filter(
				(d) =>
					d.connection.kind != "connectToLiveRemoteKernel" &&
					language ===
						d.connection.kernelSpec?.language?.toLowerCase() &&
					(!useRemoteKernelSpec ||
						d.connection.kind.includes("Remote"))
			);
				...
				`Matches: ${matches
					.map(
						(m) =>
							m.connection.kind +
							", " +
							m.connection.interpreter?.uri.path
					)
					.join("\n ")}`
			);
		...
		traceInfo(
			`Preferred kernel for selection is ${
				match.connection.kind
			}:${match?.id}, criteria = ${JSON.stringify(criteria)}`
		);
		assert.ok(match, "No kernel to auto select");
		return { labelOrId: match!.id };
	};
	return waitForKernelToChange(
		searchCriteria,
		notebookEditor!,
		timeout,
		skipAutoSelection
...
export async function waitForCellExecutionState(
	...
	try {
		await waitForCondition(
			...
			timeout,
			`Execution state did not change to ${state}`
		);
	} finally {
		disposable.dispose();
	}
	...
	return waitForCondition(
		async () => {
			...
			return (
				...
				!cellOutputs.includes("No cell outputs")
			);
		},
		defaultNotebookTestTimeout,
		"No output"
 ...
 */
export async function waitForExecutionInProgress(
	cell: NotebookCell,
	timeout: number = defaultNotebookTestTimeout
) {
	await waitForCondition(
 ...
 */
export async function waitForQueuedForExecution(
	cell: NotebookCell,
	timeout: number = defaultNotebookTestTimeout
) {
	await waitForCondition(
		...
		() =>
			`Cell ${
				cell.index + 1
			} not queued for execution, current state is ${NotebookCellStateTracker.getCellStatus(
				cell
			)}`
	);
}
...
export async function waitForExecutionCompletedWithoutChangesToExecutionCount(
	...
	await waitForCondition(
		...
		() =>
			`Cell ${
				cell.index + 1
			} did not complete, State = ${NotebookCellStateTracker.getCellStatus(
				cell
			)}`
	);
}
	...
	await waitForCondition(
		async () =>
			assertHasExecutionCompletedWithErrors(
				cell,
				executionOderShouldChange
			),
		timeout,
		() =>
			`Cell ${
				cell.index + 1
			} did not fail as expected, State =  ${NotebookCellStateTracker.getCellStatus(
				cell
			)}`
	);
...
function assertHasExecutionCompletedWithErrors(
	cell: NotebookCell,
	executionOderShouldChange = true
) {
	return (
		(executionOderShouldChange
			? (cell.executionSummary?.executionOrder ?? 0) > 0
...
export function assertHasTextOutputInVSCode(
	...
	assert.isTrue(
		result,
		`${text} not found in outputs of cell ${cell.index} ${getCellOutputs(
			cell
		)}`
	);
	return result;
...
export function assertNotHasTextOutputInVSCode(
	cell: NotebookCell,
	text: string,
	index: number,
	isExactMatch = true
	...
	if (isExactMatch) {
		...
		expect(outputText).to.not.include(
			text,
			"Output does not contain provided text"
		);
	}
	return true;
}
...
export async function runCell(
	...
	const notebookEditor = vscodeNotebook.notebookEditors.find(
		(e) => e.notebook === cell.notebook
	);
...
export async function hijackPrompt(
	...
	const stub = sinon.stub(appShell, promptType).callsFake(function (
		msg: string
	) {
		traceInfo(
			`Message displayed to user '${msg}', condition ${JSON.stringify(
				message
			)}`
		);
		if (
			("exactMatch" in message &&
				msg.trim() === message.exactMatch.trim()) ||
			("contains" in message &&
				msg.trim().includes(message.contains.trim())) ||
			...
			if (buttonToClick) {
				if (
					!buttonToClick.dismissPrompt &&
					buttonToClick?.clickImmediately === true &&
					buttonToClick.result
				) {
				...
				return buttonToClick.dismissPrompt
					? Promise.resolve(undefined)
					: clickButton.promise;
			}
		}
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		return (appShell[promptType] as any).wrappedMethod.apply(
			appShell,
			arguments
		);
	} as any);
	...
	return {
		...
		clickButton: (text?: string) =>
			clickButton.resolve(text || buttonToClick?.result),
	};
}
...
export async function hijackSavePrompt(
	...
	// eslint-disable-next-line
	const stub = sinon
		.stub(appShell, "showSaveDialog")
		.callsFake(function (msg: { saveLabel: string }) {
			traceInfo(
				`Message displayed to user '${JSON.stringify(
					msg
				)}', checking for '${saveLabel}'`
			);
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			return (appShell.showSaveDialog as any).wrappedMethod.apply(
				appShell,
				arguments
			);
		} as any);
...
export class MockQuickPick implements QuickPick<QuickPickItem> {
	...
	get onDidTriggerItemButton(): Event<
	...
	private _onDidTriggerItemButtonEmitter = new EventEmitter<
		QuickPickItemButtonEvent<QuickPickItem>
	>();
	private _onDidChangeActiveEmitter = new EventEmitter<
		readonly QuickPickItem[]
	>();
	private _onDidChangeSelectionEmitter = new EventEmitter<
		readonly QuickPickItem[]
	>();
...
export async function hijackCreateQuickPick(
	disposables: IDisposable[] = []
): Promise<QuickPickStub> {
	const api = await initialize();
	const appShell =
		api.serviceContainer.get<IApplicationShell>(IApplicationShell);
...
export async function getDebugSessionAndAdapter(
	...
	await waitForCondition(
		async () =>
			...
			(!prevSessionId ||
				prevSessionId !== debuggingManager.getDebugSession(doc)?.id),
		defaultNotebookTestTimeout,
	...
	assert.isOk<IKernelDebugAdapter | undefined>(
		debugAdapter,
		"DebugAdapter not started"
	);
...
export async function clickOKForRestartPrompt() {
	...
	const showInformationMessage = sinon
		...
		.callsFake(function (message: string) {
			...
			return (appShell.showInformationMessage as any).wrappedMethod.apply(
				appShell,
				arguments
			);
		});
