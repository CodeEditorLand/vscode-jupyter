...
suite("VSCode Notebook -", function () {
	...
	setup(async function () {
		try {
			...
			diagnosticProvider = api.serviceContainer
				.getAll<NotebookCellBangInstallDiagnosticsProvider>(
	...
	test("Show error for pip install", async () => {
		...
		await waitForCondition(
			async () =>
				(diagnosticProvider.problems.get(cell.document.uri) || [])
					.length > 0,
			5_000,
		...
		assert.equal(
			problem.message,
			DataScience.percentPipCondaInstallInsteadOfBang("pip")
		);
		assert.isTrue(
	...
	test("Show error for conda install", async () => {
		...
		assert.equal(
			problem.message,
			DataScience.percentPipCondaInstallInsteadOfBang("conda")
		);
		assert.isTrue(
