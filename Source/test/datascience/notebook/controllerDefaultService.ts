...
export class ControllerDefaultService {
	...
	public async computeDefaultController(
		...
		if (!IS_REMOTE_NATIVE_TEST() && this.interpreters) {
			...
			return createActiveInterpreterController(
				viewType,
				resource,
				this.interpreters,
				this.registration
			...
			const notebook =
				viewType === JupyterNotebookView
					? this.notebook.notebookDocuments.find((item) =>
							isEqual(item.uri, resource, true)
					  )
					: undefined;
			const controller = await this.createDefaultRemoteController(
				viewType,
				notebook
			);
			...
			throw new Error(
				"No default remote controller, hence returning the active interpreter"
			);
		}
	}
	...
	private async createDefaultRemoteController(
		...
		const defaultController =
			...
			defaultPythonKernel ||
			defaultPythonLanguageKernel;

		if (language === PYTHON_LANGUAGE) {
			return defaultController;
		} else {
			let matchingKernelNameController:
				| IVSCodeNotebookController
				| undefined;
			let matchingKernelLanguageController:
				| IVSCodeNotebookController
				| undefined;
			controllers.forEach((item) => {
				// Sort out interactive or non-interactive controllers
			...
			return (
				matchingKernelNameController ||
				matchingKernelLanguageController ||
				defaultController
			);
		}
	}
