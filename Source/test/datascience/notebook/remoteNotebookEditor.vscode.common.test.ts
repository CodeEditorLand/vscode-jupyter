...
suite("Remote Execution @kernelCore", function () {
	...
	suiteSetup(async function () {
		...
		controllerDefault = ControllerDefaultService.create(
			api.serviceContainer
		);
		storage = api.serviceContainer.get<IJupyterServerUriStorage>(
	...
	test("MRU and encrypted storage should be updated with remote Uri info", async function () {
		...
		await waitForCondition(
			...
			5_000,
			() =>
				`MRU not updated, ${JSON.stringify(
					previousList
				)} === ${JSON.stringify(newList)}`
		);
	});
	test("Use same kernel when re-opening notebook", async function () {
		await reopeningNotebookUsesSameRemoteKernel(
			ipynbFile,
			serviceContainer
		);
	});
	...
	test("Can run against a remote kernelspec", async function () {
		await waitForCondition(
			() =>
				controllerRegistration.registered.some(
					(item) =>
		...
		const defaultPythonKernel =
			await controllerDefault.computeDefaultController(
				undefined,
				"jupyter-notebook"
			);
		...
		await Promise.all([
			runCell(cell),
			waitForTextOutput(cell, "123412341234"),
		]);
	});
...
export async function runCellAndVerifyUpdateOfPreferredRemoteKernelId(
	...
	await waitForCondition(
		async () =>
			!!(await remoteKernelIdProvider.getPreferredRemoteKernelId(
...
export async function reopeningNotebookUsesSameRemoteKernel(
	...
	await runCellAndVerifyUpdateOfPreferredRemoteKernelId(
		ipynbFile,
		serviceContainer
	);
	let nbEditor = window.activeNotebookEditor!;
	...
	await waitForCondition(
		async () =>
			!nbEditor.notebook
				.getCells()
				.some((cell) => cell.outputs.length > 0),
		5_000,
