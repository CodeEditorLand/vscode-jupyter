...
export function createKernelController(controllerId = "1"): IKernelController {
	return {
		...
		createNotebookCellExecution: (cell: NotebookCell) =>
			new TestNotebookCellExecution(cell),
		id: controllerId,
	};
}
class TestNotebookCellExecution implements NotebookCellExecution {
	...
	start(startTime?: number): void {
		...
		(this.cell as ReadWrite<NotebookCell>).executionSummary = {
			...
			executionOrder:
				this.executionOrder ||
				this.cell.executionSummary?.executionOrder,
			timing:
	...
	async replaceOutput(
		...
		if (cellToEdit.outputs.length) {
			(cellToEdit.outputs as NotebookCellOutput[]).splice(
				0,
				cellToEdit.outputs.length
			);
		}
...
export class TestNotebookDocument implements NotebookDocument {
	...
	constructor(
		...
		public readonly notebookType:
			| typeof JupyterNotebookView
			| typeof InteractiveWindowView = JupyterNotebookView,
	...
	static async openFile(uri: Uri): Promise<TestNotebookDocument> {
		...
		const nb = new TestNotebookDocument(
			uri,
			JupyterNotebookView,
			notebook.metadata as any,
			false
		);
		await Promise.all(
			notebook.getCells().map((cell) => {
				if (cell.kind === NotebookCellKind.Code) {
					return nb.appendCodeCell(
						cell.document.getText(),
						cell.document.languageId,
						cell.metadata
					);
				} else {
					return nb.appendMarkdownCell(
						cell.document.getText(),
						cell.metadata
					);
				}
	...
	public async appendCodeCell(
		...
		// eslint-disable-next-line @typescript-eslint/no-use-before-define
		const cell = new TestNotebookCell(
			this,
			textDoc,
	...
	getCells(range?: NotebookRange | undefined): TestNotebookCell[] {
		return range && !range.isEmpty
			? this.cells.filter(
					(_, index) => range.start >= index && range.end <= index
			  )
			: this.cells;
	}
