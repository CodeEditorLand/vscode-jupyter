...
suite(`Interactive window Execution @iw`, async function () {
	...
	async function preSwitch() {
		...
		await waitForCondition(
			() => {
				const venvNoKernelInterpreter = interpreters.find((i) =>
					getFilePath(i.uri).includes(".venvnokernel")
				);
				const venvKernelInterpreter = interpreters.find((i) =>
					getFilePath(i.uri).includes(".venvkernel")
				);
				return venvNoKernelInterpreter && venvKernelInterpreter
					? true
					: false;
			},
			defaultNotebookTestTimeout,
		...
		assert.ok(
			originalActiveInterpreter,
			`No active interpreter when running switch test`
		);
	}
	...
	test("Export Interactive window to Notebook", async () => {
		...
		let notebookFile = await generateTemporaryFilePath(
			"ipynb",
			disposables
		);
		...
		let savePrompt = await hijackSavePrompt(
			"Export",
			promptOptions,
			disposables
		);
		...
		await vscode.commands.executeCommand(
			...
			activeInteractiveWindow.notebookUri
		);

		await waitForCondition(
			() => savePrompt.displayed,
			defaultNotebookTestTimeout,
			"save Prompt not displayed"
		);
		await waitForCondition(
		...
		let editor = await vscodeNotebook.showNotebookDocument(document, {
			preserveFocus: false,
		});
	...
	test("Switching active interpreter on a python file changes kernel in use", async function () {
		...
		try {
			...
			let controller = notebookDocument
				? notebookControllerManager.getSelected(notebookDocument)
				: undefined;
			assert.ok(
			...
			const newIW = await runCurrentFile(
				interactiveWindowProvider,
				untitledPythonFile
			);
