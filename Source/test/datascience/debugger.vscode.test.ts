...
suite("Run By Line @debugger", function () {
	...
	suiteSetup(async function () {
		...
		try {
			...
			debuggingManager = api.serviceContainer.get<IDebuggingManager>(
				INotebookDebuggingManager
			);
			vscodeNotebook =
	...
	setup(async function () {
		...
		const controllerRegistry =
			await api.serviceContainer.get<IControllerRegistration>(
		...
		const controller = await waitForCondition(
			() =>
				controllerRegistry.registered.find(
					(item) =>
						item.viewType === "jupyter-notebook" &&
						item.connection.id === metadata.id
				),
			defaultNotebookTestTimeout,
	...
	test("Delete temp debugging files @mandatory", async function () {
		...
		try {
			...
			const { debugAdapter } = await getDebugSessionAndAdapter(
				debuggingManager,
				doc
			);
			...
			assert.isDefined(
				folderName,
				"Failed to create an ipykernel debug temp folder"
			);
			if (folderName) {
				...
				assert.isEmpty(
					tempFiles,
					"Failed to delete temp debugging files"
				);
			}
		} finally {
	...
	test.skip("Stops at end of cell", async function () {
		...
		await commandManager.executeCommand(Commands.RunByLine, cell);
		traceInfo(`Executed run by line`);
		const { debugAdapter, session } = await getDebugSessionAndAdapter(
			debuggingManager,
			doc
		);
		...
		const expectedVariables = [
			{ name: "a", type: "int", length: "", value: "1" },
		];
	...
	test("Interrupt during debugging", async function () {
		...
		await commandManager.executeCommand(Commands.InterruptKernel, {
	...
	test("Stops in same-cell function called from last line", async function () {
		...
		const stack: DebugProtocol.StackTraceResponse["body"] =
	...
	test.skip("Restart while debugging", async function () {
		...
		const { debugAdapter: debugAdapter2, session: session2 } =
			await getDebugSessionAndAdapter(debuggingManager, doc, session.id);
		const stoppedEvent = await waitForStoppedEvent(debugAdapter2!); // First line
		const stack: DebugProtocol.StackTraceResponse["body"] =
			await session2!.customRequest("stackTrace", {
				threadId: stoppedEvent.body.threadId,
			});
		assert.isTrue(stack.stackFrames.length > 0, "has frames");
		assert.equal(
	...
	test.skip("Run a second time after interrupt", async function () {
		...
		commandManager
			.executeCommand(Commands.RunByLine, cell)
			.then(noop, noop);
		const { debugAdapter } = await getDebugSessionAndAdapter(
			debuggingManager,
			doc
		...
		assert.isFalse(
			getCellOutputs(cell).includes("final output"),
			`Final line did run even with an interrupt`
		);
		...
		commandManager
			.executeCommand(Commands.RunByLine, cell)
			.then(noop, noop);
		const { debugAdapter: debugAdapter2 } = await getDebugSessionAndAdapter(
			debuggingManager,
			doc
		);
		...
		await waitForCondition(
			...
			defaultNotebookTestTimeout,
			`Print during time loop is not working. Outputs: ${getCellOutputs(
				cell
			)}}`,
			1000
		);
