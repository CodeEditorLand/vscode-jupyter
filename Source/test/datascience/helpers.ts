...
export async function createStandaloneInteractiveWindow(
	interactiveWindowProvider: InteractiveWindowProvider
) {
	const activeInteractiveWindow =
		(await interactiveWindowProvider.getOrCreate(
			undefined
...
export async function insertIntoInputEditor(
	source: string,
	...
	if (interactiveWindow) {
		...
		if (!inputBox) {
			traceError(
				`couldn't find input box ${
					interactiveWindow.inputUri.path
				} in visible text editors ${JSON.stringify(
					vscode.window.visibleTextEditors.map(
						(e) => e.document.uri.path
					)
				)}`
			);
		}
...
export async function setActiveInterpreter(
	...
	if (interpreter) {
		const [pythonApi, api] = await Promise.all([
			apiProvider.getNewApi(),
			initialize(),
		]);
...
export async function submitFromPythonFileUsingCodeWatcher(
	...
	const api = await initialize();
	const interactiveWindowProvider =
		api.serviceManager.get<IInteractiveWindowProvider>(
			IInteractiveWindowProvider
		);
	const codeWatcherProvider =
		api.serviceManager.get<IDataScienceCodeLensProvider>(
...
export async function closeInteractiveWindow(
...
export async function waitForLastCellToComplete(
		...
		async () => {
			...
			codeCell =
				codeCells && codeCells.length
					...
					: undefined;
			return codeCell &&
				(numberOfCells === -1 || numberOfCells === codeCells!.length)
				? true
				: false;
		},
		defaultNotebookTestTimeout,
	...
	traceInfoIfCI(
		`finished waiting for last cell to complete of ${codeCells.length} cells`
	);
	return codeCell!;
}
...
export async function waitForCodeLenses(document: vscode.Uri, command: string) {
	...
	let editor = vscode.window.visibleTextEditors.find((e) =>
		arePathsSame(e.document.uri.fsPath, document.fsPath)
	);
	if (editor) {
		await vscode.window
	...
	await waitForCondition(
		async () => {
			...
			return (
				codeLenses &&
				codeLenses.length > 0 &&
				codeLenses.find((c) => c.command?.command === command) !=
					undefined
			);
		},
		defaultNotebookTestTimeout,
...
export async function verifySelectedControllerIsRemoteForRemoteTests(
...
export function uriEquals(expected: string | vscode.Uri) {
	class UriMatcher extends Matcher {
		...
		override match(value: vscode.Uri): boolean {
			return arePathsSame(
				getFilePath(value),
				getFilePath(this.expectedUri)
			);
		}
	}
	return new UriMatcher(
		typeof expected === "string" ? vscode.Uri.file(expected) : expected
