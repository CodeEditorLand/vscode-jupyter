...
export class JupyterServer {
	...
	public async dispose() {
		...
		if (this.availablePort) {
			await tcpPortUsed
				.waitUntilFree(this.availablePort, 200, 5_000)
				.catch(noop);
		}
		if (this.availableSecondPort) {
			await tcpPortUsed
				.waitUntilFree(this.availableSecondPort, 200, 5_000)
				.catch(noop);
		}
	}
	...
	public async startJupyterWithCert(detached?: boolean): Promise<string> {
		if (!this._jupyterServerWithCert) {
			this._jupyterServerWithCert = new Promise<string>(
	...
	public async startJupyter(options: {
		...
		const token =
			typeof options.token === "string"
				? options.token
				: this.generateToken();
		const result = await this.startJupyterServer({
			...options,
			port,
			token,
		});
	...
	public async startJupyterWithToken({
		detached,
	}: { detached?: boolean } = {}): Promise<string> {
		const token = this.generateToken();
		if (!this._jupyterServerWithToken) {
			this._jupyterServerWithToken = new Promise<string>(
				async (resolve, reject) => {
	...
	public async startSecondJupyterWithToken(
		token = this.generateToken()
	): Promise<string> {
		if (!this._secondJupyterServerWithToken) {
			this._secondJupyterServerWithToken = new Promise<string>(
				async (resolve, reject) => {
	...
	private async getSecondFreePort() {
		...
		// The remote uri is cached in a few places (known issue).
		if (!this.availableSecondPort) {
			this.availableSecondPort = await getPort({
				host: "localhost",
				port: this.nextPort,
			});
		}
	...
	private startJupyterServer({
		...
		return new Promise<{ url: string; dispose: () => void }>(
			async (resolve, reject) => {
				try {
					const args = [
						"-m",
					...
					if (typeof password === "string") {
						if (password.length === 0) {
							...
							args.push(
								`--NotebookApp.password=${generateHashedPassword(
									password
								)}`
							);
							args.push(
								`--ServerApp.password=${generateHashedPassword(
									password
								)}`
							);
						}
					...
					console.log(
						`Starting Jupyter in ${getPythonPath()} with ${args.join(
							" "
						)}`
					);
					...
					const subscription = result.out.subscribe((output) => {
						...
						if (
							...
							const lines = splitLines(allOutput, {
								trim: true,
							...
							let url = "";
							if (lineWithUrl) {
								url = lineWithUrl.substring(
									lineWithUrl.indexOf("http")
								);
							} else {
								url = `http${
									useCert ? "s" : ""
							...
							resolve({
								url,
								dispose: () => procDisposable.dispose(),
							});
						}
					});
