	...
	return new Promise<NonNullable<T>>(async (resolve, reject) => {
		...
		const timerFunc = async () => {
			if (cancelToken?.isCancellationRequested) {
				dispose(disposables);
				reject(
					new Error("Cancelled Wait Condition via cancellation token")
				);
				return;
			}
		...
		if (cancelToken) {
			cancelToken.onCancellationRequested(
				() => {
					...
					reject(
						new Error(
							"Cancelled Wait Condition via cancellation token"
						)
					);
					return;
				},
...
export async function generateScreenShotFileName(
	...
	const fullTestNameHash =
		typeof contextOrFileName === "string"
			...
			: (
					await computeHash(
						contextOrFileName.currentTest?.fullTitle() || "",
						"SHA-256"
					)
...
export function suiteMandatory(
	title: string,
	fn: (this: Suite) => void
): Suite {
