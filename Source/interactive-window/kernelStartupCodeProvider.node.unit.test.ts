...
suite("KernelWorkingFolder", () => {
	...
	setup(() => {
		...
		when(kernel.resourceUri).thenReturn(
			Uri.from({
				scheme: Schemas.vscodeNotebook,
				path: "/notebook.ipynb",
			}),
		);
	});
	...
	] = ["startUsingLocalKernelSpec", "startUsingPythonInterpreter"];
