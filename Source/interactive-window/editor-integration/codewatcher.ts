...
export class CodeWatcher implements ICodeWatcher {
	...
	public async debugCurrentCell() {
		...
		return this.runMatchingCell(
			this.documentManager.activeTextEditor.selection,
			false,
			true
		);
	}
	...
	public async runSelectionOrLine(
		...
		if (
			...
			const normalize =
				this.configService.getSettings(activeEditor.document.uri)
					.normalizeSelectionForInteractiveWindow ?? true;
