...
export class CodeGenerator implements IInteractiveWindowCodeGenerator {
	...
	public extractExecutableLines(
		...
		if (
			lines.length > 0 &&
			(cellMatcher.isCode(lines[0]) || cellMatcher.isMarkdown(lines[0]))
		) {
	...
	private async generateCodeImpl(
		...
		const doc = this.documentManager.textDocuments.find(
	...
	private handleContentChange(
		...
		const endChangedOffset =
			c.rangeLength <= 0
				? c.rangeOffset
				: c.rangeOffset + c.rangeLength - 1;
