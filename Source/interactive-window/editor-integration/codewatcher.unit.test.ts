...
suite("Code Watcher Unit Tests", () => {
	...
	setup(() => {
		...
		interactiveWindowProvider =
			TypeMoq.Mock.ofType<IInteractiveWindowProvider>();
		...
		when(notebook.onDidChangeNotebookCellExecutionState).thenReturn(
		...
		dataScienceErrorHandler =
			TypeMoq.Mock.ofType<IDataScienceErrorHandler>();
...
# fifth line`;
		jupyterSettings.codeRegularExpression =
			"(#\\s*\\<foobar\\>|#\\s*\\<baz\\>)";
		jupyterSettings.markdownRegularExpression =
...
testing2`;
		...
		helper
			...
			.returns(() => "testing2");
		helper
			.setup((h) => h.normalizeLines(TypeMoq.It.isAny()))
			.returns((x: string) => Promise.resolve(x));
