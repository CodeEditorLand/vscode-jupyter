...
export class InteractiveWindowController {
	...
	private async createKernel(): Promise<IKernel> {
		...
		try {
			...
			const { kernel, actualController } =
				await this.controllerService.createKernel(
					this.metadata,
					this.controller,
					this.owner,
	...
	private async setFileInKernel(kernel: IKernel): Promise<void> {
		...
		if (this.mode === "perFile" && !this.fileInKernel) {
			...
			(!this.fileInKernel ||
				this.fileInKernel.toString() !== file.toString()) &&
			this.mode !== "perFile"
		) {
			logger.debug(
				`Initializing __file__ in setFileInKernel with ${file} for mode ${this.mode}`,
			...
			(e: {
				...
				if (
					...
					e.notebook.uri.toString() !==
						this.interactiveWindow.notebookDocument?.uri?.toString()
				) {
					return;
				}
