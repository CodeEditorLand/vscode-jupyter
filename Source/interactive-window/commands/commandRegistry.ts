	...
	implements IDisposable, IExtensionSyncActivationService
{
	...
	constructor(
		...
		@inject(INotebookExporter)
		@optional()
		private jupyterExporter: INotebookExporter | undefined,
		...
		@optional()
		private dataScienceCodeLensProvider:
			| IDataScienceCodeLensProvider
			| undefined,
		...
		protected ipynbProvider: INotebookEditorProvider,
	...
	public activate() {
		...
		this.registerCommand(
			Commands.EnableDebugLogging,
			this.enableDebugLogging
		);
		this.registerCommand(
			Commands.ResetLoggingLevel,
			this.resetLoggingLevel
		);
		this.registerCommand(
	...
	private getCodeWatcher(file: Uri | undefined): ICodeWatcher | undefined {
		if (file && this.dataScienceCodeLensProvider) {
			...
			if (possibleDocuments && possibleDocuments.length === 1) {
				...
				throw new Error(
					DataScience.documentMismatch(getFilePath(file))
				);
			}
		}
	...
	private async runAllCellsAbove(
		file: Uri,
		stopLine: number,
	...
	private async runCellAndAllBelow(
		file: Uri | undefined,
		startLine: number,
	...
	private async runSelectionOrLine(
		textOrUri: string | undefined | Uri
	): Promise<void> {
		const activeCodeWatcher = this.getCurrentCodeWatcher();
		if (activeCodeWatcher) {
	...
	private async debugStepOver(): Promise<void> {
		...
		if (this.debugService?.activeDebugSession) {
			this.commandManager
				.executeCommand("workbench.action.debug.stepOver")
				.then(noop, noop);
		}
	}
	...
	private async debugStop(uri: Uri): Promise<void> {
		...
		if (
			...
			if (iw && iw.notebookDocument) {
				...
				if (kernel) {
					traceVerbose(
						`Interrupt kernel due to debug stop of IW ${uri.toString()}`
					);
	...
	private async debugContinue(): Promise<void> {
		...
		if (this.debugService?.activeDebugSession) {
			this.commandManager
				.executeCommand("workbench.action.debug.continue")
				.then(noop, noop);
		}
	}
		...
		if (filePath && filePath.length > 0 && this.jupyterExporter) {
			...
			if (
				...
				if (cells) {
					...
					const uri = await this.exportDialog.showDialog(
						ExportFormat.ipynb,
						file
					);
					await this.waitForStatus(
						async () => {
							if (uri) {
								const notebook =
									await this.jupyterExporter?.translateToNotebook(
										cells
									);
								await this.fileSystem.writeFile(
									uri,
									JSON.stringify(notebook, undefined, 1)
								);
							}
						},
			...
			if (
				...
				if (cells) {
					...
					await this.notebook
						...
						.then((document) =>
							this.notebook.showNotebookDocument(document)
						);
	...
	private async createNewInteractiveWindow(
		connection?: KernelConnectionMetadata
	): Promise<void> {
		await this.interactiveWindowProvider?.getOrCreate(
			undefined,
			connection
		);
	}
	...
	private async scrollToCell(file: Uri, id: string): Promise<void> {
		if (id && file) {
			...
			const possibles =
				this.interactiveWindowProvider.getInteractiveWindowsWithSubmitter(
					file
				);
	...
	private async clearAllCellsInInteractiveWindow(context?: {
	...
	private async goToCodeInInteractiveWindow(context?: NotebookCell) {
		if (context && context.metadata?.interactive) {
			...
			const editor = await this.documentManager.showTextDocument(uri, {
	...
	private async copyCellInInteractiveWindow(context?: NotebookCell) {
		if (context) {
			...
			const source = [
				...
				context.metadata.interactiveWindowCellMarker ??
