	...
	implements IConditionalJupyterVariables, DebugAdapterTracker
{
	...
	public get active(): boolean {
		return (
			(this.debugService.activeDebugSession !== undefined ||
	...
	public async getMatchingVariable(
		...
		if (this.active) {
			...
			if (
				result &&
				kernel?.resourceUri &&
				uriPath.extname(kernel?.resourceUri).toLowerCase() === ".ipynb"
			) {
	...
	public async getDataFrameInfo(
		...
		const { cleanupCode, initializeCode, code } =
			await this.dfScriptGenerator.generateCodeToGetDataFrameInfo({
				isDebugging: true,
	...
	public async getDataFrameRows(
		...
		if (
			...
			targetVariable.columns === undefined
		) {
		...
		const { cleanupCode, initializeCode, code } =
			await this.dfScriptGenerator.generateCodeToGetDataFrameRows({
				isDebugging: true,
				variableName: expression,
				startIndex: start,
				endIndex: end,
	...
	public override onWillReceiveMessage(message: any) {
		...
		if (
			...
			message.arguments &&
			this.currentVariablesReference ===
	...
	public async getFullVariable(
		...
		const { initializeCode, code, cleanupCode } =
			await this.varScriptGenerator.generateCodeToGetVariableInfo({
				isDebugging: true,
				...
				if (stResponse && stResponse.stackFrames[0]) {
					...
					if (mode === KernelDebugMode.RunByLine) {
						...
						if (
							sf.source &&
							cell &&
							sf.source.path === cell.document.uri.toString()
						) {
