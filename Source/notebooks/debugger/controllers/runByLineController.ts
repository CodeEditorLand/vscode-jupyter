...
import { commands, NotebookCell, Position } from "vscode";
...
export class RunByLineController implements IDebuggingDelegate {
	...
	private async shouldStepIn(threadId: number): Promise<boolean> {
		...
		const stResponse = await this.debugAdapter.stackTrace({
			threadId,
			startFrame: 0,
			levels: 1,
		});
	...
	private async initializeExecute() {
		...
		const textLines = splitLines(this.debugCell.document.getText(), {
			trim: false,
		...
		commands
			.executeCommand("notebook.cell.execute", {
				ranges: [
					{
						start: this.debugCell.index,
						end: this.debugCell.index + 1,
					},
				],
