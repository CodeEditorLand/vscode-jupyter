	...
	implements IExtensionSyncActivationService, INotebookDebuggingManager
{
	...
	public constructor(
		...
		this.runByLineCells = new ContextKey(
			EditorContexts.RunByLineCells,
			commandManager
		);
		...
		this.debugDocuments = new ContextKey(
			EditorContexts.DebugDocuments,
			commandManager
		);
	}
	...
	private async startDebuggingCell(
		...
		const config: INotebookDebugConfig = {
			...
			request: "attach",
			justMyCode:
				mode === KernelDebugMode.Cell ? settings.debugJustMyCode : true,
	...
	protected async createDebugAdapterDescriptor(
		...
		const notebook = this.vscNotebook.notebookDocuments.find(
