...
export class FileConverter implements IFileConverter {
	constructor(
		@inject(IExport)
		@named(ExportFormat.python)
		private readonly exportToPythonPlain: IExport,
		@inject(INbConvertExport)
		@named(ExportFormat.pdf)
		private readonly exportToPDF: INbConvertExport,
		@inject(INbConvertExport)
		@named(ExportFormat.html)
		private readonly exportToHTML: INbConvertExport,
		@inject(INbConvertExport)
		@named(ExportFormat.python)
		private readonly exportToPython: INbConvertExport,
		...
		@inject(ExportFileOpener)
		protected readonly exportFileOpener: ExportFileOpener,
	...
	protected async performExport(
		...
		if (
			...
			this.configuration.getSettings(sourceDocument.uri)
				.pythonExportMethod !== "nbconvert"
		) {
			// Unless selected by the setting use plain conversion for python script convert
			await this.performPlainExport(
				format,
				sourceDocument,
				target,
				token
		...
		if (target) {
			switch (format) {
				case ExportFormat.python:
					await this.exportToPythonPlain.export(
						sourceDocument,
						target,
						cancelToken
					);
	...
	protected async exportToFormat(
		...
		switch (format) {
			case ExportFormat.python:
				return await this.exportToPython.export(
					sourceDocument,
					target,
					interpreter,
					cancelToken
			...
			case ExportFormat.pdf:
				return await this.exportToPDF.export(
					sourceDocument,
					target,
					interpreter,
					cancelToken
			...
			case ExportFormat.html:
				return await this.exportToHTML.export(
					sourceDocument,
					target,
					interpreter,
					cancelToken
