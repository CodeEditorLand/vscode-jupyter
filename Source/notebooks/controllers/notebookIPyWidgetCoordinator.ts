	...
	public changeController(controller: IVSCodeNotebookController) {
		...
		this.controllerMessageHandler = controller.onDidReceiveMessage(
			(e) => {
				...
				if (
					e.editor === this.editor &&
					this.controller?.id === controller.id
				) {
...
{
	private readonly messageCoordinators = new WeakMap<
		NotebookDocument,
	 ...
	 */
	public readonly notebookCommunications = new WeakMap<
		NotebookEditor,
