...
suite("Preferred Kernel Connection", () => {
	...
	const serverProviderHandle1 = {
		handle: "handle1",
		id: "id1",
		extensionId: "",
	};
	const serverProviderHandle2 = {
		handle: "handle2",
		id: "id2",
		extensionId: "",
	};
	const remoteLiveKernelConnection1 =
	...
	const remoteLiveKernelConnection2 =
		LiveRemoteKernelConnectionMetadata.create({
			baseUrl: "",
			id: "liveRemote2",
			kernelModel: instance(mock<LiveKernelModel>()),
	...
	const remoteLiveJavaKernelConnection =
		LiveRemoteKernelConnectionMetadata.create({
			baseUrl: "",
			id: "liveRemoteJava",
			kernelModel: {
	...
	setup(() => {
		...
		remoteKernelFinder =
			mock<IContributedKernelFinder<RemoteKernelConnectionMetadata>>();
		localKernelSpecFinder =
			mock<IContributedKernelFinder<LocalKernelConnectionMetadata>>();
	...
	suite("Live Remote Kernels (preferred match)", () => {
		test("Find preferred kernel spec if there is no exact match for the live kernel connection (match kernel spec name)", async () => {
			...
			notebookMetadata.kernelspec!.name =
				remoteJavaKernelSpec.kernelSpec.name;
		...
		test("Find preferred kernel spec if there is no exact match for the live kernel connection (match kernel spec language)", async () => {
			...
			notebookMetadata.language_info!.name =
				remoteJavaKernelSpec.kernelSpec.language!;
		...
		test("Find existing session if there is an exact match for the notebook", async () => {
			...
			notebook = new TestNotebookDocument(
				Uri.file("/foo/bar/baz/sample.ipynb"),
				"jupyter-notebook",
				{
					custom: { metadata: notebookMetadata },
				}
	...
	suite("Local Kernel Specs (preferred match)", () => {
		...
		test("Find match for notebook when kernel spec name matches", async () => {
			...
			notebookMetadata.kernelspec!.name =
				localJavaKernelSpec.kernelSpec.name;
		...
		test("Find match for notebook when kernel spec language matches", async () => {
			...
			notebookMetadata.language_info!.name =
				localJavaKernelSpec.kernelSpec.language!;
