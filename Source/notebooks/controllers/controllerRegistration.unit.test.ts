...
suite("Controller Registration", () => {
	...
	setup(() => {
		...
		when(serviceContainer.get<ICommandManager>(ICommandManager)).thenReturn(
		...
		when(serviceContainer.get<IBrowserService>(IBrowserService)).thenReturn(
			instance(browser)
		);
		when(
		...
		).thenReturn(instance(documentManager));
		when(
	...
	[true, false].forEach((web) => {
		suite(`${web ? "Web" : "Desktop"}`, () => {
			...
			test("Disposed controller for if associated kernel is removed", async function () {
				...
				onDidChangeKernelsInContributedLocalKernelFinder.fire({
					removed: [javaKernelConnection],
				});
				await clock.runAllAsync();
				...
				onDidChangeKernelsInContributedPythonKernelFinder.fire({
					removed: [condaPythonConnection],
				});
				await clock.runAllAsync();
