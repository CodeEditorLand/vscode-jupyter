...
export class PythonEnvKernelConnectionCreator {
		...
		controllerSelection.onControllerSelected(
			(e) => {
				...
				if (
					...
					(e.controller.connection.kind !==
						"startUsingPythonInterpreter" ||
						!e.controller.connection.interpreter ||
						e.controller.connection.interpreter.id !==
							this.createdEnvId)
				) {
	...
	> {
		...
		if (
			this.cancelTokeSource.token.isCancellationRequested ||
			envResult.action
		) {
		...
		const dependencyService =
			ServiceContainer.instance.get<IKernelDependencyService>(
				IKernelDependencyService
			);
	...
	private async waitForPythonKernel(env: PythonEnvironment) {
		...
		const finder = kernelFinder.registered.find(
			(item) =>
				item.kind === ContributedKernelFinderKind.LocalPythonEnvironment
	...
	private async waitForPythonKernelFromFinder(
		...
		return new Promise<PythonKernelConnectionMetadata>((resolve) => {
			...
			if (!kernel) {
				traceWarning(
					`New Python Environment ${getDisplayPath(
						env.uri
					)} not found as a kernel`
				);
			}
			return kernel;
		});
	...
	private async createPythonEnvironment(): Promise<{
		...
		try {
			...
			if (!env) {
				traceWarning(
					`No interpreter details for New Python Environment ${getDisplayPath(
						Uri.file(path)
					)}`
				);
			}
