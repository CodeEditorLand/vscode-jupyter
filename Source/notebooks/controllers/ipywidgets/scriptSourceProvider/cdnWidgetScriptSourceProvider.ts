 ...
 */
@injectable()
export class CDNWidgetScriptSourceProvider
	...
	public async isOnCDN(moduleName: string): Promise<boolean> {
		...
		const promise = (async () => {
			const unpkgPromise = createDeferredFromPromise(
				this.httpClient.exists(`${unpgkUrl}${moduleName}`)
			);
			const jsDeliverPromise = createDeferredFromPromise(
				this.httpClient.exists(`${jsdelivrUrl}${moduleName}`)
			);
			await Promise.race([
	...
	private async handleWidgetSourceNotFound(
		...
		switch (selection) {
			...
			case Common.reportThisIssue:
				return this.appShell.openUrl(
					"https://aka.ms/CreatePVSCDataScienceIssue"
				);
			default:
				noop();
		}
