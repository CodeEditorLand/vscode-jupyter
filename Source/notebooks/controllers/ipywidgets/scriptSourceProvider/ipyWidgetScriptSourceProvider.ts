 ...
 */
export class IPyWidgetScriptSourceProvider
	implements IWidgetScriptSourceProvider
{
	...
	private async sendTelemetryForWidgetModule(
		...
		const isOnCDN =
			source === "cdn" ||
			isJupyterWidget ||
			(await this.cdnScriptProvider.isOnCDN(moduleName));
	...
	private monitorKernel() {
		...
		const subscription = this.kernel.kernelSocket.subscribe(() =>
	...
	private onAnyMessage(_: IKernelConnection, msg: IAnyMessageArgs) {
		if (
			msg.direction === "recv" &&
			msg.msg.header.msg_type === "comm_open"
		) {
