...
export class IPyWidgetMessageDispatcher implements IIPyWidgetMessageDispatcher {
	...
	private subscribeToKernelSocket(kernel: IKernel) {
		...
		kernel.session.kernelSocket.subscribe((info) => {
			// Remove old handlers.
			this.kernelSocketInfo?.socket?.removeReceiveHook(
				this.onKernelSocketMessage
			); // NOSONAR
			...
			if (this.kernelWasConnectedAtLeastOnce) {
				...
				this.waitingMessageIds.forEach((d) =>
					d.resultPromise.resolve()
				);
			...
			this.kernelSocketInfo.socket?.addReceiveHook(
				this.onKernelSocketMessage
			); // NOSONAR
	...
	private sendKernelOptions() {
		...
		this.raisePostMessage(
			IPyWidgetMessages.IPyWidgets_kernelOptions,
			this.kernelSocketInfo.options
		);
	}
	...
	private sendPendingMessages() {
		...
		while (this.pendingMessages.length) {
			try {
				this.kernelSocketInfo.socket?.sendToRealKernel(
