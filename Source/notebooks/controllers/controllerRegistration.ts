...
export class ControllerRegistration
	implements IControllerRegistration, IExtensionSyncActivationService
{
	...
	activate(): void {
		...
		this.notebook.notebookDocuments.forEach((notebook) =>
			this.onDidOpenNotebookDocument(notebook).catch(noop)
		);
	...
	private async loadControllersImpl() {
		...
		const disposedControllers = Array.from(this.registered).filter(
			(controller) => {
				const connectionIsStillValid =
					...
					(isCI &&
						this._activeInterpreterControllerIds.has(
							controller.id
						));
	...
	private createNotebookControllers(
		...
		try {
			this.batchAdd(kernelConnections, [
				JupyterNotebookView,
				InteractiveWindowView,
			]);
		} catch (ex) {
	...
	private onDidChangeFilter() {
		...
		this.registered.forEach((item) => {
			...
			if (
				this.isFiltered(item.connection) &&
				this.canControllerBeDisposed(item)
			) {
				traceWarning(
	...
	addImpl(
		...
		traceInfoIfCI(
			`Create Controller for ${metadata.kind} and id '${
				metadata.id
			}' for view ${types.join(", ")}`
		);
		try {
			...
			types
				...
				.filter(([id]) => {
					...
					if (controller) {
						...
						traceInfoIfCI(
							`Existing controller '${id}' will be excluded as it is filtered`
						);
						return false;
					}
					traceInfoIfCI(
				...
				.forEach(([id, viewType]) => {
					const controller = VSCodeNotebookController.create(
						...
						this.serviceContainer.get<IKernelProvider>(
							IKernelProvider
						),
						this.serviceContainer.get<IExtensionContext>(
							IExtensionContext
						),
						this.disposables,
						this.serviceContainer.get<NotebookCellLanguageService>(
						...
						this.serviceContainer.get<IDocumentManager>(
							IDocumentManager
						),
						this.serviceContainer.get<IApplicationShell>(
							IApplicationShell
						),
						this.serviceContainer.get<IBrowserService>(
							IBrowserService
						),
						this.extensionChecker,
						this.serviceContainer,
						this.serviceContainer.get<IConnectionDisplayDataProvider>(
							IConnectionDisplayDataProvider
						)
					);
						...
						(e) => {
							traceInfoIfCI(
								`Controller ${e.controller
									?.id} selected for ${e.notebook.uri.toString()}`
							);
	...
	private getControllerId(
		...
		return viewType === JupyterNotebookView
			? metadata.id
			: `${metadata.id}${InteractiveControllerIdSuffix}`;
	}

	private isControllerAttachedToADocument(
		controller: IVSCodeNotebookController
	) {
		return this.notebook.notebookDocuments.some((doc) =>
			controller.isAssociatedWithDocument(doc)
		);
	}
}
