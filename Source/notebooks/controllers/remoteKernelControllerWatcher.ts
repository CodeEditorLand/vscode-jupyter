 ...
 */
@injectable()
export class RemoteKernelControllerWatcher
	implements IExtensionSyncActivationService
{
	private readonly handledProviders =
		new WeakSet<IInternalJupyterUriProvider>();
	private readonly handledCollections =
		new WeakSet<JupyterServerCollection>();
	...
	constructor(
		...
		private readonly providerRegistry: IJupyterUriProviderRegistration,
	...
	activate(): void {
		this.providerRegistry.onDidChangeProviders(
	...
	@swallowExceptions(
		"Failed to check what servers were shutdown in a Jupyter Collection in Controller Watcher"
	)
	private async checkJupyterServerProviderHandlers(
		...
		if (collection.serverProvider) {
			await this.checkExpiredServersInJupyterCollection(
				collection,
				collection.serverProvider
			);
		}
	...
	private async checkExpiredServersInJupyterCollection(
		...
		if (
			!this.handledServerProviderChanges.has(serverProvider) &&
			serverProvider.onDidChangeServers
		) {
			const serverProviderChangeHandler =
				serverProvider.onDidChangeServers(
					() => {
						if (collection.serverProvider === serverProvider) {
							...
							this.handledServerProviderChanges.delete(
								serverProvider
							);
						}
					},
		...
		try {
			const currentServers = await Promise.resolve(
				serverProvider.provideJupyterServers(tokenSource.token)
			);
		...
		this.providerRegistry.providers.forEach((provider) => {
			...
			if (provider.onDidChangeHandles) {
				provider.onDidChangeHandles(
					() => this.onProviderHandlesChanged(provider),
					this,
					this.disposables
				);
			}
	...
	private async onProviderHandlesChanged(
