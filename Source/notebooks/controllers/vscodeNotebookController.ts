	...
	implements Disposable, IVSCodeNotebookController
{
	...
	private runningCellExecutions = new Map<
		NotebookDocument,
		NotebookCellExecution
	>();
	get id() {
	...
	private async handleExecution(
		...
		traceInfo(
			`Handle Execution of Cells ${cells.map(
				(c) => c.index,
			)} for ${getDisplayPath(notebook.uri)}`,
		);
		...
		await Promise.all(
			cells.map((cell) => this.executeCell(notebook, cell)),
		);
	}
	private warnWhenUsingOutdatedPython() {
		...
		if (
			pyVersion.major < 3 ||
			(pyVersion.major === 3 && pyVersion.minor <= 5)
		) {
			window
				.showWarningMessage(
	...
	private async onDidChangeSelectedNotebooks(event: {
		...
		traceInfoIfCI(
			`Controller ${this.connection.kind}:${
				this.id
			} associated with nb ${getDisplayPath(event.notebook.uri)}`,
		);
		...
		this._onNotebookControllerSelected.fire({
	...
	public async executeCell(
		...
		disposables.add(
			workspace.onDidCloseNotebookDocument((e) =>
				e === doc ? token.cancel() : undefined,
			),
		);
		// Start execution now (from the user's point of view)
		let exec = this.createCellExecutionIfNecessary(
			cell,
		...
		try {
			...
			if (kernel && kernel.controller.id !== controller.id) {
				...
				exec = this.createCellExecutionIfNecessary(
					cell,
					kernel.controller,
				);
			}
			...
			const promise = this.kernelProvider
				.getKernelExecution(kernel)
				.executeCell(cell, codeOverride);
			...
			promise
				.then((state) => {
					...
					return lastCellExecutionTracker.deleteTrackedCellExecution(
						cell,
						kernel,
					);
				})
			...
			await endCellAndDisplayErrorsInCell(
				cell,
				controller,
				await errorHandler.getErrorMessageForDisplayInCell(
					ex,
					currentContext,
					doc.uri,
				),
	...
	private async onDidSelectController(document: NotebookDocument) {
		...
		if (
			...
			this.connectToKernel(document, new DisplayOptions(true)).catch(
				noop,
			);
		}
	}
...
async function updateNotebookDocumentMetadata(
	...
	const { changed } = await updateNotebookMetadata(
		metadata,
		kernelConnection,
		kernelInfo,
	);
