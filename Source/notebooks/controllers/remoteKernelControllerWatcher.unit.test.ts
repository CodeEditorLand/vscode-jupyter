...
suite("RemoteKernelControllerWatcher", () => {
	...
	setup(() => {
		...
		when(uriProviderRegistration.onDidChangeProviders).thenReturn(
			onDidChangeProviders.event
		);
	...
	test("Dispose controllers associated with an Jupyter Collection", async () => {
		when(uriProviderRegistration.providers).thenReturn([]);
		const localKernel = createControllerForLocalKernelSpec("local1");
		const remoteKernelSpecExt1Coll1Server1 =
		...
		const collection1OfExt1: JupyterServerCollection = {
			...
			serverProvider: {
				...
				provideJupyterServers: async () =>
					validServersInCollection1OfExt1,
		...
		onDidChangeJupyterServerCollections.fire({
			added: [],
			removed: [collection1OfExt1],
		});
		await clock.runAllAsync();
