...
suite("Python Environment Kernel Connection Creator", () => {
	...
	let onControllerSelected: EventEmitter<{
	...
	setup(() => {
		...
		localPythonEnvFinder =
			mock<IContributedKernelFinder<PythonKernelConnectionMetadata>>();
		...
		when(controllerRegistration.onControllerSelected).thenReturn(
		...
		).thenReturn(instance(controllerRegistration));
		when(
			serviceContainer.get<IKernelDependencyService>(
				IKernelDependencyService
			)
		).thenReturn(instance(kernelDependencyService));
	...
	test("Not does create a Python Env when Python extension fails to create it", async () => {
		...
		const kernel =
			await pythonEnvKernelConnectionCreator.createPythonEnvFromKernelPicker();
		const connection =
			"kernelConnection" in kernel ? kernel.kernelConnection : undefined;
	...
	test("Can cancel after creation of the Environment", async () => {
		...
		when(
			interpreterService.getInterpreterDetails(
				deepEqual({ path: newCondaEnvPath })
			)
		).thenCall(() => {
			cancellation.cancel();
	...
	test("Installs missing dependencies and returns the kernel connection", async () => {
		...
		).thenResolve(newCondaPythonKernel.interpreter);
		when(
			kernelDependencyService.installMissingDependencies(anything())
		).thenResolve(KernelInterpreterDependencyResponse.ok);
	...
	test("Abort creation if another controller is selected for the notebook", async () => {
		...
		when(
			...
			when(differentController.connection).thenReturn(
				differentConnection
			);
			onControllerSelected.fire({
				notebook,
