...
export class BaseKernelSelector extends Disposables implements IDisposable {
	...
	public async selectKernel(
		...
		| undefined
	> {
		return this.selectKernelImpl(quickPickFactory, {
			quickPick: undefined,
		});
	}
	...
	> {
		...
		if (isCommandQuickPickItem(result)) {
			try {
				...
				return connection
					? { selection: "controller", connection: connection }
					: undefined;
			} catch (ex) {
	...
	private rebuildQuickPickItems(quickPick: QuickPick<CompoundQuickPickItem>) {
		...
		if (
			recommendedItem &&
			isKernelPickItem(recommendedItem) &&
			currentActiveItem
		) {
			if (!isKernelPickItem(currentActiveItem)) {
				...
				currentActiveItem.connection.id !==
				recommendedItem.connection.id
			) {
	...
	private connectionToQuickPick(
		...
		displayData.onDidChange(
			(e) => setData(info, e, connection),
			this,
			this.disposables
		);
		return info;
