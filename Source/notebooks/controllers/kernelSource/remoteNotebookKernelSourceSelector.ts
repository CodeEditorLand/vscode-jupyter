...
function doesExtensionSupportRemovingAServer(extensionId: string) {
	return (
		...
		extensionId === JUPYTER_HUB_EXTENSION_ID
	);
}
// Provides the UI to select a Kernel Source for a given notebook document
@injectable()
export class RemoteNotebookKernelSourceSelector
	implements IRemoteNotebookKernelSourceSelector
{
	...
	constructor(
		...
		@inject(JupyterServerSelector)
		private readonly serverSelector: JupyterServerSelector,
		@inject(JupyterConnection)
		private readonly jupyterConnection: JupyterConnection,
		@inject(IConnectionDisplayDataProvider)
	...
	public async selectRemoteKernel(
		...
		const appShell =
			ServiceContainer.instance.get<IApplicationShell>(IApplicationShell);
		...
		try {
			...
			if (state.selection?.type === "connection") {
				return state.selection
					.connection as RemoteKernelConnectionMetadata;
			}
		} finally {
			dispose(state.disposables);
		}
	...
	private async getRemoteServersFromProvider(
			...
			Promise.all(
				servers
					...
					.map(async (server) => {
						...
						serversAndTimes.push({
							server,
							time: lastUsedTime?.time,
						});
					})
			),
		]);
		...
		serversAndTimes
			...
			.forEach(({ server, time }) => {
				quickPickServerItems.push({
					...
					description: time
						? DataScience.jupyterServerLastConnectionForQuickPickDescription(
								new Date(time)
						  )
						: undefined,
					buttons:
						serverProvider?.removeJupyterServer &&
						doesExtensionSupportRemovingAServer(
		...
		let defaultSelection: (typeof items)[0] | undefined =
			items.length === 1 && "default" in items[0] && items[0].default
				? items[0]
				: undefined;
		if (serverProvider) {
			...
			if (
				...
				itemsWithoutSeparators.every(
					(i) =>
						i.type !==
						KernelFinderEntityQuickPickType.UriProviderQuickPick
				)
			) {
			...
			>({
				...
				onDidTriggerItemButton: async (e) => {
					if (
						"type" in e.item &&
						(e.item.type ===
							KernelFinderEntityQuickPickType.KernelFinder ||
							e.item.type ===
			...
			switch (selectedSource.type) {
				case KernelFinderEntityQuickPickType.KernelFinder: {
					const result = await this.selectRemoteKernelFromPicker(
						state.notebook,
						Promise.resolve(
							selectedSource.kernelFinderInfo as IRemoteKernelFinder
						),
						token
					).catch((ex) =>
	...
	private async selectRemoteServerFromRemoteKernelFinder(
		...
		if (
			!selectedSource.provider.handleQuickPick ||
		...
		const finderPromise = (async () => {
			...
			const displayName = await raceCancellationError(
				token,
				this.jupyterPickerRegistration.getDisplayNameIfProviderIsLoaded(
					serverId
				)
			);
			return this.kernelFinderController.getOrCreateRemoteKernelFinder(
	...
	private async selectRemoteKernelFromPicker(
		...
		const getCategory = (item: KernelConnectionMetadata) => {
			return <{ label: string; sortKey?: string }>{
				label:
					this.displayDataProvider.getDisplayData(item).category ||
					"Other",
			};
		};
