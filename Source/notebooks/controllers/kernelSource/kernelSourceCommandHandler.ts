	...
	implements IExtensionSyncActivationService
{
	...
	constructor(
		...
		private readonly kernelDependency: IKernelDependencyService,
	...
	activate(): void {
				...
				notebooks.registerKernelSourceActionProvider(
					JupyterNotebookView,
					{
						onDidChangeNotebookKernelSourceActions:
							kernelSpecActionChangeEmitter.event,
						provideNotebookKernelSourceActions: () => {
		...
		const uriRegistration =
			ServiceContainer.instance.get<IJupyterUriProviderRegistration>(
				IJupyterUriProviderRegistration
			);
		uriRegistration.onDidChangeProviders(
