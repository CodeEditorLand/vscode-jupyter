	...
	literalType.members.forEach((m) => {
		if (
			...
			if (
				...
				(m.type?.kind === ts.SyntaxKind.UnionType &&
				...
				if (
					...
					if (
						...
						(newPossibleValues.length === possibleValues.length &&
							[...newPossibleValues].sort().join("") ===
								[...possibleValues].sort().join(""))
					) {
				...
				if (gdprEntry) {
					...
					if (
						...
						gdprEntry.comment = `${gdprEntry.comment}${
							gdprEntry.comment.trim().length === 0 ||
							gdprEntry.comment.trim().endsWith(".")
								? " "
								: ". "
						}${comment}`.trim();
					}
		...
		if (
			...
			(type.aliasTypeArguments[0] as any).types.map((item: any) => {
				if (
					...
					item.symbol.declarations[0].kind ===
						ts.SyntaxKind.TypeLiteral
				) {
					const props = computePropertiesForLiteralType(
	...
	const comment = commentLines.map((line, index) => {
		...
		if (commentLines.length === index + 1) {
			...
			return line.trim().startsWith("*")
				? line.trim().replace("*", "")
				: line; // CodeQL [SM02383] Replace just the first occurrence of '*'.
		}
	});
...
function generateDocumentationForCommonTypes(fileNames: string[]): void {
	...
	function visit(sourceFile: ts.SourceFile, node: ts.Node) {
		...
		if (
			...
			CommonPropertyAndMeasureTypeNames.includes(node.name.text)
		) {
...
function generateDocumentation(fileNames: string[]): void {
	...
	function visit(sourceFile: ts.SourceFile, node: ts.Node) {
		...
		if (
			...
			node.name?.text !== "IEventNamePropertyMapping"
		) {
			return;
		}
			...
			node.members.forEach((m) => {
				if (ts.isPropertyDeclaration(m)) {
					...
					if (
						...
						if (
							m.name.kind === ts.SyntaxKind.ComputedPropertyName
						) {
							const defs =
	...
	output.forEach((o) => {
		if (o.propertyGroups.length === 0) {
			...
			entries.push({
				...
				owner: o.gdpr.owner,
				feature: Array.isArray(o.gdpr.feature)
					? o.gdpr.feature.join(", ")
					: o.gdpr.feature || "",
				tags: Array.isArray(o.gdpr.tags)
					? o.gdpr.tags.join(", ")
					: o.gdpr.tags || "",
				groupDescription: "",
				propertyName: "",
					...
					const possibleValues =
						...
						p.possibleValues.length
							? p.possibleValues
									.map(
										(item) =>
											`${item.value} ${item.comment ? `(${item.comment})` : ""}`,
									)
									.join("\n")
							: "";
...
async function generateTelemetryOutput() {
	const files = await new Promise<string[]>((resolve, reject) => {
		glob(
			path.join(EXTENSION_ROOT_DIR_FOR_TESTS, "src/**/*.ts"),
			(ex, res) => {
				if (ex) {
					reject(ex);
				} else {
