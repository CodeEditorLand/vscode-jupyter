...
suite("Kernel Dependency Service", () => {
	...
	setup(() => {
		...
		when(
			cmdManager.executeCommand("notebook.selectKernel", anything())
		).thenResolve();
	...
	[undefined, Uri.file("test.py"), Uri.file("test.ipynb")].forEach(
		(resource) => {
			suite(`With resource = ${resource?.toString()}`, () => {
				...
				setup(() => {
					...
					const interactiveWindowProvider =
						mock<IInteractiveWindowProvider>();
					...
					if (resource && getResourceType(resource) === "notebook") {
						...
						when(notebooks.activeNotebookEditor).thenReturn(
							instance(editor)
						);
						when(notebooks.notebookDocuments).thenReturn([
							instance(document),
