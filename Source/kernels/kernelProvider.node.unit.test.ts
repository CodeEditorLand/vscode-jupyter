...
suite("Jupyter Session", () => {
	suite("Node Kernel Provider", function () {
		...
		function testKernelProviderEvents(thirdPartyKernelProvider = false) {
			const kernelProvider = thirdPartyKernelProvider
				? create3rdPartyKernelProvider()
			...
			if (kernelProvider instanceof KernelProvider) {
				sinon
					.stub(Kernel.prototype, "onStarted")
					.get(() => onStarted.event);
				sinon
					.stub(Kernel.prototype, "onStatusChanged")
					.get(() => onStatusChanged.event);
				sinon
					.stub(Kernel.prototype, "onRestarted")
					.get(() => onRestartedEvent.event);
				sinon
					.stub(Kernel.prototype, "onDisposed")
					.get(() => onDisposedEvent.event);
				...
				sinon
					.stub(ThirdPartyKernel.prototype, "onStarted")
					.get(() => onStarted.event);
				sinon
					.stub(ThirdPartyKernel.prototype, "onStatusChanged")
					.get(() => onStatusChanged.event);
				sinon
					.stub(ThirdPartyKernel.prototype, "onRestarted")
					.get(() => onRestartedEvent.event);
				sinon
					.stub(ThirdPartyKernel.prototype, "onDisposed")
					.get(() => onDisposedEvent.event);
	...
	suite("KernelProvider Node", () => {
		...
		setup(() => {
			...
			const onDidChangeNotebookCellExecutionState =
			...
			when(vscNotebook.onDidCloseNotebookDocument).thenReturn(
