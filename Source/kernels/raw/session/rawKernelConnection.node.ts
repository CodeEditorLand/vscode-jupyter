...
export class RawKernelConnection implements Kernel.IKernelConnection {
	...
	public get connectionStatus() {
		return this.realKernel
			? this.realKernel.connectionStatus
			: "connecting";
	}
	...
	public get info() {
		return this.realKernel
			? this.realKernel.info
			: this.infoDeferred.promise;
	}
	...
	public async start(token: CancellationToken): Promise<void> {
		...
		try {
			...
			const kernelProcess = (this.kernelProcess =
	...
	private hookupKernelProcessExitHandler(kernelProcess: IKernelProcess) {
		...
		this.exitHandler = kernelProcess.exited(
			(e: {
	...
	public async interrupt(): Promise<void> {
		...
		if (this.kernelProcess?.canInterrupt) {
			...
			this.kernelConnectionMetadata.kernelSpec.interrupt_mode ===
			"message"
		) {
...
async function postStartKernel(
	...
	traceVerbose(
		`Kernel status before requesting kernel info and after ready is ${kernel?.status}`
	);
	...
	try {
		...
		if (gotIoPubMessage.completed) {
			...
			traceWarning(
				`Didn't get response for requestKernelInfo after ${stopWatch.elapsedTime}ms.`
			);
		}
		sendKernelTelemetryEvent(
			resource,
...
async function waitForReady(
	...
	traceVerbose(
		`Waiting for Raw session to be ready, currently ${kernel.connectionStatus}`
	);
