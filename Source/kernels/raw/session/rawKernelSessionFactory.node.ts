...
export class RawKernelSessionFactory implements IRawKernelSessionFactory {
	...
	public async create(
		...
		const [workingDirectory] = await Promise.all([
			raceCancellationError(
				...
				this.workspaceService
					.computeWorkingDirectory(options.resource)
					.then((dir) => vscode.Uri.file(dir))
			),
			raceCancellationError(
				options.token,
		...
		const rawSession = new RawJupyterSessionWrapper(
			session,
			options.resource,
			options.kernelConnection
		);
		rawSession.onDidDispose(
			() => this.sessions.delete(rawSession),
			this,
			this.disposables
		);
