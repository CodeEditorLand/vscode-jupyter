...
export class ContributedLocalKernelSpecFinder
	implements
		IContributedKernelFinder<LocalKernelConnectionMetadata>,
		IExtensionSyncActivationService
{
	...
	activate() {
		...
		const updateCombinedStatus = () => {
			const latestStatus: (typeof this.nonPythonKernelFinder.status)[] = [
				...
				this.interpreters.status === "refreshing"
					? "discovering"
					: "idle",
			];
		...
		this.interpreters.onDidChangeInterpreters(
			async () => this.loadData().then(noop, noop),
			this,
			this.disposables
		);
	...
	private async updateCache() {
		try {
			...
			const kernelSpecs = this.nonPythonKernelFinder.kernels.filter(
				(item) => {
	...
	private async writeToCache(values: LocalKernelConnectionMetadata[]) {
		...
		if (values.length) {
			...
			this.disposables.push(
				new Disposable(() => clearTimeout(this.cacheLoggingTimeout))
			);
		}
	}
}
