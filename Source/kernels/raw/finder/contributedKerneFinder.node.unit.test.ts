...
[false, true].forEach((isWindows) => {
	suite(
		...
		() => {
			...
			let onDidChangeInterpreter: EventEmitter<
			...
			async function initialize(
				...
				onDidChangeInterpreter = new EventEmitter<
					PythonEnvironment | undefined
				>();
				onDidChangeInterpreters = new EventEmitter<
					PythonEnvironment[]
				>();
				...
				const testWorkspaceFolder = Uri.file(
				...
				when(workspaceService.rootFolder).thenReturn(
					testWorkspaceFolder
				);
				...
				(testData.interpreters || []).forEach((interpreter) => {
					if ("interpreter" in interpreter) {
						(interpreter.kernelSpecs || []).forEach(
							(kernelSpec) => {
				...
				when(fs.exists(anything())).thenResolve(true);
				const nonPythonKernelSpecFinder =
					new LocalKnownPathKernelSpecFinder(
						instance(fs),
				...
				const pythonKernelFinderWrapper =
					new OldLocalPythonAndRelatedNonPythonKernelSpecFinder(
			...
			const python39PyEnv_HelloWorld: PythonEnvironment = {
				...
				sysPrefix: isWindows
					? "C:/pyenv/envs/temp"
					: "/users/username/pyenv/envs/temp",
			...
			const fullyQualifiedPythonKernelSpecForGlobalPython36WithCustomEnvVars: KernelSpec.ISpecModel =
				{
					...
					display_name:
						"Custom Kernel for Global Python 36 with Custom Env Vars",
					language: "python",
			...
			suiteSetup(async () => {
				kernelspecRegisteredByOlderVersionOfExtension = {
					...
					display_name:
						"Kernelspec registered by older version of extension",
					language: "python",
			...
			async function verifyKernels(expectations: ExpectedKernels) {
				...
				actualKernels.forEach((kernel) => {
					...
					if (kernel.interpreter) {
						...
						(kernel.interpreter as any).displayPath =
							kernel.interpreter.displayPath || undefined;
					}

					const serialize = kernel.toJSON();
					const deserialized =
						BaseKernelConnectionMetadata.fromJSON(serialize);
					...
					const lowerCasedDeserialized =
						cloneWithAppropriateCase(deserialized);
			...
			test("Kernelspecs registered by older versions of extensions `should not` be displayed & must be deleted", async () => {
				...
				const globalKernelSpecDir =
					await jupyterPaths.getKernelSpecRootPath();
