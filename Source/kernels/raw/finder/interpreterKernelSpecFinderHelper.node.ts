...
export async function findKernelSpecsInInterpreter(
	...
	if (!sysPrefix) {
		logger.warn(
			`Failed to get sysPrefix for interpreter ${getDisplayPath(interpreter.id)}`,
		);
	...
	await Promise.all(
		kernelSpecs.map(async (kernelSpecFile) => {
			try {
				...
				logger.error(
					`Failed to load kernel spec ${kernelSpecFile}`,
					ex,
				);
			}
		}),
...
export class InterpreterSpecificKernelSpecsFinder extends DisposableBase {
	...
	private _kernels = new Map<
		string,
		PythonKernelConnectionMetadata | LocalKernelConnectionMetadata
	>();
	...
	constructor(
		...
		this.lastKnownEnvFolder =
			getCachedEnvironment(interpreter)?.environment?.folderUri;
	...
	public async listKernelSpecs(refresh?: boolean) {
		...
		void this.kernelSpecPromise.then(() => {
			switch (this._kernels.size) {
				case 0:
					logger.trace(
						`No Kernels found in interpreter ${this.interpreter.id}`,
					);
				...
				default:
					logger.trace(
						`Kernels for interpreter ${this.interpreter.id} are ${Array.from(
							this._kernels.keys(),
						).join(", ")}`,
					);
			}
	...
	private clearCacheWhenInterpretersChange() {
		...
		if (
			...
			this.lastKnownEnvFolder?.fsPath !==
				interpreter.environment?.folderUri?.fsPath ||
			this.lastKnownInterpreterVersion?.sysVersion !==
				version?.sysVersion ||
			getEnvironmentType(this.interpreter) !==
	...
	private async listKernelSpecsImpl() {
		...
		logger.trace(
			`Search for KernelSpecs in Interpreter ${getDisplayPath(this.interpreter.uri)}`,
		);
		...
		const disposable = onFound.event((jupyterKernelSpec) => {
			...
			if (
				...
				(!jupyterKernelSpec.env ||
					Object.keys(jupyterKernelSpec.env).length === 0) &&
			...
			const kernelSpec = isKernelLaunchedViaLocalPythonIPyKernel(
				jupyterKernelSpec,
			)
		...
		if (pathInArgv && path.basename(pathInArgv) !== pathInArgv) {
			...
			const exactMatchBasedOnArgv = interpreters.find((i) => {
				if (areInterpreterPathsSame(pathInArgVUri, i.executable.uri)) {
					logger.trace(
						`Kernel ${kernelSpec.name} matches ${getDisplayPath(i.id)} based on argv.`,
					);
		...
		if (kernelSpec.interpreterPath) {
			...
			const matchBasedOnInterpreterPath = interpreters.find((i) => {
				if (
					...
					logger.trace(
						`Kernel ${kernelSpec.name} matches ${getDisplayPath(i.id)} based on interpreterPath.`,
					);
		...
		return resolvedPythonEnvToJupyterEnv(
			interpreters.find((i) => {
				...
				if (
					...
					getCachedEnvironment(i)?.environment?.name
				) {
					logger.trace(
						`Kernel ${kernelSpec.name} matches ${getDisplayPath(i.id)} based on display name`,
					);
