...
} from "../../../platform/common/platform/fs-paths.node";
...
export function localPythonKernelsCacheKey() {
	const LocalPythonKernelsCacheKey =
		"LOCAL_KERNEL_PYTHON_AND_RELATED_SPECS_CACHE_KEY_V_2023_3";
	return `${LocalPythonKernelsCacheKey}:${env.appHost}:${
		env.remoteName || ""
	}`;
}
...
export async function findKernelSpecsInInterpreter(
	...
	const kernelSearchPath = Uri.file(
		path.join(interpreter.sysPrefix, baseKernelPath)
	);
	const rootSpecPaths =
		await jupyterPaths.getKernelSpecRootPaths(cancelToken);
	...
	const kernelSpecs = await kernelSpecFinder.findKernelSpecsInPaths(
		kernelSearchPath,
		cancelToken
	);
	if (cancelToken.isCancellationRequested) {
	...
	await Promise.all(
		kernelSpecs.map(async (kernelSpecFile) => {
			...
			const kernelSpec = await kernelSpecFinder.loadKernelSpec(
				kernelSpecFile,
				cancelToken,
				interpreter
			);
	...
	results = results.filter(
		(r) => !r.specFile || !originalSpecFiles.has(r.specFile)
	);
	if (results.length) {
		traceVerbose(
			`Kernel Specs found in interpreter ${
				interpreter.id
			} are ${JSON.stringify(results)}`
		);
	}
...
export class InterpreterSpecificKernelSpecsFinder implements IDisposable {
	...
	constructor(
		...
		this.interpreterService.onDidChangeInterpreter(
			this.clearCacheWhenInterpretersChange,
			this,
			this.disposables
		);
	...
	public async listKernelSpecs(refresh?: boolean) {
		...
		this.kernelSpecPromise
			.then((kernels) => {
				traceVerbose(
					`Kernels for interpreter ${
						this.interpreter.id
					} are ${kernels.map((k) => k.id).join(", ")}`
				);
			})
			.catch(noop);
		return this.kernelSpecPromise;
	...
	private clearCacheWhenInterpretersChange() {
		const interpreter = this.interpreterService.resolvedEnvironments.find(
			(i) => i.id === this.interpreter.id
		);
		if (!interpreter) {
			return;
		}
	...
	private async listKernelSpecsImpl() {
		...
		traceVerbose(
			`Search for KernelSpecs in Interpreter ${getDisplayPath(
				this.interpreter.uri
			)}`
		);
		const [kernelSpecsBelongingToPythonEnvironment, tempDirForKernelSpecs] =
			await Promise.all([
		...
		const interpreterSpecificKernelSpec = createInterpreterKernelSpec(
			this.interpreter,
		...
		for (const k of kernelSpecsBelongingToPythonEnvironment) {
			if (
				...
				traceVerbose(
					`Hiding default kernel spec '${k.display_name}', '${
						k.name
					}', ${getDisplayPath(
						k.argv[0]
					)} for interpreter ${getDisplayPath(
						k.interpreterPath
					)} and spec ${getDisplayPath(k.specFile)}`
				);
				continue;
			}
...
export class GlobalPythonKernelSpecFinder implements IDisposable {
	private readonly kernelsPerInterpreter = new Map<
		string,
		Promise<IJupyterKernelSpec[]>
	>();
	private readonly disposables: IDisposable[] = [];
	...
	private lastKnownGlobalPythonKernelSpecs: LocalKernelSpecConnectionMetadata[] =
		[];
	private static globalPythonKernelSpecsForWhichWeCouldNotFindInterpreterInfo =
		new Set<string>();
	constructor(
		private readonly interpreterService: IInterpreterService,
		...
		kernelSpecsFromKnownLocations.onDidChangeKernels(() => {
			const lastKnownGlobalPythonKernelSpecs =
				this.lastKnownGlobalPythonKernelSpecs;
			const newGlobalPythonKernelSpecs =
				this.listGlobalPythonKernelSpecs();
			if (
				lastKnownGlobalPythonKernelSpecs.length !==
					newGlobalPythonKernelSpecs.length ||
				!areObjectsWithUrisTheSame(
					lastKnownGlobalPythonKernelSpecs,
					newGlobalPythonKernelSpecs
				)
			) {
	...
	public async findMatchingInterpreter(
		...
		const kernelSpecHash = kernelSpec.specFile
			? await getTelemetrySafeHashedString(kernelSpec.specFile)
			: "";
		const isCreatedByUs = getKernelRegistrationInfo(kernelSpec)
			? true
			: false;
		...
		if (kernelSpec.language && kernelSpec.language !== PYTHON_LANGUAGE) {
			...
			if (
				...
				pathInArgv &&
				kernelSpec.specFile &&
				isLikelyAPythonExecutable(pathInArgv)
			) {
				sendTelemetryEvent(
		...
		const exactMatch = interpreters.find((i) => {
			if (
				...
				areInterpreterPathsSame(
					Uri.file(kernelSpec.metadata.interpreter.path),
					i.uri
				)
			) {
				traceVerbose(
		...
		if (pathInArgv && path.basename(pathInArgv) !== pathInArgv) {
			...
			if (
				!kernelSpec.specFile ||
				this.trustedKernels.isTrusted(Uri.file(kernelSpec.specFile))
			) {
				const interpreterInArgv =
		...
		if (kernelSpec.interpreterPath) {
			const kernelSpecInterpreterPath = Uri.file(
				kernelSpec.interpreterPath
			);
			const matchBasedOnInterpreterPath = interpreters.find((i) => {
				if (
					kernelSpec.interpreterPath &&
					areInterpreterPathsSame(kernelSpecInterpreterPath, i.uri)
				) {
					traceVerbose(
			...
			if (
				...
				const interpreterInInterpreterPath =
					await this.interpreterService.getInterpreterDetails(
	...
	private listGlobalPythonKernelSpecs(): LocalKernelSpecConnectionMetadata[] {
		return (this.lastKnownGlobalPythonKernelSpecs =
			this.kernelSpecsFromKnownLocations.kernels.filter(
				(item) => item.kernelSpec.language === PYTHON_LANGUAGE
			));
	}

	private warnAboutPythonKernelSpecWithInvalidPythonExec(
		kernelSpec: Readonly<IJupyterKernelSpec>
	) {
		const key = kernelSpec.specFile || JSON.stringify(kernelSpec);

		if (
			GlobalPythonKernelSpecFinder.globalPythonKernelSpecsForWhichWeCouldNotFindInterpreterInfo.has(
				key
			)
		) {
			return;
		}
		GlobalPythonKernelSpecFinder.globalPythonKernelSpecsForWhichWeCouldNotFindInterpreterInfo.add(
			key
		);
		traceWarning(
	...
	private async listKernelSpecsImpl() {
		...
		const activeInterpreters = await raceCancellation(
			cancelToken,
			[],
		...
		const globalPythonKernelSpecsRegisteredByUs =
			globalPythonKernelSpecs.filter((item) =>
		...
		const usingNonIpyKernelLauncher = (
			item: LocalKernelConnectionMetadata
		) => {
		...
		await Promise.all(
			globalPythonKernelSpecs
				.filter((item) => {
					const registrationInfo = getKernelRegistrationInfo(
						item.kernelSpec
					);
					if (
						!registrationInfo &&
						(usingNonIpyKernelLauncher(item) ||
							Object.keys(item.kernelSpec.env || {}).length > 0)
					) {
						return true;
					}
				...
				.map(async (item) => {
					...
					const matchingInterpreter =
						await this.findMatchingInterpreter(
							item.kernelSpec,
					...
					if (!matchingInterpreter) {
						...
						const executable = item.kernelSpec.argv.length
							? item.kernelSpec.argv[0].toLowerCase()
							: "";
						if (isLikelyAPythonExecutable(executable)) {
							this.warnAboutPythonKernelSpecWithInvalidPythonExec(
								item.kernelSpec
							);
							return;
						}
					...
					if (kernelSpec.kernelSpec.specFile) {
						globalKernelSpecsLoadedForPython.add(
							kernelSpec.kernelSpec.specFile
						);
					}
				})
		);
			...
			globalPythonKernelSpecsRegisteredByUs
				...
				.filter((kernelSpec) => {
					if (
						...
						traceVerbose(
							`Hiding default kernel spec '${
								kernelSpec.display_name
							}', '${kernelSpec.name}', ${getDisplayPath(
								kernelSpec.argv[0]
							)} for interpreter ${getDisplayPath(
								kernelSpec.interpreterPath
					...
					if (
						kernelSpec.specFile &&
						globalKernelSpecsLoadedForPython.has(
							kernelSpec.specFile
						)
					) {
						traceVerbose(
				...
				.map(async (k) => {
					...
					if (
						matchingInterpreter &&
						isKernelLaunchedViaLocalPythonIPyKernel(k)
					) {
						const result = isKernelLaunchedViaLocalPythonIPyKernel(
							k
						)
							? PythonKernelConnectionMetadata.create({
									kernelSpec: k,
						...
						if (kernelSpecKind === "registeredByOldVersionOfExt") {
							traceVerbose(
								`Ignoring Global Python KernelSpec '${
									k.display_name
								}', '${k.name}' (${getDisplayPath(
									k.specFile
						...
						traceInfo(
							`Using interpreter ${getDisplayPath(
								matchingInterpreter.id
							)} for Global Python kernel '${k.display_name}', ${
								k.name
							} (${getDisplayPath(k.specFile)})`
						);
						return result;
					} else {
						...
						const activeInterpreterOfAWorkspaceFolder =
							activeInterpreters.find((i) => !!i);
						...
						const kernelSpecInterpreterPath = k.metadata
							?.interpreter?.path
						...
						if (kernelSpecInterpreterPath) {
							...
							if (interpreterInKernelSpec) {
								...
								try {
									...
									traceError(
										`Failed to get interpreter details for Kernel Spec '${
											k.display_name
										}', '${
											k.name
										}' ${getDisplayPathFromLocalFile(
						...
						traceVerbose(
							`Interpreter for Local kernel ${result.id} is ${kernelInterpreter?.id}`
						);
				...
				.map(async (item) => {
					...
					traceVerbose(
						`Found kernel spec at end of discovery ${kernelSpec?.id}`
					);
					// Check if we have already seen this.
					if (
						kernelSpec &&
						!distinctKernelMetadata.has(kernelSpec.id)
					) {
						distinctKernelMetadata.set(kernelSpec.id, kernelSpec);
					}
				})
