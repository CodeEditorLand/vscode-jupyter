...
function isBestPythonInterpreterForAnInterruptDaemon(
...
export class PythonKernelInterruptDaemon {
	...
	public async createInterrupter(
		...
		try {
			return await this.createInterrupterImpl(
				pythonEnvironment,
				resource
			);
		} catch (ex) {
	...
	private async createInterrupterImpl(
		...
		return {
			...
			dispose: async () => {
				await this.sendCommand(
					{
						command: "DISPOSE_INTERRUPT_HANDLE",
						handle: interruptHandle,
					},
					pythonEnvironment,
					resource
					...
					traceError(
						`Failed to dispose interrupt handle for ${pythonEnvironment.id}`,
						ex
					)
				);
			},
	...
	private async getInterpreter(interpreter: PythonEnvironment) {
		if (
			interpreter &&
			isBestPythonInterpreterForAnInterruptDaemon(interpreter)
		) {
			return interpreter;
		}
			...
			await new Promise<void>((resolve, reject) => {
				...
				const subscription = proc.out.subscribe((out) => {
					...
					if (
						...
						out.out.includes(
							"ERROR: handling command :INITIALIZE_INTERRUPT:"
						) &&
						started
					) {
						splitLines(out.out, {
							...
							.forEach((output) => {
								try {
									...
									traceError(
										`Failed to parse interrupt daemon response, ${out.out}`,
										ex
									);
								}
							});
						...
						if (!started) {
							return reject(
								new Error(
									`Interrupt daemon failed to start, ${out.out}`
								)
							);
						}
						try {
				...
				this.disposableRegistry.push(
					new Disposable(() => subscription.unsubscribe())
				);
			});
			this.disposableRegistry.push(
				new Disposable(() => swallowExceptions(() => proc.proc?.kill()))
			);
