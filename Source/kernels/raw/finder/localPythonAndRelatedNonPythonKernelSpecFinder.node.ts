...
export class LocalPythonAndRelatedNonPythonKernelSpecFinder extends LocalKernelSpecFinderBase<LocalKernelConnectionMetadata> {
	 ...
	 */
	private _kernelsExcludingCachedItems = new Map<
		string,
		LocalKernelConnectionMetadata
	>();
	...
	private cachedInformationForPythonInterpreter = new Map<
		InterpreterId,
	...
	public refreshData(forcePythonInterpreterRefresh: boolean = false) {
		...
		const cancelToken = (this.refreshCancellation =
		...
		const promise = (async () => {
			...
			if (this.interpreterService.status === "idle") {
				...
				const validInterpreterIds = new Set(
					getCachedEnvironments().map((i) => i.id),
				);
				...
				const kernelsThatPointToInvalidValidInterpreters = Array.from(
					...
					if (
						item.interpreter &&
						!validInterpreterIds.has(item.interpreter.id)
					) {
						return true;
					}
				...
				if (kernelsThatPointToInvalidValidInterpreters.length) {
					logger.warn(
						`The following kernels use interpreters that are no longer valid or not recognized by Python extension, Kernels ${kernelsThatPointToInvalidValidInterpreters
							...
							.join(
								",",
							)} and valid interpreter ids include ${Array.from(validInterpreterIds).join(", ")}`,
					);
			...
			.catch((ex) =>
				logger.error(`Failed to discover kernels in interpreters`, ex),
			)
			.finally(() => {
	...
	private interpreterKernelSpecs = new Map<
		InterpreterId,
		InterpreterSpecificKernelSpecsFinder
	>();
