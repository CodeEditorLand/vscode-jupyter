...
export class KernelProcess implements IKernelProcess {
	...
	public async launch(
		...
		if (exeObs.proc) {
			exeObs.proc.stdout?.on("data", (data: Buffer | string) => {
				...
				traceVerbose(
					`KernelProcess output: ${(data || "").toString()}`
				);
	...
	private addPythonConnectionArgs(connectionFile: Uri): string[] {
		...
		newConnectionArgs.push(
			`--Session.signature_scheme="${this._connection.signature_scheme}"`
		);
	...
	@traceDecoratorVerbose(
		"Launching kernel in kernelProcess.ts",
		TraceOptions.Arguments | TraceOptions.BeforeCall
	)
