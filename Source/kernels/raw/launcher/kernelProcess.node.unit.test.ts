...
suite("kernel Process", () => {
	...
	setup(() => {
		...
		when(processServiceFactory.create(anything(), anything())).thenResolve(
			instance(processService)
		);
		when(
		...
		when(daemon.createInterrupter(anything(), anything())).thenResolve(
			interrupter
		);
		(instance(processService) as any).then = undefined;
	...
	test("Ensure kernelspec json file is created & the temp file disposed (to prevent file handle being left open)", async () => {
		...
		when(
			fs.createTemporaryLocalFile(deepEqual(tempFileCreationOptions))
		).thenResolve({
	...
	test("Ensure kernelspec json file is created with the connection info in it", async () => {
		...
		verify(
			fs.writeFile(uriEquals(tempFile), JSON.stringify(connection))
		).atLeast(1);
	});
	...
	test("Ensure connection file is created in jupyter runtime directory (python daemon kernel)", async () => {
		...
		verify(
			pythonProcess.execObservable(deepEqual(expectedArgs), anything())
		).once();
