		...
		>
{
	...
	private requestFuture:
		| Kernel.IShellFuture<
		  ...
		  >
		| undefined;
	private pendingOnReply:
		| ((msg: KernelMessage.IExecuteReplyMsg) => void | PromiseLike<void>)
		| undefined;
	...
	constructor(
		...
		previousLink: Kernel.IShellFuture<
			KernelMessage.IExecuteRequestMsg,
			KernelMessage.IExecuteReplyMsg
		>,
		...
		previousLink.done
			.then(() => this.requestExecute())
			.catch((e) => this.doneDeferred.reject(e));
	...
	private requestExecute() {
		...
		if (this.requestFuture) {
			...
			this.requestFuture.done
				.then((r) => this.doneDeferred.resolve(r))
				.catch((e) => this.doneDeferred.reject(e));
			this.clear();
		}
	}
