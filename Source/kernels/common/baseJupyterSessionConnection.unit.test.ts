...
suite("Base Jupyter Session Connection", () => {
	...
	let jupyterSession: BaseJupyterSessionConnection<
		INewSessionWithSocket,
		"remoteJupyter"
	>;
	...
	let sessionIOPubMessage: Signal<
		INewSessionWithSocket,
	...
	let sessionKernelChanged: Signal<
		...
		>
	>;
	let sessionUnhandledMessage: Signal<
		INewSessionWithSocket,
		KernelMessage.IMessage
	>;
	let sessionConnectionStatusChanged: Signal<
		INewSessionWithSocket,
		Kernel.ConnectionStatus
	>;
	let sessionAnyMessage: Signal<
		INewSessionWithSocket,
		Kernel.IAnyMessageArgs
	>;
	class DummySessionClass extends BaseJupyterSessionConnection<
		INewSessionWithSocket,
		"remoteJupyter"
	> {
	...
	setup(() => {
		...
		sessionDisposed = new Signal<INewSessionWithSocket, void>(
			instance(session)
		);
		sessionPropertyChanged = new Signal<INewSessionWithSocket, "path">(
			instance(session)
		);
		...
		sessionKernelChanged = new Signal<
			...
			>
		>(instance(session));
		sessionUnhandledMessage = new Signal<
			INewSessionWithSocket,
			KernelMessage.IMessage
		>(instance(session));
		sessionConnectionStatusChanged = new Signal<
			INewSessionWithSocket,
			Kernel.ConnectionStatus
		>(instance(session));
		sessionAnyMessage = new Signal<
			INewSessionWithSocket,
			Kernel.IAnyMessageArgs
		>(instance(session));
		kernelSocketInformation = mock<KernelSocketInformation>();
		when(kernelSocketInformation.socket).thenReturn(
			instance(mock<IKernelSocket>())
		);
		when(session.disposed).thenReturn(sessionDisposed);
		when(session.propertyChanged).thenReturn(sessionPropertyChanged);
		when(session.kernelSocketInformation).thenReturn(
			instance(kernelSocketInformation)
		);
		...
		when(session.statusChanged).thenReturn(
			new Signal<INewSessionWithSocket, Kernel.Status>(instance(session))
		);
	...
	test("Events are propagated", () => {
		...
		const event: { sender: unknown; args: unknown } = {
			sender: undefined,
			args: undefined,
		};
	...
	test("Methods are invoked in real session", () => {
		const methodNames: (keyof typeof jupyterSession)[] = [
			"changeKernel",
			"setName",
			"setPath",
			"setType",
	...
	test("Kernel Socket information available on start", async () => {
		...
		assert.strictEqual(
			socketInfo?.socket,
			instance(kernelSocketInformation).socket
		);
	...
	test("Kernel Socket information changes after restarting kernel", async () => {
		...
		when(session.kernelSocketInformation).thenReturn(
			instance(newKernelSocketInformation)
		);
	...
	test("Wrap the Kernel.IKernelConnection only when necessary", async () => {
		...
		await jupyterSession.restart();
		assert.strictEqual(
			jupyterSession.kernel as KernelConnectionWrapper,
			wrapperKernel
		);
		assert.strictEqual(wrapperKernel.originalKernel, instance(kernel));
		...
		assert.strictEqual(
			(jupyterSession.kernel as KernelConnectionWrapper).originalKernel,
			instance(newKernel)
		);
	});
});
