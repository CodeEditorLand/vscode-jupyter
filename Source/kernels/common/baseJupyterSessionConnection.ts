	...
	implements Session.ISessionConnection, IBaseKernelSession<T>
{
	...
	public get kernel(): Kernel.IKernelConnection | null {
		...
		this._wrappedKernel = new KernelConnectionWrapper(
			this.session.kernel,
			this.disposables
		);
		return this._wrappedKernel;
	...
	protected _kernelSocket = new ReplaySubject<
	...
	protected initializeKernelSocket() {
		...
		if (this.session.kernelSocketInformation.socket?.onAnyMessage) {
			...
			this.previousAnyMessageHandler =
				this.session.kernelSocketInformation.socket?.onAnyMessage(
					(msg) => {
						try {
							if (this._wrappedKernel) {
								...
								const message =
									...
									msg.msg instanceof ArrayBuffer
										...
										: msg.msg;
								this._wrappedKernel.anyMessage.emit({
							...
							traceWarning(
								`failed to deserialize message to broadcast anymessage signal`
							);
						}
					}
				);
