...
const allowedAccessToProposedApi = new Set([
...
export function getKernelsApi(extensionId: string): Kernels {
	return {
		async getKernel(uri: Uri) {
			...
			const notebooks =
				ServiceContainer.instance.get<IVSCodeNotebook>(IVSCodeNotebook);
...
async function requestKernelAccess(extensionId: string): Promise<boolean> {
	...
	const promise =
		apiCache.get(extensionId) || requestKernelAccessImpl(extensionId);
...
async function requestKernelAccessImpl(extensionId: string) {
	if (!allowedAccessToProposedApi.has(extensionId)) {
		throw new Error(
			`Extension ${extensionId} does not have access to proposed API`
		);
	}
	const displayName =
