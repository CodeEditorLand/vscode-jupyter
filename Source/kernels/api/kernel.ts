...
class WrappedKernelPerExtension implements Kernel {
	...
	constructor(
		...
		this.progress = new KernelExecutionProgressIndicator(
			this.extensionDisplayName,
			kernel
		);
	...
	async *executeCode(
	...
	): AsyncGenerator<OutputItem[], void, unknown> {
			...
			.then((codeExecution) => {
				...
				codeExecution.onRequestSent(
					() => {
						...
						if (!token.isCancellationRequested) {
							const progress = (this.previousProgress =
								this.progress.show());
				...
				codeExecution.onRequestAcknowledged(
					() => {
						...
						measures.requestAcknowledgedAfter =
		...
		token.onCancellationRequested(
			() => {
				...
				traceVerbose(
					`Code execution cancelled by extension ${this.extensionDisplayName}`
				);
			},
			this,
			disposables
