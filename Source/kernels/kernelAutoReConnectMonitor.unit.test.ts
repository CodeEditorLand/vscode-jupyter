...
} from "./execution/cellExecutionCreator";
...
suite("Kernel ReConnect Progress Message", () => {
	...
	function createKernel() {
		...
		const kernelConnectionStatusSignal = new Signal<
		...
		let onWillRestart: (e: "willRestart") => Promise<void> = () =>
			Promise.resolve();
		...
		return {
			...
			onRestarted,
			kernelConnectionStatusSignal,
			onWillRestart: () => onWillRestart("willRestart"),
		};
	}
...
suite("Kernel ReConnect Failed Monitor", () => {
	...
	setup(() => {
		...
		const stub = sinon
			.stub(CellExecutionCreator, "getOrCreate")
			.callsFake(() => instance(cellExecution));
	...
	test("Handle contributed server disconnect (server contributed by uri provider)", async () => {
		...
		when(
			jupyterUriProviderRegistration.getProvider(anything(), anything())
		).thenResolve({
