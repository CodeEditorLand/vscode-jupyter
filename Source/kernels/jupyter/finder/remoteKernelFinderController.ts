...
@injectable()
export class RemoteKernelFinderController
	implements IRemoteKernelFinderController, IExtensionSyncActivationService
{
	private serverFinderMapping: Map<string, RemoteKernelFinder> = new Map<
		string,
		RemoteKernelFinder
	>();
	...
	activate() {
		...
		this.jupyterPickerRegistration.onDidChangeProviders(
	...
	private async lookForServersInCollectionAndRemoveOldServers(
		...
		try {
			...
			(servers || []).forEach((server) => {
				...
				if (
					this.mappedServers.has(serverId) ||
					!usedServers.has(serverId)
				) {
					return;
				}
			...
			traceError(
				`Failed to get servers for Collection ${collection.id} in ${collection.extensionId}`,
				ex
			);
		} finally {
			token.dispose();
	...
	private async validateAndCreateFinder(serverUri: IJupyterServerUriEntry) {
		...
		const getDisplayNameFromNewApi = async () => {
			const collectionProvider =
				this.jupyterServerProviderRegistry.jupyterCollections.find(
					(c) =>
			...
			const servers =
				await collectionProvider.serverProvider.provideJupyterServers(
					token.token
				);
		...
		const getDisplayNameFromOldApi = async () => {
			const displayName =
				await this.jupyterPickerRegistration.getDisplayNameIfProviderIsLoaded(
					serverUri.provider
				);
			if (displayName) {
