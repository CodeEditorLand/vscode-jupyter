	...
	implements IRemoteKernelFinderController, IExtensionSyncActivationService
{
	...
	activate() {
		...
		this.jupyterPickerRegistration.onDidChangeCollections(
	...
	private handleProviderHandleChanges() {
		this.jupyterPickerRegistration.jupyterCollections.forEach(
			(collection) => {
	...
	private async validateAndCreateFinder(serverUri: IJupyterServerUriEntry) {
		...
		const getDisplayNameFromOldApi = async () => {
			const displayName =
				await this.jupyterPickerRegistration.jupyterCollections.find(
					(c) =>
						c.extensionId === serverUri.provider.extensionId &&
						c.id === serverUri.provider.id
				)?.label;
