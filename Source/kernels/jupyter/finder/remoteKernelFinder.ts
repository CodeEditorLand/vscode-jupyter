...
export class RemoteKernelFinder implements IRemoteKernelFinder, IDisposable {
	...
	private getListOfKernelsWithCachedConnection(
		...
		return this.cachedConnection
			.then((connInfo) => {
				...
				return connInfo
					? this.listKernelsFromConnection(connInfo)
					: Promise.resolve([]);
			})
			.catch((ex) => {
				if (usingCache) {
	...
	public async loadCache(
		...
		const promise = (async () => {
			...
			if (
				!ignoreCache &&
				Array.isArray(kernelsFromCache) &&
				kernelsFromCache.length > 0
			) {
	...
	private async writeToCache(values: RemoteKernelConnectionMetadata[]) {
		try {
			...
			this.cache = oldValues
				.filter((k) => latestValidKernels.has(k.id))
				.concat(added);
