...
export class JupyterServerHelper implements IJupyterServerHelper {
	...
	constructor(
		...
		if (workspace) {
			workspace.onDidChangeConfiguration(
				(e) => {
					if (
						e.affectsConfiguration("python.dataScience", undefined)
					) {
	...
	public async isJupyterServerSupported(
		...
		return this.jupyterInterpreterService
			? this.jupyterInterpreterService.isNotebookSupported(cancelToken)
			: false;
	}
	...
	private async startImpl(
		...
		const workingDir =
			await this.workspace.computeWorkingDirectory(resource);
		...
		const workingDirectory = expandWorkingDir(
			workingDir,
			this.workspace.rootFolder
				? urlPath.joinPath(this.workspace.rootFolder, `${uuid()}.txt`)
				: undefined,
			this.workspace,
			settings
		);
