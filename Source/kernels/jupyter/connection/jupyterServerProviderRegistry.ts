...
export class JupyterServerCollectionImpl
	extends Disposables
	implements JupyterServerCollection
{
...
class JupyterUriProviderAdaptor
	extends Disposables
	implements IJupyterUriProvider
{
	readonly id: string;
	...
	constructor(
		...
		if (this.provider.extensionId === JVSC_EXTENSION_ID) {
			this.getServerUriWithoutAuthInfo =
				this.getServerUriWithoutAuthInfoImpl.bind(this);
		}
	}
	override dispose() {
		super.dispose();
		...
		try {
			const items =
				(await Promise.resolve(
					this.provider.commandProvider.provideCommands(
						value || "",
						token.token
					)
				)) || [];
	...
	async handleQuickPick(
		...
		if (!this.provider.commandProvider) {
			throw new Error(
				`No Jupyter Server Command Provider for ${this.provider.extensionId}#${this.provider.id}`
			);
		}
		const token = new CancellationTokenSource();
		try {
			...
			try {
				const result = await Promise.resolve(
					this.provider.commandProvider.handleCommand(
						command,
						token.token
					)
				);
	...
	private cachedServerUri = new Map<
		string,
		{ lastValidated: StopWatch; promise: Promise<IJupyterServerUri> }
	>();
	async getServerUri(handle: string): Promise<IJupyterServerUri> {
		...
		if (!cache) {
			...
			cache = {
				promise: this.getServerUriImpl(handle),
				lastValidated: new StopWatch(),
			};
			cache.promise.catch(() => {
	...
	async getServerUriImpl(handle: string): Promise<IJupyterServerUri> {
		...
		try {
			...
			if (server.connectionInformation) {
				...
				return {
					...
					mappedRemoteNotebookDir:
						server.mappedRemoteDirectory?.fsPath,
			...
			if (this.provider.serverProvider?.resolveJupyterServer) {
				const result = await Promise.resolve(
					this.provider.serverProvider?.resolveJupyterServer(
						server,
						token.token
					)
				);
				const info =
					result?.connectionInformation ||
				...
				return {
					...
					authorizationHeader: info.headers,
					// eslint-disable-next-line local-rules/dont-use-fspath
					mappedRemoteNotebookDir: (result || server)
						.mappedRemoteDirectory?.fsPath,
					webSocketProtocols: info.webSocketProtocols,
					fetch: info.fetch,
					WebSocket: info.WebSocket,
				};
			...
			throw new Error(
				"Jupyter Provider does not implement the method resolveJupyterServer"
			);
		} finally {
			token.dispose();
		}
		...
		if (this.provider.serverProvider) {
			...
			try {
				const servers = await Promise.resolve(
					this.getServers(token.token)
				);
				return (servers || []).map((s) => s.id);
			} catch (ex) {
				traceError(
					`Failed to get Jupyter Servers from ${this.provider.extensionId}#${this.provider.id}`,
					ex
				);
				return [];
			} finally {
	...
	async getServerUriWithoutAuthInfoImpl(
		handle: string
	...
	async getServers(token: CancellationToken) {
		...
		if (!this.provider.serverProvider) {
			throw new Error(
				`No Jupyter Server Provider for ${this.provider.extensionId}#${this.provider.id}`
			);
		}
		const servers = await Promise.resolve(
			this.provider.serverProvider.provideJupyterServers(token)
		);
		this._servers.clear();
...
@injectable()
export class JupyterServerProviderRegistry
	extends Disposables
	implements IJupyterServerProviderRegistry
{
		...
		collection.onDidChangeProvider(
			() => {
				if (collection.serverProvider) {
					...
					adapter = new JupyterUriProviderAdaptor(
						collection,
						extensionId,
						this.jupyterConnection
					);
					uriRegistration =
						this.jupyterUriProviderRegistration.registerProvider(
							adapter,
							extensionId
						);
