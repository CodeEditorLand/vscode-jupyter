...
suite("Server Uri Storage", async () => {
	...
	setup(() => {
		...
		).thenResolve(resolvableInstance(mock<IJupyterServerUri>()));
		...
		).thenCall((_, defaultValue) => dataInMemento || defaultValue);
		when(
	...
	test("Migrate data from old storage to new & get all", async () => {
		...
		when(
			encryptedStorage.store(anything(), anything(), anything())
		).thenResolve();
		when(
			memento.update(Settings.JupyterServerUriList, anything())
		).thenResolve();
		...
		when(fs.readFile(anything())).thenCall(() =>
			JSON.stringify(itemsInNewStorage)
		);
	...
	test("Clear when we have some old data", async () => {
		...
		verify(
			memento.update(Settings.JupyterServerUriList, deepEqual([]))
		).once();
		verify(
	...
	function generateDummyData(
		numberOfEntries: number = 2,
		generateNewDataAsWell: boolean = false
	) {
		const data: any[] = [];
		const uris: string[] = [];
		const itemsInNewStorage: StorageMRUItem[] = [];
		...
		when(
			...
			uris.join(Settings.JupyterServerRemoteLaunchUriSeparator)
		);
		...
		when(fs.writeFile(anything(), anything())).thenCall((_, data) => {
			...
			when(fs.readFile(anything())).thenCall(() =>
				JSON.stringify(itemsWrittenIntoStorage)
			);
			when(fs.delete(anything())).thenCall(() => {
				itemsInNewStorage.splice(0, itemsInNewStorage.length);
				itemsWrittenIntoStorage.splice(
					0,
					itemsWrittenIntoStorage.length
				);
				return Promise.resolve();
			});
			return Promise.resolve();
