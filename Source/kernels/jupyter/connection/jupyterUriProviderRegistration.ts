...
export const REGISTRATION_ID_EXTENSION_OWNER_MEMENTO_KEY =
	...
	implements IJupyterUriProviderRegistration, IExtensionSyncActivationService
{
	...
	constructor(
		...
		this.disposables.push(
			new Disposable(() => this._providers.forEach((p) => p.dispose()))
		);
	}
	...
	private loadListOfExtensionsWithProviders() {
		const registeredList = this.globalMemento.get<
			{ extensionId: string }[]
		>(REGISTRATION_ID_EXTENSION_OWNER_MEMENTO_KEY, []);
 ...
 */
class JupyterUriProviderWrapper
	extends Disposables
	implements IInternalJupyterUriProvider
{
	...
	public async getServerUri(
		...
		if (
			...
			provider.getServerUriWithoutAuthInfo
		) {
