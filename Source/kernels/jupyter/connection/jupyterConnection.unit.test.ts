...
suite("Jupyter Connection", async () => {
	...
	setup(() => {
		...
		sinon
			...
			.callsFake(() => resolvableInstance(sessionManager));
	});
	teardown(() => {
		sinon.restore();
		dispose(disposables);
	...
	test("Validation will result in fetching kernels and kernelSpecs (Uri info provided)", async () => {
		...
		verify(
			registrationPicker.getJupyterServerUri(deepEqual(provider))
		).never();
	});
	...
	test("Validation will fail if info could not be fetched from provider", async () => {
		...
		when(sessionManager.getRunningKernels()).thenResolve([]);
		when(
			registrationPicker.getJupyterServerUri(anything(), anything())
	...
	test("Ensure Auth headers are returned", async () => {
		...
		when(
			registrationPicker.getJupyterServerUri(
				deepEqual({ id, handle, extensionId: "" })
			)
		).thenResolve(uriInfo);
		when(sessionManager.getKernelSpecs()).thenReject(
			new Error("Kaboom kernelspec failure")
