	...
	implements IJupyterServerUriStorage
{
	...
	constructor(
		...
		const storageFile = Uri.joinPath(
		...
		// eslint-disable-next-line @typescript-eslint/no-use-before-define
		this.newStorage = this._register(
			new NewStorage(fs, storageFile, this.oldStorage, globalMemento)
		);
	}
...
class OldStorage {
	...
	public async getAllRaw(): Promise<IJupyterServerUriEntry[]> {
				...
				.map(async (item, index) => {
					...
					try {
						...
						if (uri.startsWith(Identifiers.REMOTE_URI)) {
							traceError(
								`Failed to parse stored Uri information`,
								ex
							);
						}
					}
		...
		traceVerbose(
			`Found ${servers.length} saved URIs, ${JSON.stringify(servers)}`
		);
		return servers;
	}
	public async clear(): Promise<void> {
		...
		await this.globalMemento
			.update(Settings.JupyterServerUriList, [])
			.then(noop, noop);
...
class NewStorage {
	...
	async migrateMRU() {
		if (!this.migration) {
			...
			this.migration = this.migrateToStorageFile().then(() =>
				this.migrateToMemento()
			);
		}
	...
	async migrateToMemento() {
		...
		(JSON.parse(json) as StorageMRUItem[]).map((item) => {
			item.serverHandle.extensionId =
				item.serverHandle.extensionId ||
				getOwnerExtensionOfProviderHandle(item.serverHandle.id) ||
				"";
		...
		await Promise.all([
			this.fs.delete(this.storageFile).catch(noop),
			this.memento.update(this.mementoKey, items),
		]);
	}
