...
export class JupyterConnection {
	constructor(
		...
		@optional()
		private readonly requestAgentCreator:
			| IJupyterRequestAgentCreator
			| undefined,
	...
	public async validateRemoteUri(
		...
		try {
			...
			if (JupyterSelfCertsError.isSelfCertsError(err)) {
				...
				JupyterSelfCertsExpiredError.isSelfCertsExpiredError(err)
			) {
	...
	private async getJupyterServerUri(provider: JupyterServerProviderHandle) {
		try {
			return await this.jupyterPickerRegistration.getJupyterServerUri(
				provider
			);
		} catch (ex) {
