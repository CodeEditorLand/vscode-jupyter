...
@injectable()
export class RemoteJupyterServerMruUpdate
	implements IExtensionSyncActivationService
{
	...
	private onDidStartKernel(kernel: IKernel) {
		...
		if (
			...
			this.monitoredKernels.has(kernel)
		) {
			return;
		}
		...
		const updateConnectionTime = () => {
			...
			const timeout = setTimeout(() => {
				...
				this.serverStorage
					.update(connection.serverProviderHandle)
					.catch(noop);
		...
		this.kernelProvider
			...
			.onPreExecute(updateConnectionTime, this, this.disposables);
		// Log this remote URI into our MRU list
