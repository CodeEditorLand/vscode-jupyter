...
export function createJupyterConnectionInfo(
	...
	const webSocketProtocols = (serverUri?.webSocketProtocols || []).length
	...
	const getWebsocketProtocols = webSocketProtocols
		? () => webSocketProtocols
		: () => [];
	...
	const allowUnauthorized =
		configService.getSettings(undefined).allowUnauthorizedRemoteConnection;
	...
	const connection: IJupyterConnection = {
		...
		mappedRemoteNotebookDir:
			...
			(serverUri as any)?.workingDirectory,
...
export function generateIdFromRemoteProvider(
	...
	if (
		...
		return `${Identifiers.REMOTE_URI}?${Identifiers.REMOTE_URI_ID_PARAM}=${
			provider.id
		}&${Identifiers.REMOTE_URI_HANDLE_PARAM}=${encodeURI(provider.handle)}`;
	} else {
		return `${Identifiers.REMOTE_URI}?${Identifiers.REMOTE_URI_ID_PARAM}=${
			provider.id
...
function getSafeUrlForLogging(uri: string) {
	if (
		(uri || "")
			.trim()
			.toLowerCase()
			.startsWith(Identifiers.REMOTE_URI.toLowerCase())
	) {
		return uri;
	} else {
		try {
			...
			return `${url.protocol}//${
				isLocalHost ? url.hostname : "<REMOTE SERVER>"
			}:${url.port}`;
		} catch {
			return uri;
		}
