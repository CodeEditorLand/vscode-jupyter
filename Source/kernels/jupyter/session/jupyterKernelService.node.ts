...
export class JupyterKernelService implements IJupyterKernelService {
	constructor(
		...
		private readonly kernelEnvVars: KernelEnvironmentVariablesService
	) {}
	...
	public async ensureKernelIsUsable(
		...
		if (
			isKernelLaunchedViaLocalPythonIPyKernel(kernel) &&
			kernel.interpreter
		) {
			const result =
			...
			contents.metadata = {
				...
				vscode: {
					...
					originalSpecFile:
						...
						kernel.kernelSpec.specFile,
					originalDisplayName:
						kernel.kernelSpec.metadata?.vscode
							?.originalDisplayName ||
	...
	private async updateKernelEnvironment(
		...
		if (specFile && kernelSpecRootPath) {
			...
			const kernelSpecFilePath =
				path.basename(specFile).toLowerCase() ===
				kernel.name.toLowerCase()
					? specFile
					: uriPath.joinPath(
							kernelSpecRootPath,
							kernel.name,
