...
import {
	JupyterSessionWrapper,
	getRemoteSessionOptions,
} from "./jupyterSession";
...
export class JupyterKernelSessionFactory implements IKernelSessionFactory {
	...
	public async create(
		...
		try {
			...
			await raceCancellationError(
				options.token,
				this.validateLocalKernelDependencies(options)
			);

			const sessionManager = JupyterLabHelper.create(connection.settings);
			this.asyncDisposables.push(sessionManager);
			disposablesIfAnyErrors.push(
				new Disposable(() => sessionManager.dispose().catch(noop))
			);

			await raceCancellationError(
				options.token,
				this.validateRemoteServer(options, sessionManager)
			);
			...
			const workingDirectory = isLocalConnection(options.kernelConnection)
				? await raceCancellationError(
						options.token,
						this.workspaceService.computeWorkingDirectory(
							options.resource
						)
				  )
				: "";
			...
			if (isRemoteConnection(options.kernelConnection)) {
				...
				if (!connection) {
					...
					JupyterSelfCertsExpiredError.isSelfCertsExpiredError(ex)
				) {
			...
			if (
				...
				session.kernelSocketInformation = {
					get socket() {
						...
						return requestCreator.getWebsocket(
							options.kernelConnection.id
						);
					},
					options: {
						clientId: "",
				...
				await raceCancellationError(
					...
					waitForCondition(
						async () =>
							session.kernel?.connectionStatus === "connected" ||
	...
	private async createNewSession(options: {
		...
		const kernelName =
			getNameOfKernelConnection(options.kernelConnection) ??
