...
suite("New Jupyter Kernel Session Factory", () => {
	...
	const localPythonWithoutInterpreterKernelConnectionMetadata =
	...
	const localPythonWithInterpreterKernelConnectionMetadata =
		LocalKernelSpecConnectionMetadata.create({
			id: "1234",
			kernelSpec: {
				argv: [
	...
	setup(() => {
		...
		when(requestCreator.getWebsocket(anything())).thenReturn(
			instance(socket)
		);

		when(workspaceService.computeWorkingDirectory(anything())).thenResolve(
			"someDir"
		);
		when(
	...
	function createSession() {
		...
		const sessionKernelChanged = new Signal<
			...
			>
		>(instance(session));
		const sessionUnhandledMessage = new Signal<
			Session.ISessionConnection,
			KernelMessage.IMessage
		>(instance(session));
		const sessionConnectionStatusChanged = new Signal<
			Session.ISessionConnection,
			Kernel.ConnectionStatus
		>(instance(session));
		const sessionAnyMessage = new Signal<
			Session.ISessionConnection,
	...
	test("Start new Local Session (Python without interpreter info)", async () => {
		...
		const options: KernelSessionCreationOptions = {
			kernelConnection:
				localPythonWithoutInterpreterKernelConnectionMetadata,
	...
	test("Start new Local Session (Python with interpreter info)", async () => {
		...
		const options: KernelSessionCreationOptions = {
			kernelConnection:
				localPythonWithInterpreterKernelConnectionMetadata,
			creator: "jupyterExtension",
			resource,
			token: token.token,
			ui,
