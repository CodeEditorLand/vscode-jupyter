...
suite("Jupyter Interpreter Configuration", () => {
	...
	setup(() => {
		...
		when(
			commandFactory.createInterpreterCommand(
				anything(),
				anything(),
				anything(),
				anything(),
	...
	test("Prompt to install if Jupyter is not installed", async () =>
		testPromptIfModuleNotInstalled(false, true));
	test("Prompt to install if notebook is not installed", async () =>
		testPromptIfModuleNotInstalled(true, false));
