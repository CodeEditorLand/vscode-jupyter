...
export class CellExecutionQueue implements Disposable {
	private readonly queueOfItemsToExecute: (
		| ICellExecution
		| ICodeExecution
	)[] = [];

	private get queueOfCellsToExecute(): ICellExecution[] {
		return this.queueOfItemsToExecute
			.filter((c) => c.type === "cell")
			.map((c) => c as ICellExecution);
	}
		...
		while (this.queueOfItemsToExecute.length) {
			...
			try {
				if (
					...
					itemToExecute.cell.notebook.isClosed
				) {
					notebookClosed = true;
				} else if (
					itemToExecute.type === "cell" &&
			...
			if (
				itemToExecute.type === "cell" &&
				getCellMetadata(itemToExecute.cell).metadata?.tags?.includes(
					"raises-exception",
				)
			) {
			...
			if (
				...
				if (
					typeof executionResultIsSuccessful === "number" &&
					!executionResultIsSuccessful
				) {
				...
				if (
					...
					this.queueOfCellsToExecute.length ===
						this.queueOfItemsToExecute.length
				) {
					// Only dealing with cells
					// Cancel everything and stop execution.
					logger.warn(
						`Cancel all remaining cells due to ${reasons.join(" or ")}`,
					);
