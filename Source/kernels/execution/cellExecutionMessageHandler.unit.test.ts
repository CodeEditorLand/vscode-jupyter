...
suite(`Cell Execution Message Handler`, () => {
	...
	function sendRequest(cell: NotebookCell, code: string) {
		...
		const producer =
			createMessageProducers(msgIdProducer).forExecRequest(request);
		const handler =
			messageHandlerService.registerListenerForResumingExecution(cell, {
				kernel,
				msg_id: request.msg.header.msg_id,
				cellExecution:
					createKernelController().createNotebookCellExecution(cell),
			});
	...
	suite("Display Updates", () => {
		...
		test("Execute cell and update Display Data with metadata (even if Cell DOM has not yet been updated) (Issue 8621)", async () => {
			...
			notebook
				.cellAt(0)
				.outputs.slice(0, notebook.cellAt(0).outputs.length);
	...
	suite("Resume Cell Execution", () => {
		...
		test("Execute cell and resume cell execution on reload", async () =>
		...
		function resumeExecution(cell: NotebookCell, msg_id: string) {
			const handler =
				messageHandlerService.registerListenerForResumingExecution(
					cell,
					{
						kernel,
						msg_id,
