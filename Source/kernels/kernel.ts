...
abstract class BaseKernel implements IBaseKernel {
	...
	private readonly _kernelSocket = new ReplaySubject<
		KernelSocketInformation | undefined
	>();
	...
	constructor(
		...
		this.disposables.push({
			dispose: () => this._kernelSocket.unsubscribe(),
		});
	...
	public async interrupt(): Promise<void> {
		...
		traceInfo(
			`Interrupt requested ${getDisplayPath(
		...
		traceInfo(
			`Interrupt requested & sent for ${getDisplayPath(
				this.uri
			)} in notebookEditor.`
		);
		if (result === InterruptResult.TimedOut) {
			...
			const v = await this.appShell.showInformationMessage(
				message,
				{ modal: true },
				yes
			);
	...
	public async dispose(): Promise<void> {
		traceInfo(
			`Dispose Kernel '${getDisplayPath(
				this.uri
			)}' associated with '${getDisplayPath(this.resourceUri)}'`
		);
	...
	protected async startJupyterSession(
		...
		if (!this._jupyterSessionPromise) {
			...
			this._jupyterSessionPromise = this.createJupyterSession()
				...
				.catch((ex) => {
					traceInfoIfCI(
						`Failed to create Jupyter Session in Kernel.startNotebook for ${getDisplayPath(
							this.uri
						)}`
					);
		...
		this._jupyterSessionPromise
			.finally(() => this.sendKernelStartedTelemetry())
			.catch(noop);
	...
	private async createJupyterSession(): Promise<IKernelSession> {
		...
		try {
			...
			traceInfo(
				...
				} ${pythonInfo} for '${getDisplayPath(this.uri)}' (disableUI=${
					this.startupUI.disableUI
				})`
			);
	...
	protected async initializeAfterStart(session: IKernelSession | undefined) {
		...
		traceVerbose(
			`Started running kernel initialization for ${getDisplayPath(
				this.uri
			)}`
		);
		if (!session) {
        ...
        `;
			...
			if (Array.isArray(version)) {
				...
				const newVersion = (this._ipywidgetsVersion = isVersion7
					...
					: undefined);
				traceVerbose(
					`Determined IPyWidgets Version as ${newVersion} and event fired`
				);
	...
	protected getMatplotLibInitializeCode(): string[] {
		...
		if (this.kernelSettings.themeMatplotlibPlots) {
			...
			traceInfoIfCI(
				`Initialize config for plots for ${getDisplayPath(
					this.resourceUri || this.uri
				)}`
			);
			...
			traceVerbose(
				`Initialize matplotlib for ${getDisplayPath(
					this.resourceUri || this.uri
				)}`
			);
			...
			const useDark =
				this.appShell.activeColorTheme.kind === ColorThemeKind.Dark;
		...
		if (this.kernelSettings.generateSVGPlots) {
			results.push(
				...splitLines(CodeSnippets.AppendSVGFigureFormat, {
					trim: false,
				})
			);
