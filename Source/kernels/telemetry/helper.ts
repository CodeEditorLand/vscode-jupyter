...
export async function trackKernelResourceInformation(
	...
	if (kernelConnection) {
		...
		[
			...
			currentData.kernelId,
			currentData.kernelSpecHash,
		] = await Promise.all([
			language,
		...
		currentData.kernelConnectionType =
			currentData.kernelConnectionType || kernelConnection?.kind;
	} else {
		context.previouslySelectedKernelConnectionId = "";
	}
