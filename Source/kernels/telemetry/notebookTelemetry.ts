	...
	disposable.add(
		workspace.onDidOpenNotebookDocument((e) => {
			if (!notebooksOpenedTime.has(e) && !wasAnyCellExecutedInSession) {
				notebooksOpenedTime.set(
					e,
					createNotebookTracker(e, false, stopWatch),
				);
			}
		}),
	);
	...
	return {
		stop: once(() => {
			pythonExtensionActivation.completedAfter =
				mainStopWatch.elapsedTime;
		}),
	};
});

export function trackControllerCreation(
	kernelConnectionId: string,
	pythonInterpreterId?: string,
) {
	controllerCreationTimes.set(kernelConnectionId, mainStopWatch?.elapsedTime);
	controllerInterpreterMapping.set(
		kernelConnectionId,
		pythonInterpreterId || "",
	);
}
...
function createNotebookTracker(
	...
	return {
		...
		tracker: {
				...
				(kernelConnectionId: string, interpreterId?: string) => {
					...
					if (typeof controllerCreatedTime === "number") {
						if (info.wasAlreadyOpen) {
							...
							measures.controllerCreatedAfter =
								notebookOpenTimeRelativeToActivation -
								controllerCreatedTime;
						}
					}
					...
					if (!info.manuallySelectedKernel) {
						sendTelemetryForFirstAutoSelectedKernel(
							...
							{
								controllerCreatedAfter:
									measures.controllerCreatedAfter,
								kernelSelectedAfter:
									measures.kernelSelectedAfter,
								openedAfter: openedAfter,
								interpreterDiscoveredAfter:
									measures.interpreterDiscoveredAfter,
							},
						);
					}
			...
			kernelManuallySelected: once(
				() => (info.manuallySelectedKernel = true),
			),
			...
			startKernel: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("startKernel")),
			executeCell: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("executeCell")),
			...
			computeCwd: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("computeCwd")),
			getConnection: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("getConnection")),
			updateConnection: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("updateConnection")),
			kernelReady: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("kernelReady")),
			portUsage: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("portUsage")),
			spawn: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("spawn")),
			// Breakdown of spawning the kernel
			pythonEnvVars: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("pythonEnvVars")),
			envVars: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("envVars")),
			interruptHandle: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("interruptHandle")),
			// Breakdown of post startup
			kernelInfo: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("kernelInfo")),
			kernelIdle: wasAnyCellExecutedInSession
				? emptyTracker
				: once(() => createTracker("kernelIdle")),
		},
	};
}
...
const sendTelemetry = once(function (
	...
	{
		const startupSummaryParts: {
			name: keyof StartupSummary;
			start: number;
			end: number;
		}[] = [
			{
		...
		const duration =
			measures.startKernelCompletedAfter -
			measures.startKernelStartedAfter;
		computeSummary(
			startupSummary,
			startupSummaryParts,
			duration,
	...
	{
		const postKernelStartSummaryParts: {
			name: keyof PostKernelStartupSummary;
			start: number;
			end: number;
		}[] = [
			{
		...
		const duration =
			measures.postKernelStartCompletedAfter -
			measures.postKernelStartStartedAfter;
		computeSummary(
			postKernelStartSummary,
			postKernelStartSummaryParts,
			duration,
			measures.openedAfter,
		);
	}
	...
	{
		const spawnSummaryParts: {
			name: keyof SpawnSummary;
			start: number;
			end: number;
		}[] = [
			{
		...
		const duration =
			measures.spawnCompletedAfter - measures.spawnStartedAfter;
		computeSummary(
			spawnSummary,
			spawnSummaryParts,
			duration,
	...
	sendKernelTelemetryEvent(
		notebook.uri,
		Telemetry.NotebookFirstStartBreakDown,
		allMeasures,
		info,
	);
});
	...
	notebook.getCells().forEach((cell) => {
		...
		if (cell.kind === NotebookCellKind.Markup) {
			...
			try {
				...
				logger.warn(
					`Error parsing attachments in cell metadata`,
					error,
				);
			}
		} else {
			...
			notebookSummary.outputsByteSize += cell.outputs.reduce(
				(acc, output) =>
					acc +
					output.items.reduce(
						(itemTotal, item) => itemTotal + item.data.byteLength,
						0,
					),
				0,
			);
...
const sendTelemetryForFirstAutoSelectedKernel = once(function (
	...
	measures.pythonExtensionActivationStartedAfter =
	...
	if (
		...
		typeof measures.pythonExtensionActivationCompletedAfter ===
			"undefined" ||
	...
	sendKernelTelemetryEvent(
		notebook.uri,
		Telemetry.NotebookFirstKernelAutoSelectionBreakDown,
		summary,
		info,
	);
});
