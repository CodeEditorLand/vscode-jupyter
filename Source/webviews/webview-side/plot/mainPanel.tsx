...
export class MainPanel
	extends React.Component<IMainPanelProps, IMainPanelState>
	implements IMessageHandler
{
	...
	private addPlot(payload: any) {
		this.setState({
			...
			thumbnails: [
				...
				this.generateThumbnail(payload),
			],
	...
	private renderThumbnails(_baseTheme: string) {
		return (
			<SvgList
				...
				themeMatplotlibBackground={
					this.state.settings?.themeMatplotlibPlots ? true : false
				}
			/>
		);
	}
	...
	private renderToolbar(baseTheme: string) {
		...
		const deleteClickHandler =
			this.state.currentImage !== -1 ? this.deleteClicked : undefined;
		return (
			<Toolbar
				baseTheme={baseTheme}
	...
	private renderPlot(baseTheme: string) {
		...
		const currentSize =
			this.state.currentImage >= 0
				? this.state.sizes[this.state.currentImage]
				: undefined;
		const currentId =
			this.state.currentImage >= 0
				? this.state.ids[this.state.currentImage]
				: undefined;
		const value =
			this.state.currentImage >= 0
				? this.state.values[this.state.currentImage]
				: undefined;
	...
	private deleteClicked = () => {
		if (this.state.currentImage >= 0) {
			...
			const newCurrent =
				this.state.images.length > 1 ? this.state.currentImage : -1;
