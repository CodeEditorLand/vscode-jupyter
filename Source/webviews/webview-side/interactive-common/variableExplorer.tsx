...
} from "./variableExplorerButtonCellFormatter";
import {
	CellStyle,
...
> {
	...
	private requestedPagesExecutionCount = 0;
	private requestedRefreshCount = 0;
	private gridColumns: {
		key: string;
		name: string;
		type: string;
	...
	private setVariableExplorerHeight(e: MouseEvent) {
		...
		const addHeight =
			relY - variablePanel.offsetHeight - this.props.offsetHeight;
		const updatedHeight = this.state.containerHeight + addHeight;

		// min height is one row of visible data
		const minHeight =
			this.getRowHeight() * 2 + variableExplorerMenuBar.clientHeight;
		const maxHeight =
			document.body.scrollHeight -
			this.props.offsetHeight -
			variableExplorerMenuBar.clientHeight;
	...
	private ensureLoaded = (index: number) => {
		...
		if (!haveValue && (newExecution || notRequested)) {
			...
			if (
				this.requestedPagesExecutionCount !==
