...
function setVariableExplorerHeight(
	...
	if (containerHeight && gridHeight) {
		postActionToExtension(
			arg,
			InteractiveWindowMessages.SetVariableExplorerHeight,
			{
				containerHeight,
				gridHeight,
			},
...
function handleResponse(
	...
	if (
		...
		(response.executionCount === arg.prevState.currentExecutionCount &&
			arg.prevState.variables.length === 0) ||
		(response.refreshCount === arg.prevState.refreshCount &&
			arg.prevState.variables.length === 0)
	) {
...
type VariableReducerFunctions<T> = {
	[P in keyof T]: T[P] extends never | undefined
		? VariableReducerFunc
		: VariableReducerFunc<T[P]>;
};

type VariableActionMapping =
	VariableReducerFunctions<IInteractiveWindowMapping> &
