	...
	implements IMessageHandler
{
	...
	private resetGridEvent: Slick.Event<ISlickGridSlice> =
		new Slick.Event<ISlickGridSlice>();
	private resizeGridEvent: Slick.Event<void> = new Slick.Event<void>();
	private gridAddEvent: Slick.Event<ISlickGridAdd> =
		new Slick.Event<ISlickGridAdd>();
	private gridColumnUpdateEvent: Slick.Event<
		Slick.Column<Slick.SlickData>[]
	> = new Slick.Event<Slick.Column<Slick.SlickData>[]>();
	private rowFetchSizeFirst: number = 0;
	private rowFetchSizeSubsequent: number = 0;
	private rowFetchSizeAll: number = 0;
	// Just used for testing.
	private grid: React.RefObject<ReactSlickGrid> =
		React.createRef<ReactSlickGrid>();
	...
	public override render = () => {
		...
		if (
			...
			this.state.totalRowCount === this.state.fetchedRowCount &&
			!this.sentDone
		) {
			this.sentDone = true;
		...
		return (
			<div className="main-panel" ref={this.container}>
				{progressBar}
				{!this.state.deprecationBannerDismissed &&
		...
		return (
			<div className="custom-editor-banner">
				<div
					...
					onClick={this.handleDeprecationWarningClick}></div>
				<div
					className="codicon codicon-close"
					onClick={() =>
						this.setState({ deprecationBannerDismissed: true })
					}
	...
	private handleDeprecationWarningClick = (
	...
	private initializeData(payload: any) {
		if (payload) {
			...
			if (variable) {
				...
				const originalVariableType =
					variable.type ?? this.state.originalVariableType;
				const originalVariableShape =
					variable.shape ?? this.state.originalVariableShape;
	...
	private getRowsInChunks(
		...
		const chunkEnd =
			startIndex + Math.min(this.rowFetchSizeFirst, endIndex);
	...
	private handleGetRowChunkResponse(response: IGetRowsResponse) {
		...
		const after =
			response.end < this.state.gridRows.length
				? this.state.gridRows.slice(response.end)
				: [];
				...
				(
					...
					if (isDataFrameOrSeries || c.key !== indexColumn) {
						accum.push({
							...
							field:
								isDataFrameOrSeries && c.key === indexColumn
									? "index"
									: c.key.toString(),
							id: `${i}`,
							name: c.key.toString(),
	...
	private handleCheckboxToggle = (newState: boolean) => {
		this.sendMessage(DataViewerMessages.SliceEnablementStateChanged, {
			newState,
		});
	};
