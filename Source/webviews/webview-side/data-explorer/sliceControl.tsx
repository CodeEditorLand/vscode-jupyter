...
export class SliceControl extends React.Component<
	ISliceControlProps,
	ISliceControlState
> {
					...
					<form onSubmit={this.handleSubmit} className="slice-form">
						...
						<div
							...
							<div className="slice-control-column">
								...
								{this.state.errorMessage ? (
									<div
										className={`error-message${isDisabled ? "disabled" : ""}`}>
										{this.state.errorMessage}
									</div>
								) : null}
							</div>
	...
	private generateDropdowns = () => {
		...
		const width =
			measureText(
				Math.max(...this.props.originalVariableShape).toString(),
				null,
			) + 40;
		...
		for (let i = 0; i < numDropdowns; i++) {
			...
			dropdowns.push(
				<div className="slice-control-row slice-form-container">
					<label>
						{getLocString("sliceDropdownAxisLabel", "Axis:")}
					</label>
					<select
						className="slice-control-dropdown"
						style={{ marginRight: "10px", width: `${width}px` }}
						disabled={
							!this.state.isEnabled || this.props.loadingData
						}
						value={axisKey}
					...
					<label>
						{getLocString("sliceDropdownIndexLabel", "Index:")}
					</label>
					<select
						className="slice-control-dropdown"
						style={{ marginRight: "10px", width: `${width}px` }}
	...
	private renderReadonlyIndicator = () => {
		if (this.state.isEnabled && this.props.sliceExpression) {
			return (
				<span className="slice-summary-detail current-slice">
					{this.props.sliceExpression}
				</span>
			);
		}
	};
	...
	private toggleEnablement = () => {
		...
		if (willBeEnabled) {
			...
			if (
				...
				this.state.inputValue !== fullSlice
			) {
				this.props.handleSliceRequest({
					slice: this.state.inputValue,
					source: SliceOperationSource.Checkbox,
				});
			}
		} else {
			// Disabling slicing
	...
	private applyInputBoxToDropdowns = () => {
		setTimeout(() => {
			...
			if (shape.startsWith("[") && shape.endsWith("]")) {
				...
				shape
					...
					.forEach((shapeEl, idx) => {
						...
						if (match?.groups?.Start && !match.groups.Stop) {
							...
							dropdowns.push({
								axis: idx,
								index: parseInt(match.groups.Start),
							});
						} else if (
							match?.groups?.StopRange !== undefined ||
							match?.groups?.StartRange !== undefined
						) {
				...
				if (
					numRangeObjects === 0 &&
					dropdowns.length === Math.max(1, ndim - 2)
				) {
					// Apply values to dropdowns
					for (let i = 0; i < dropdowns.length; i++) {
						const selection = dropdowns[i];
						(state as any)[`selectedAxis${i.toString()}`] =
							selection.axis;
						(state as any)[`selectedIndex${i.toString()}`] =
							selection.index;
					}
				} else {
					// Unset dropdowns
	...
	private applyDropdownsToInputBox = () => {
		setTimeout(() => {
			...
			for (let i = 0; i < numDropdowns; i++) {
				const selectedAxisKey = this.state[
					`selectedAxis${i}`
				] as number;
				const selectedIndexKey = this.state[
					`selectedIndex${i}`
				] as number;
				if (
			...
			if (
				...
				this.props.handleSliceRequest({
					slice: newSliceExpression,
					source: SliceOperationSource.Dropdown,
				});
			}
		});
	};
	...
	private generateIndexDropdownOptions = (dropdownIndex: number) => {
		...
		if (axisSelection !== undefined) {
			const range =
				this.props.originalVariableShape[axisSelection as number];
			const result = [];
