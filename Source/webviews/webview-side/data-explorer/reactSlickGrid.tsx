...
class ColumnFilter {
	...
	private lessThanRegEx =
		/^\s*<\s*((?<Number>-?\d+.*)|(?<NaN>nan)|(?<Inf>inf)|(?<NegInf>-inf))/i;
	private lessThanEqualRegEx =
		/^\s*<=\s*((?<Number>-?\d+.*)|(?<NaN>nan)|(?<Inf>inf)|(?<NegInf>-inf)).*/i;
	private greaterThanRegEx =
		/^\s*>\s*((?<Number>-?\d+.*)|(?<NaN>nan)|(?<Inf>inf)|(?<NegInf>-inf)).*/i;
	private greaterThanEqualRegEx =
		/^\s*>=\s*((?<Number>-?\d+.*)|(?<NaN>nan)|(?<Inf>inf)|(?<NegInf>-inf)).*/i;
	private equalToRegEx =
		/^\s*(?:=|==)\s*((?<Number>-?\d+.*)|(?<NaN>nan)|(?<Inf>inf)|(?<NegInf>-inf)).*/i;
	...
	private generateNumericOperation(text: string): (v: any) => boolean {
		if (this.nanRegEx.test(text)) {
			...
			return (v: any) =>
				v !== undefined &&
				(v <= n2 || (Number.isNaN(v) && Number.isNaN(n2)));
...
> {
	...
	private dataView: Slick.Data.DataView<ISlickRow> =
	...
	private autoResizedColumns = false;
	...
	private updateGridSize = () => {
		if (
			...
			const height =
				this.measureRef.current.offsetTop -
				this.containerRef.current.offsetTop;
			this.containerRef.current.style.height = `${
				this.props.forceHeight ? this.props.forceHeight : height
			}px`;
				...
				if (c.field !== this.props.idProperty) {
					...
					c.header = {
						buttons: [
							{
								cssClass:
									"codicon codicon-filter codicon-button header-cell-button",
							...
							{
								cssClass:
									"codicon codicon-refresh codicon-button header-cell-button refresh-button",
	...
	private renderFilterCell = (
		...
		if (args.column.field === this.props.idProperty) {
			...
			const filter = args.column.field
				? this.columnFilters.get(args.column.field)?.text
				: "";
			ReactDOM.render(
	...
	private compareElements(
			...
			if (sortColumn && col.hasOwnProperty("type")) {
				...
				if (isStringColumn) {
					...
					if (a[sortColumn].toString().toLowerCase() === "nan") {
						...
						b[sortColumn].toString().toLowerCase() === "nan"
					) {
						return -1;
					}
		...
		if (
			a.hasOwnProperty(this.props.idProperty) &&
			b.hasOwnProperty(this.props.idProperty)
		) {
