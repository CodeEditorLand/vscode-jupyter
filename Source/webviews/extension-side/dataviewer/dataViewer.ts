...
export class DataViewer
	extends WebviewPanelHost<IDataViewerMapping>
	implements IDataViewer, IDisposable
{
	...
	public async refreshData() {
		...
		if (
			currentSliceExpression !== undefined &&
			dataFrameInfo.shape !== undefined
		) {
			if (
				isValidSliceExpression(
	...
	private getDataFrameInfo(
		...
		if (
			!this.dataFrameInfoPromise ||
			sliceExpression !== this.currentSliceExpression
		) {
					...
					.then((v) => {
						...
						if (v === actionTitle) {
							this.applicationShell.openUrl(
								HelpLinks.JupyterDataRateHelpLink
							);
						}
					}, noop);
				this.dispose();
