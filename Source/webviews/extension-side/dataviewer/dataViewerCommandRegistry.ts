	...
	implements IExtensionSyncActivationService
{
	...
	private async delegateDataViewer(
		request: IJupyterVariable | IShowDataViewerFromVariablePanel,
	) {
		const variable =
			"variable" in request
	...
	private async getVariableFromRequest(
	...
	private async showJupyterVariableView(requestVariable: IJupyterVariable) {
		...
		if (
			...
			this.globalMemento
				.update(PromptAboutDeprecation, true)
				.then(noop, noop);
