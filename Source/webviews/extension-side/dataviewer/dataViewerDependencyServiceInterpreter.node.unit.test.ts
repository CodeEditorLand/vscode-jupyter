...
suite("DataViewerDependencyService (PythonEnvironment, Node)", () => {
	...
	setup(async () => {
		interpreter = {
			displayName: "",
			id: Uri.file(path.join("users", "python", "bin", "python.exe"))
				.fsPath,
	...
	test("All ok, if pandas is installed and version is > 1.20", async () => {
		when(
			pythonExecService.exec(
				deepEqual(["-c", "import pandas;print(pandas.__version__)"]),
				anything()
			)
	...
	test("Throw exception if pandas is installed and version is = 0.20", async () => {
		...
		const promise =
			dependencyService.checkAndInstallMissingDependencies(interpreter);
