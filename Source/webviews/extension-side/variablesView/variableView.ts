...
export class VariableView
	extends WebviewViewHost<IVariableViewPanelMapping>
	implements IDisposable
{
	...
	public async load(codeWebview: vscodeWebviewView) {
		await super
			.loadWebview(Uri.file(process.cwd()), codeWebview)
			.catch(traceError);
	...
	protected handleMessage<
		M extends IVariableViewPanelMapping,
		T extends keyof M,
	>(
		_message: T,
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
	...
	private handleVisibilityChanged() {
		const context = new ContextKey(
			"jupyter.variableViewVisible",
			this.commandManager
		);
		let visible = false;
		if (this.webviewView) {
		...
		if (visible) {
			...
			if (
				this.notebookWatcher.activeNotebookExecutionCount !== undefined
			) {
				this.postMessage(
		...
		try {
			if (
				...
				if (variableViewers.length === 0) {
					...
					// show quick pick
					const quickPick = this.appShell.createQuickPick<
